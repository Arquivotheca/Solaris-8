...
#pragma ident	"@(#)rpc.c	1.31	99/10/14 SMI"
...
/*ARGSUSED*/
static bool_t
rpc_hdr(XDR *xdrs, u_int xid, rpcprog_t prog, rpcvers_t vers, rpcproc_t proc)
{
...
brpc_call(
	...
	u_int			auth)		/* type of auth wanted. */
{
	int s;
	...
	if (dontroute) {
		(void) setsockopt(s, IPPROTO_IP, SO_DONTROUTE,
		    (const void *)&dontroute, sizeof (dontroute));
	}

	if (to->sin_addr.s_addr == cached_destination.s_addr) {
		(void) getsockopt(s, IPPROTO_IP, SO_RCVTIMEO, (void *)&timeout,
		    sizeof (timeout));
	} else {
	...
	/*
	 * Set our local port to a random number within the last quandrant of
	 * the IPPORT_RESERVED space. We are a privileged "process".
	 */
	from.sin_port = htons(IPPORT_RESERVED -
	    (in_port_t)(prom_gettime() & (IPPORT_RESERVED/4 - 1)) - 1);
	...
	(void) setsockopt(s, IPPROTO_IP, SO_RCVTIMEO, (void *)&xdelay,
	    sizeof (xdelay));
