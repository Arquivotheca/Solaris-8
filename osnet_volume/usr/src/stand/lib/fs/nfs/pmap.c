...
#pragma ident	"@(#)pmap.c	1.26	99/02/23 SMI"
...
struct pmaplist pre_init[3] = {
	...
	{ {NFS_PROGRAM, NFS_VERSION, IPPROTO_UDP, NFS_PORT}, NULL }
};
...
#ifdef DEBUG
	printf("bpmap_getport: called with: prog: 0x%x, vers: 0x%x\n",
	    prog, vers);
#endif /* DEBUG */
...
#ifdef DEBUG
	printf("bpmap_getport: prog: %x, vers: %x; returning port: %d.\n",
	    prog, vers, dport);
#endif /* DEBUG */
