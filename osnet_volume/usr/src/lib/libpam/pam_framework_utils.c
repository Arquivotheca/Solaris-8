...
#pragma ident	"@(#)pam_framework_utils.c	1.12	99/05/04 SMI"
...
static int __pam_input_output(pam_handle_t *, int, int,
	char [PAM_MAX_NUM_MSG][PAM_MAX_MSG_SIZE], void *,
	struct pam_response **);
	...
	struct pam_response	**ret_respp;
{
	...
	/*
	 * The UNIX pam module always calls __pam_get_input() and
...
__pam_get_authtok(pam_handle_t *pamh, int source, int type, int len,
	char *prompt, char **authtok, void *conv_apdp)
{
	...
	case PAM_PROMPT:
		...
		if ((error = __pam_get_input(pamh, PAM_PROMPT_ECHO_OFF,
			1, messages, conv_apdp, &ret_resp)) != PAM_SUCCESS) {
			free(*authtok);
			*authtok = NULL;
			return (error);
		}
