...
#pragma	ident	"@(#)gethostent.c	1.34	99/09/20 SMI"
	...
	void			*a;
{
	...
	/*
	 * The BIND resolver set retry to 4 by default but that
	 * is too many as we want the NSswitch engine to control the retries.
	 */
	if (set_res_retry != 0)
		((*set_res_retry)())->retry = 1;
	...
	void			*a;
{
	...
	nss_XbyY_args_t	*argp = (nss_XbyY_args_t *)a;
	int		ret, save_h_errno, mt_disabled = 1;
	...
	/*
	 * The BIND resolver sets retry to 4 by default but that
	 * is too many as we want the NSswitch engine to control the retries.
	 */
	if (set_res_retry != 0)
		((*set_res_retry)())->retry = 1;
