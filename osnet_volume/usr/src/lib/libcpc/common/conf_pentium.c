...
#pragma ident	"@(#)conf_pentium.c	1.2	99/08/15 SMI"
	...
	if (ver == -1) {
		...
		if ((buf = malloc(bufsize)) != NULL) {
			if (sysinfo(SI_ISALIST, buf, bufsize) == bufsize) {
				if (strstr(buf, "pentium_pro+mmx") != NULL)
					ver = CPC_PENTIUM_PRO_MMX;
				else if (strstr(buf, "pentium_pro") != NULL)
					ver = CPC_PENTIUM_PRO;
				else if (strstr(buf, "pentium+mmx") != NULL)
					ver = CPC_PENTIUM_MMX;
