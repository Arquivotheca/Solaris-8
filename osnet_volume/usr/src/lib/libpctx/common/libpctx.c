...
#pragma ident	"@(#)libpctx.c	1.2	99/11/20 SMI"
    ...
    pctx_errfn_t *errfn)
{
	...
	if ((pctx->Pr = Pcreate(filename, argv, &err, 0, 0)) == NULL) {
		...
		case C_LP64:
			...
			break;
		case C_NOEXEC:
			pctx_error(pctx, fn, gettext("cannot find/execute "
			    "program '%s'\n"), filename);
			break;
		case C_FORK:
    ...
    int cmd, id_t lwpid, void *data, int flags, size_t size)
{
	...
	rval = Psyscall(pctx->Pr, SYS_cpc, 4, &argd[0]);
