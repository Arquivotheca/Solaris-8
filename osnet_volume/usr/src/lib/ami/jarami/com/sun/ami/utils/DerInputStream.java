 ...
 * All rights reserved.
 * #pragma ident "@(#)DerInputStream.java	1.1 99/07/11 SMI"
 *
 */
...
public class DerInputStream {
    ...
    public String getPrintableString() throws IOException
    {
        ...
        //System.out.println("Reading printable string");
	if (buffer.read() != DerValue.tag_PrintableString)
	    throw new IOException("DER input not a printable string");

        //System.out.println("Found tag for printable string");

	int length = getLength(buffer);

        //System.out.println("length of PS = " + length);

	byte[] retval = new byte[length];
	if (buffer.read(retval) != length)
	    throw new IOException("short read of DER printable string");

        //System.out.println("read bytes = " + retval.length);
    ...
    {
	if (buffer.read() != DerValue.tag_IA5String)
	    throw new IOException("DER input not a IA5 string");

	int length = getLength(buffer);
	byte[] retval = new byte[length];
	if (buffer.read(retval) != length)
	    throw new IOException("short read of DER IA5 string");
  ...
  /*
    ...
    {
	throw new IOException("DerInputStream.getT61String() NYI");
    }
   */
