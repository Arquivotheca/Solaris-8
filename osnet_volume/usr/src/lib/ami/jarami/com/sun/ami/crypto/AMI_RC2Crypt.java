 ...
 * All rights reserved.
 * #pragma ident "@(#)AMI_RC2Crypt.java	1.1 99/07/11 SMI"
 *
 */
...
import com.sun.ami.keygen.AMI_RC2Key;

/**
 * This class is the internal RC2 class that does encryption and 
 * decryption on an array. 
 * <P>The encryption and decryption is defined by the RC2 algorithm.
 *
...
public class AMI_RC2Crypt extends AMI_SymmetricCipher {
    ...
    public void init(Key key, AlgorithmParameterSpec params)
        throws InvalidKeyException, InvalidAlgorithmParameterException {
            ...
            if ((params != null) && (params instanceof RC2ParameterSpec)) {
                RC2ParameterSpec rc2params = (RC2ParameterSpec) params;
                _iv = rc2params.getIV();
                if (_iv == null || _iv.length != 8) {
                    throw new InvalidAlgorithmParameterException
                        ("Wrong IV length: must be 8 bytes long");
                }
		_effectiveKeySize = rc2params.getEffectiveKeyBits();
            } else {
                throw new InvalidAlgorithmParameterException
                    ("Wrong parameter type: RC2ParameterSpec expected");
            }
    }
    ...
    private void cipherBlock(byte[] input, int inputLen, byte[] output) {
	 ...
	 if (_decrypting) {  // Decrypting
	   try {
		...
		_crypto.ami_rc2_decrypt(input, inputLen, _stage,
		    _key, _key.length, _effectiveKeySize, _iv);
	   ...
	   try {
		if (!_crypto.getAMIHandleValid()) {
			...
			_crypto.setAMIHandleValid(true);
   	       ...
   	       _crypto.ami_rc2_encrypt(input, inputLen, _stage,
		    _key, _key.length, _effectiveKeySize, _iv);
	   } catch (AMI_CryptoException e) {
		e.printStackTrace();
	   }	
