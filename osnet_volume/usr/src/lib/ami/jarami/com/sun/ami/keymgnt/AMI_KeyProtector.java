 ...
 * All rights reserved.
 * #pragma ident "@(#)AMI_KeyProtector.java	1.1 99/07/11 SMI"
 *
 */
...
final class AMI_KeyProtector {
     ...
     */
    public AMI_KeyProtector(char[] password)
	...
	try {
	    return (new EncryptedPrivateKeyInfo(encrAlg,
		encrKey).getEncoded());
	...
	try {
	    return PKCS8Key.parse(new DerValue(plainKey));
	} catch (IOException ioe) {
