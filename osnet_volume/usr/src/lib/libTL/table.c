...
#ident	"@(#)table.c	1.5	96/04/04 SMI"       /* SVr4.0 1.9 */
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
#include <errno.h>
...
TLdesc_t *descr;
{
	...
	/*
	 * stat the file before the open to determine
	 * whether or not it exists
	 */
	if (stat64((char *) filename, &buf) == -1) {
		if(errno == ENOENT) {
			tptr->file.mode = mode;
		...
		/* Open the Table file */
		if((tptr->file.fid =
		    open((char *)filename, oflag, tptr->file.mode)) == -1) {
			TLt_unget(tid);
			return (TLFAILED);
		}
	}
	...
	if(!(tptr->file.name =
	    (unsigned char *)Malloc(Strlen(filename) + 1))) {
		TLt_unget( tid );
		return( TLNOMEMORY );
	}
