...
#pragma ident	"@(#)kvm.c	1.5	98/07/29 SMI"
...
{
	kvm_t *kd;
	struct stat64 memstat, kmemstat, corestat;
	...
	if (S_ISCHR(corestat.st_mode) &&
	    (corestat.st_rdev == memstat.st_rdev ||
	    corestat.st_rdev == kmemstat.st_rdev)) {
		if ((kd->kvm_kmemfd = open64("/dev/kmem", flag)) == -1)
			return (fail(kd, err, "cannot open /dev/kmem"));
