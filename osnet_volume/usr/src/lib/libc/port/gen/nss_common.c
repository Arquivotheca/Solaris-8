...
#pragma ident	"@(#)nss_common.c	1.26	99/12/05 SMI"
...
{
	fprintf(__nss_debug_file,
		"NSS_retry(%d): '%s': trying '%s' ... ",
		n, dbase, lkp->service_name);
	fflush(__nss_debug_file);
...
{
	fprintf(__nss_debug_file, "result=");
	switch (res) {
	case NSS_SUCCESS:
		fprintf(__nss_debug_file, "SUCCESS");
		break;
	case NSS_NOTFOUND:
		fprintf(__nss_debug_file, "NOTFOUND");
		break;
	case NSS_UNAVAIL:
		fprintf(__nss_debug_file, "UNAVAIL");
		break;
	case NSS_TRYAGAIN:
		fprintf(__nss_debug_file, "TRYAGAIN");
		break;
	case NSS_NISSERVDNS_TRYAGAIN:
		fprintf(__nss_debug_file, "NISSERVDNS_TRYAGAIN");
		break;
	default:
		fprintf(__nss_debug_file, "undefined");
	}
	fprintf(__nss_debug_file, ", action=");
	switch (lkp->actions[res]) {
	case __NSW_CONTINUE:
		fprintf(__nss_debug_file, "CONTINUE");
		break;
	case  __NSW_RETURN:
		fprintf(__nss_debug_file, "RETURN");
		break;
	case __NSW_TRYAGAIN_FOREVER:
		fprintf(__nss_debug_file, "TRYAGAIN_FOREVER");
		break;
	case __NSW_TRYAGAIN_NTIMES:
		fprintf(__nss_debug_file, "TRYAGAIN_NTIMES (N=%d)",
			lkp->max_retries);
		break;
	default:
		fprintf(__nss_debug_file, "undefined");
...
retry_test(nss_status_t res, int n, struct __nsw_lookup_v1 *lkp)
{
	...
	if ((res == NSS_TRYAGAIN &&
		...
		if (n <= lkp->max_retries)
			...
			lkp->actions[res] = __NSW_CONTINUE;
			return (0);
		}
