...
#pragma ident	"@(#)fmtmsg.c	1.10	96/11/15 SMI"	/* SVr4.0 1.9	*/
...
msgverbset(void)
{
	...
	if ((opts = getenv(MSGVERB)) == (char *) NULL) {
		...
		if (!(alloced =
		    (char *) malloc(strlen(opts) + (size_t)1))) {
			msgverb = MV_DFLT;
		} else {
...
sevstrset(void)
{
	...
	if ((value = getenv(SEV_LEVEL)) != (char *) NULL) {
		...
		if (sevspace =
		    (char *) malloc(strlen(value) + (size_t)1)) {
...
addseverity(int value, const char *string)
{
	...
	if (!found && (string != (char *) NULL)) {
		...
		if ((p = (struct sevstr *) malloc(sizeof (struct sevstr))) ==
		    (struct sevstr *) NULL) {
			(void) _mutex_unlock(&fmt_lock);
			return (-1);
		}
...
static int
writemsg(FILE *stream, int verbosity, const char *label, int severity,
...
const char *text, const char *action, const char *tag)
{
	...
	if (class & MM_PRINT) {
		if (writemsg(stderr, msgverb, label,
		    severity, text, action, tag))
			rtnval |= MM_NOMSG;
	}
	...
	if (class & MM_CONSOLE) {
		if (console = fopen(CONNAME, "w")) {
			if (writemsg(console, MV_ALL, label, severity,
			    text, action, tag)) {
				rtnval |= MM_NOCON;
			}
			(void) fclose(console);
