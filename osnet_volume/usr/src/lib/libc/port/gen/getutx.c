...
#pragma ident	"@(#)getutx.c	1.29	99/05/04 SMI"
...
static void	*invoke_utmp_update(const struct utmpx *);
...
invoke_utmp_update(const struct utmpx *entryx)
{
	...
	if (childpid = fork()) {
		...
		(void) execl(UTMP_UPDATE, UTMP_UPDATE,
...
utmpname(const char *newfile)
{
	...
	if (strcmp(newfile, "/var/adm/utmp") == 0 ||
		...
		(void) strncpy(name, newfile, strlen(newfile));
		(void) strcat(name, "x");
...
updwtmp(const char *file, struct utmp *ut)
{
	...
	if (strcmp(file, "/var/adm/wtmp") == 0) {
		(void) strncpy(xfile, file, strlen(file));
		(void) strcat(xfile, "x");
		getutmpx(ut, &utmpx);
...
makeutx(const struct utmpx *utmp)
{
	...
	int wild;			/* flag, true if wild card char seen */
