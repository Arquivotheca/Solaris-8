...
#pragma ident	"@(#)hsearch.c	1.13	96/12/04 SMI"	/* SVr4.0 1.11	*/
...
#define	RETURN(n)    { (void) _mutex_unlock(&table_lock); return (n); }
#else
	...
	/* Minimum "size" for hash table */
{
	...
	(void) _mutex_lock(&table_lock);
	if (size == 0) {
		(void) _mutex_unlock(&table_lock);
		return (FALSE);
	}
