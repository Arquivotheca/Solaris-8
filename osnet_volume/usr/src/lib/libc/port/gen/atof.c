...
#pragma	ident	"@(#)atof.c	1.14	99/04/09 SMI"	/* SVr4.0 2.25	*/
...
#if STRTOD
#define	STORE_PTR	(*ptr = (char *)up)
#define	DEC_PTR		(*ptr = (char *)(up - 1))
...
#endif
{
	...
	STORE_PTR;
#endif
	while (isspace(c = *up)) /* eat leading white space */
		up++;
	...
	{	/* accumulate value */
		...
		while (isdigit(c = *up++) || c == dpchar && !decpt++) {
			if (c == dpchar)
				continue;
			GOT_DIGIT;
	...
	if (c == 'E' || c == 'e') { /* accumulate exponent */
		...
		if (isdigit(c = *up)) { /* found a legitimate exponent */
			do {
