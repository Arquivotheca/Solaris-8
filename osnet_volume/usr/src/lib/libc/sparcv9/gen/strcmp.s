...
.ident	"@(#)strcmp.s	1.10	97/02/09 SMI"
...
3:	clr	%i3
	andcc	%i5, %l0, %g0		! check if first byte was zero
	be,pn	%icc, 4f
	andcc   %i5, %l1, %g0           ! check if second byte was zero
	be,pn	%icc, 4f
	andcc   %i5, %l2, %g0           ! check if third byte is zero
	be,pn	%icc, 4f
	nop
...
4:	clr	%i3
	...
	nop
  	...
  	lduw	[%i1], %i3		! read a word from s1
5:	lduw	[%i1 + %i0], %i4	! read a word from s2
	inc	4, %i1
