...
.ident	"@(#)divrem.m4.s	1.22	95/07/27 SMI"
...
',`
#if defined(PIC)
	ENTRY(.urem_sw)		! UNSIGNED REMAINDER
#else
	ENTRY(.urem)
#endif
	save	%sp,-96,%sp
...
1:
ifelse( ANSWER, `quotient',
`	mov	%i2,%i0	! quotient  <-  Q
',`	mov	%i3,%i0	! remainder <-  R
')
	ret
	restore
