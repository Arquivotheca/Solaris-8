...
#pragma ident	"@(#)__floatprint.c	1.6	96/12/06 SMI"
...
__base_conversion_set_exception(fp_exception_field_type ef)
{
	...
	__base_conversion_write_only_double = t * t;
...
void
__double_to_digits(double x, char *s, int *pn)
...
double
__arint_set_n(double x, int nrx, enum __fbe_type *pe)
...

{
	...
	case 1:
		*pe = (__abs_ulp(rx) < (0.5 - rmx)) ? __fbe_one : __fbe_many;
		break;
	default:
		*pe = ((2.0 * __abs_ulp(rx)) < (0.5 - rmx)) ?
				__fbe_one : __fbe_many;
		break;
	}
	return (rx);
}
