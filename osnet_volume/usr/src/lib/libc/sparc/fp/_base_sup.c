...
#pragma	ident	"@(#)_base_sup.c	1.7	96/12/06 SMI"
/*LINTLIBRARY*/
...
void
__display_big_float(_big_float *pbf, unsigned int base)
{
	int	i;
	...
	for (i = 0; i < pbf->blength; i++) {
		switch (base) {
		case 2:
			printf(" + %d * 2** %d", pbf->bsignificand[i],
			    (16 * i + pbf->bexponent));
			break;
		case 10:
			printf(" + %d * 10** %d", pbf->bsignificand[i],
			    (4 * i + pbf->bexponent));
			break;
		}
		if ((i % 4) == 3)
...
void
__integerstring_to_big_decimal(char ds[], unsigned int ndigs,
		unsigned int nzin, unsigned int *pnzout, _big_float *pd)
...
#define	IDIGIT(i) ((i < 0) ? 0 : ((i < ndigs) ? (ds[i] - '0') : 0))
	...
	pd->bsignificand[id] = 1000 * IDIGIT(ids) + 100 * IDIGIT(ids + 1) + 10
	    * IDIGIT(ids + 2) + IDIGIT(ids + 3);
	ids += 4;

	for (; ids < (int) (ndigs + taken - 4); ids += 4) {
		/*
		...
		id--;
		pd->bsignificand[id] = 1000 * ds[ids] + 100 * ds[ids + 1] + 10
		    * ds[ids + 2] + ds[ids + 3] - 1111 * '0';
	}
...
#endif
	if (id != 0)
		pd->bsignificand[0] = 1000 * IDIGIT(ids) + 100 * IDIGIT(ids + 1)
		    + 10 * IDIGIT(ids + 2) + IDIGIT(ids + 3);
...
void
__fractionstring_to_big_decimal(char ds[], unsigned int ndigs,
		unsigned int nzin, _big_float *pbf)
{
	/*
...
#define	FDIGIT(i) ((i < nzin) ? 0 : ((i < (nzin+ndigs)) ?  \
		(ds[i-nzin] - '0') : 0))

	pbf->bsignificand[ibf] = 1000 * FDIGIT(ids) + 100 * FDIGIT(ids + 1)
	    + 10 * FDIGIT(ids + 2) + FDIGIT(ids + 3);
	ids += 4;

	for (; ids < (int) (nzin + ndigs - 4); ids += 4) {
		/*
		 * Additional digits to
		 * be found. Main loop.
		 */
		ibf--;
		pbf->bsignificand[ibf] = 1000 * ds[ids - nzin] + 100 *
		    ds[ids + 1 - nzin] + 10 * ds[ids + 2 - nzin] +
		    ds[ids + 3 - nzin] - 1111 * '0';
	}
	...
	if (ibf > 0) {
		...
		assert(ibf == 1);
#endif
		pbf->bsignificand[0] = 1000 * FDIGIT(ids) + 100 *
		    FDIGIT(ids + 1) + 10 * FDIGIT(ids + 2) + FDIGIT(ids + 3);
	}
#ifdef DEBUG
	else
	{
...
unsigned int
__prod_10000_b65536(_BIG_FLOAT_DIGIT x, unsigned int c)
/* p = x * 10000 + c ; return p */
{
	return (x * (unsigned int) 10000 + c);
}

void
__mul_10000short(_big_float *pbf, unsigned int carry)
{
	int	j;
	unsigned int 	p;
...
void
__big_decimal_to_big_binary(_big_float *pd, _big_float *pb)
{
	...
	if ((id == 0) && (pd->bexponent < 0)) {
		...
		idbound = (pd->bexponent < 0) ? 1 : 0;
		/*
		 * How far to carry next
		 * for loop depends on
		 * whether last digit
		 * requires special
		 * treatment.
		 */
		for (; id >= idbound; id--) {
		__mul_10000short(pb, (unsigned int) pd->bsignificand[id]);
		}
	}
	if (pd->bexponent < 0) {
		/*
		 * Have to save some integer bits, discard
		...
		switch (pd->bexponent) {
		case -1:
			carry = __quorem((unsigned int) carry, 10, &sticky);
			multiplier = 1000;
			break;
		case -2:
			carry = __quorem((unsigned int) carry, 100, &sticky);
			multiplier = 100;
			break;
		case -3:
			carry = __quorem((unsigned int) carry, 1000, &sticky);
			multiplier = 10;
			break;
		default:
/* Ifdef'ed out diagnostic message to keep tsort happy. */
#ifdef DEBUG
	(void) sprintf(bcastring, " __big_decimal_to_big_binary exponent %d ",
	    pd->bexponent);
#endif
			__base_conversion_abort(ERANGE, bcastring);
		}
		__multiply_base_two(pb, multiplier, (unsigned int) carry);
		if (sticky != 0)
...
void
__big_binary_to_unpacked(_big_float *pb, unpacked *pu)
{
...
#ifdef DEBUG
	assert(pb->bsignificand[pb->blength - 1] != 0);
	/*
	 * assert pb is
	 * normalized.
	 */
	...
	iu = 0;
	for (ib = pb->blength - 1; ((ib - 1) >= 0) &&
			    (iu < UNPACKED_SIZE); ib -= 2) {
		pu->significand[iu++] = pb->bsignificand[ib] << 16 |
		    pb->bsignificand[ib - 1];
	}
	if (iu < UNPACKED_SIZE) {
		/*
		...
		/*
		 * The big float is too big; chop, stick, and
		 * normalize.
		 */
...
void
__left_shift_base_two(_big_float *pbf, unsigned short multiplier)
{
	/*
...
double
__digits_to_double(char *s, int n, enum __fbe_type *pe)
/*
		...
		*pe = __fbe_none;
		...
		for (i = n - 8; i < n; i++) {
			...
			acc = t2 + t8 + s[i];
					...
					/* nine '0' worth */
		t = __add_set(t, (double) (acc - 0x3DE43550), pe);
	}
	return (t);
}
