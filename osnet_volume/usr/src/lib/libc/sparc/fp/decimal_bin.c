...
#pragma ident	"@(#)decimal_bin.c	1.10	97/08/08 SMI"
...
void
__decimal_to_binary_integer(char ds[], unsigned ndigs, unsigned nzeros,
				unsigned nsig, _big_float *pb)
...

{
	...
	if (nzout != 0) {
		__big_float_times_power(pb, 10, (int)nzout, (int)nsig,
					&pbout);
		if (pbout == BIG_FLOAT_TIMES_TOOBIG) {
#ifdef DEBUG
			(void) printf(" __decimal_to_binary_integer: "
			"decimal exponent %d too large for tables \n", nzout);
#endif
			pb->bexponent = 0x7fff;
...
/* Ifdef'ed out diagnostic message to keep tsort happy. */
#ifdef DEBUG
			(void) sprintf(bcastring, " decimal exponent %d ",
					nzout);
#endif
			...
			if (pbout != pb)
				(void) printf(" __decimal_to_binary_integer: "
				"large decimal exponent %d needs heap "
				"buffer \n", nzout);
			...
			if (pbout != pb) {
						 ...
						 * a quad!
						 */
				unsigned 	i, allweneed;

				allweneed = 2 + (nsig + 2) / 16;
				for (i = 0; i < allweneed; i++)
					pb->bsignificand[i] =
						pbout->bsignificand[i +
						pbout->blength - allweneed];
				for (i = 0; (pbout->bsignificand[i] == 0); i++);
				if (i < ((int)pbout->blength - allweneed))
					pb->bsignificand[0] |= 1;
				/* Stick discarded bits. */

				pb->blength = (unsigned short)allweneed;
				pb->bexponent = pbout->bexponent +
					16 * (pbout->blength - allweneed);
#ifdef DEBUG
				printf(" __decimal_to_binary_integer: "
				"removed %d excess digits from product \n",
				pbout->blength - allweneed);
				__display_big_float(pb, 2);
#endif
...
void
__decimal_to_binary_fraction(char ds[], unsigned ndigs, unsigned nzeros,
				unsigned nsig, _big_float *pb)
/* Input decimal integer string. */
/* Input number of explicit digits in ds. */
/* Input number of implicit trailing zeros. */
...

{
	...
	twopower = nsig + 3 + (((nzeros + 1) * (unsigned int) 217706) >> 16);
	twosig = 1 + (((nsig + 2) * (unsigned int) 19729) >> 16);

#ifdef DEBUG
	printf(" __decimal_to_binary_fraction sigbits %d twopower %d twosig "
	"%d\n", nsig, twopower, twosig);
#endif
	__big_float_times_power(&d, 2, (int)twopower, (int)twosig, &pdout);
	if (pdout == BIG_FLOAT_TIMES_TOOBIG) {
#ifdef DEBUG
		(void) printf(" __decimal_to_binary_fraction binary exponent "
		"%d too large for tables ", twopower);
#endif
		pb->bexponent = 0x7fff;
		goto ret;
...
/* Ifdef'ed out diagnostic message to keep tsort happy. */
		...
		if (&d != pdout)
			printf(" __decimal_to_binary_fraction large binary "
			"exponent %d needs heap buffer \n", twopower);
		printf(" product ");
...
#endif
		for (i = 0; (i < excess) && ((pdout)->bsignificand[i] == 0);
			i++);
		if (i < excess)
			(pdout)->bsignificand[excess] |= 1;
		/* Sticky bit for discarded fraction. */
		for (i = excess; i < (int)(pdout)->blength; i++)
			(pdout)->bsignificand[i - excess] =
				(pdout)->bsignificand[i];
...
void
__decimal_to_unpacked(unpacked *px, decimal_record *pd,
...

{
	...
	if (ndigs != 0) {	/* Convert integer digits. */
		...
		__decimal_to_binary_integer(pd->ds, ndigs, ntz,
			significant_bits, &bi);
	...
	bf.blength = 0;
	if ((nfrac != 0) && (frac_bits > 0)) {
	/* Convert fraction digits, even if we only need a round or sticky. */

		bf.bsize = _BIG_FLOAT_SIZE;
		__decimal_to_binary_fraction(&(pd->ds[ifrac]), nfrac, nlz,
				(unsigned)frac_bits, &bf);
	...
	if (bf.blength != 0) {	/* Combine integer and fraction bits. */
		int	expdiff = bi.bexponent - (bf.bexponent +
			16 * (bf.blength - 1));	/* Exponent difference. */
		int	uneeded = 2 + (significant_bits + 2) / 16;
				/* Number of big float digits needed. */
		...
		assert(bf.blength != 0);
		/* Normalized bi. */
		assert(bf.bsignificand[bf.blength - 1] != 0);
		/* Normalized bi. */
		assert(bf.bsignificand[bf.blength - 1] != 0);
		assert(bi.bexponent >= 0);	/* bi is all integer */
		assert(((-bf.bexponent - 16 * (bf.blength - 1)) >= 16) ||
			((bf.bsignificand[bf.blength - 1] >>
			(-bf.bexponent - 16 * (bf.blength - 1))) == 0));
		...
		if (leftshift > 0) {	/* shift bf to align with bi. */
			...
			__left_shift_base_two(&bf, (unsigned short)leftshift);
			expdiff -= 16 * bf.blength;
	/* if bf.blength is longer, adjust expdiff. */
		}
		expdiff += leftshift;
...
#ifdef DEBUG
		/*
		 * expdiff is now equal to the size
		...
		if0 = i;
		for (; i >= 0; i--)
			bi.bsignificand[i] = bf.bsignificand[i +
				bf.blength - 1 - if0];
		for (i = (bf.blength - 2 - if0); bf.bsignificand[i] == 0; i--);
		/* Find first non-zero. */
		if (i >= 0) {
			/* If non-zero found, stick it. */
			bi.bsignificand[0] |= 1;
		}
		bi.blength += nmove;
...
punpack:
	...
	(void) __big_binary_to_unpacked(ptounpacked, px);
...
int
__inrange_single(single *px, decimal_mode *pm, decimal_record *pd,
...
#define	IDSMIN  ((ncs >= (__TBL_TENS_MAX-36))?(-__TBL_TENS_MAX):-(ncs+36))
	...
	else if ((idsexp <= -1) && (idsexp >= IDSMIN)) {
		if (-idsexp > __TBL_TENS_EXACT)
			esum++;
		if (esum > 0)
	...
	if (esum > 0) {
		...
		ef = (df1 == dds) ? (fp_exception_field_type) 0 :
...
notquickx:
	__set_ieee_flags(&fb);
	/* Result can't be computed by simplified means */
notquick:
	return (0);
}

int
__inrange_double(double *px, decimal_mode *pm, decimal_record *pd,
			fp_exception_field_type *ps)
...

{
	...
	__get_ieee_flags(&fb);
		/* small positive exponent */
	if ((pd->exponent <= __TBL_TENS_EXACT) && (pd->exponent >= 1)) {
		...
		esum += (int)eround;
		/* small negative exponent */
	} else if ((-pd->exponent <= __TBL_TENS_EXACT) &&
			(-pd->exponent >= 1)) {
		dds = __digits_to_double(pd->ds, nds, &eround);
		if (eround != __fbe_none)
			NOTQUICKX('U')
	...
	if (esum > 1)
		NOTQUICKX('W')
	*px = dds;
	ef = (esum == 0) ? (fp_exception_field_type) 0 :
		(fp_exception_field_type) (1 << fp_inexact);
	__set_ieee_flags(&fb);
	*ps = ef;
	return (1);
...
int
__inrange_quadex(double *px, decimal_mode *pm, decimal_record *pd,
		fp_exception_field_type *ps)
...

{
	...
	nds = pd->ndigits;
	/* small positive exponent */
	if ((pd->exponent <= __TBL_TENS_EXACT) && (pd->exponent > 0)) {
		dds = __digits_to_double(pd->ds, nds, &eround);
		if (eround != __fbe_none)
			NOTQUICKX('X')
		if (pd->sign == 1)
	...
	/* small positive exponent */
	/* small negative exponent */
	else if ((pd->exponent <= -1) &&
		(pd->exponent >= -__TBL_TENS_EXACT)) {
		dds = __digits_to_double(pd->ds, nds, &eround);
		if (eround != __fbe_none)
			NOTQUICKX('Y')
		if (pd->sign == 1)
...

{
	...
	/* Initialize to no floating-point exception */
	ef = 0;
	...
	default:
		/* Guaranteed overflow. */
		if (pd->exponent > EXTENDED_MAXE) {
			u.sign = pd->sign == 0 ? 0 : 1;
			u.fpclass = fp_normal;
			u.exponent = 0x000fffff;
			u.significand[0] = 0x80000000;
		/* Guaranteed in range. */
		} else if (pd->exponent >= -EXTENDED_MAXE) {
			goto inrange;
		/* Guaranteed deep underflow. */
		} else if (pd->exponent <=
			(-EXTENDED_MAXE - DECIMAL_STRING_LENGTH)) {
			goto underflow;

		/* Deep underflow possible, depending on string length. */
		} else {
			int		i;
			for (i = 0; (pd->ds[i] != 0) &&
				(i < (-pd->exponent - EXTENDED_MAXE)); i++);
			/* Deep underflow */
			if (i < (-pd->exponent - EXTENDED_MAXE)) {
		underflow:
				u.sign = pd->sign == 0 ? 0 : 1;
				u.fpclass = fp_normal;
				u.exponent = -0x000fffff;
			...
			fp_exception_field_type *ps)
{
	...
	default:
		if (pd->exponent > QUAD_MAXE) {	/* Guaranteed overflow. */
			...
			u.significand[0] = 0x80000000;
			/* Guaranteed in range. */
		} else if (pd->exponent >= -QUAD_MAXE) {
			goto inrange;
		} else if (pd->exponent <=
				(-QUAD_MAXE - DECIMAL_STRING_LENGTH)) {
			/* Guaranteed deep underflow */
			goto underflow;
		/* Deep underflow possible, depending on string length */
		} else {
			for (i = 0; (pd->ds[i] != 0) &&
				(i < (-pd->exponent - QUAD_MAXE)); i++);
				/* Deep underflow */
			if (i < (-pd->exponent - QUAD_MAXE)) {
		underflow:
				u.sign = pd->sign == 0 ? 0 : 1;
				u.fpclass = fp_normal;
				u.exponent = -0x000fffff;
			...
			fp_exception_field_type *ps)
{
	...
	/* Initialize to no floating-point exceptions. */
	ef = 0;
	kluge.f.msw.sign = pd->sign ? 1 : 0;
	switch (pd->fpclass) {
	case fp_zero:
	...
	default:
		if (pd->exponent > SINGLE_MAXE) { /* Guaranteed overflow. */
			...
			u.significand[0] = 0x80000000;
			/* Guaranteed in range. */
		} else if (pd->exponent >= -SINGLE_MAXE) {
			goto inrange;
		/* Guaranteed deep  underflow. */
		} else if (pd->exponent <=
			(-SINGLE_MAXE - DECIMAL_STRING_LENGTH)) {
			goto underflow;
		/* Deep underflow possible, depending on string length. */
		} else {
			int	i;
			...
			for (i = 0; (pd->ds[i] != 0) &&
				(i < (-pd->exponent - SINGLE_MAXE)); i++);
			/* Deep underflow */
			if (i < (-pd->exponent - SINGLE_MAXE)) {
		underflow:
				u.sign = pd->sign == 0 ? 0 : 1;
				u.fpclass = fp_normal;
				u.exponent = -0x000fffff;
			...
			fp_exception_field_type *ps)
{
	...
	default:
		if (pd->exponent > DOUBLE_MAXE) { /* Guaranteed overflow. */
			...
			u.exponent = 0x000fffff;
			u.significand[0] = 0x80000000;
		} else if (pd->exponent >= -DOUBLE_MAXE) {
			/* Guaranteed in range. */
			goto inrange;
		} else if (pd->exponent <=
				(-DOUBLE_MAXE - DECIMAL_STRING_LENGTH)) {
	/* Guaranteed deep underflow */
			goto underflow;
			...
			for (i = 0; (pd->ds[i] != 0) &&
				(i < (-pd->exponent - DOUBLE_MAXE)); i++);
			if (i < (-pd->exponent - DOUBLE_MAXE)) {
			/* Deep underflow */
		underflow:
				u.sign = pd->sign == 0 ? 0 : 1;
				u.fpclass = fp_normal;
