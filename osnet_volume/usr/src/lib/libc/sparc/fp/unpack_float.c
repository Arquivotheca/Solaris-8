...
#pragma ident	"@(#)unpack_float.c	1.5	96/12/04 SMI"
...
__class_extended(extended *x)
{
	...
	if (kluge.f.msw.exponent == 0) {	/* 0 or sub */
		...
		if (((kluge.f.significand & 0x7fffffff) == 0) &&
...
void
__unpack_single(unpacked *pu, single *px)
{
	...
	if (x.f.msw.exponent == 0) {	/* zero or sub */
		...
		if (x.f.msw.significand == 0) {	/* inf */
			...
			pu->significand[0] = 0x40000000 |
						(x.f.msw.significand << 8);
			return;
		}
	}
...
void
__unpack_extended(unpacked *pu, extended *px)
{
	...
	if (x.f.msw.exponent == 0x7fff) {	/* inf or nan */
		if (((x.f.significand & 0x7fffffff) == 0) &&
			(x.f.significand2 == 0)) {	/* inf */
			pu->fpclass = fp_infinity;
			return;
		} else {	/* nan */
	...
	if (x.f.significand < 0x80000000) {	/* zero or unnormal */
		if ((x.f.significand == 0) && (x.f.significand2 == 0)) {
			/* zero */
			pu->fpclass = fp_zero;
			return;
		} else {	/* unnormal */
...
#ifdef DEBUG
__display_unpacked(unpacked *pu)
{
	int		i, e;
