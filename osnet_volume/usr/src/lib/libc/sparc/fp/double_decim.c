...
#pragma ident	"@(#)double_decim.c	1.13	97/08/27 SMI"
...
void
__decimal_round(decimal_mode *pm, decimal_record *pd,
		fp_exception_field_type *ps, char round,
		unsigned sticky)
/*
...
roundup:
	...
	if (i >= 0)
		...
		if (pm->df == floating_form) {
			/* For E format, simply adjust exponent. */
			pd->exponent++;
...
void
__binary_to_decimal_integer(_big_float *pfint, unsigned nsig,
			char ds[], unsigned *nzeros, unsigned *ndigs)

/* Input number of significant digits required */
/* Input number of digits after point required */
/* Output decimal integer string output - must be large enough. */
/* Output number of implicit leading zeros produced. */
/* Output number of explicit digits produced in ds. */
/*
...

{
	...
	if (e <= 0)
		...
		if (pd == BIG_FLOAT_TIMES_TOOBIG) {
			...
			if (pd != &d)
				(void) printf(" large binary exponent %d \
needs heap buffer \n", e);
			printf(" product ");
	...
	__four_digits_quick((short unsigned) pd->bsignificand[pd->blength - 1],
				s);
	...
	for (i = (pd->blength - 2); i >= excessbdigits; i--) {
	/* Convert powers of 10**4 to decimal digits */
		__four_digits_quick((short unsigned) pd->bsignificand[i],
					&(ds[is]));
		is += 4;
	}
...
void
__binary_to_decimal_fraction(_big_float *pfrac, unsigned nsig,
				unsigned nfrac, char ds[],
				int *nzeros, int *ndigs)
...

{
	...
	*nzeros = 0;
	...
	d.bsize = _BIG_FLOAT_SIZE;
	tenpower = nsig + (int)(((17 - pfrac->bexponent -
			16 * pfrac->blength) * (unsigned int)19729) >> 16);
	if (tenpower < nfrac)
		tenpower = nfrac;
	tensig = nfrac;
	if (nsig > tensig)
...
#ifdef DEBUG
	(void) printf(" binary to decimal fraction nsig %d nfrac %d \
tenpower %d tensig %d \n", nsig, nfrac, tenpower, tensig);
	...
	if (pb == BIG_FLOAT_TIMES_TOOBIG) {
		...
		if (pb != pfrac)
			printf(" large decimal exponent %d \
needs heap buffer \n", tenpower);
		printf(" product ");
	...
	if (pb->bexponent <= -16) {
		...
		if (i < excess)
			/* Sticky bit for discarded fraction. */
			pb->bsignificand[excess] |= 1;
	...
	if (pb->bexponent < 0) {
		__right_shift_base_two(pb,
			(short unsigned) -pb->bexponent, &stickyshift);
		if (stickyshift != 0)
	...
	for (i--; i >= 0; i--) {
		/* Convert powers of 10**4 to decimal digits. */
		__four_digits_quick((short unsigned) d.bsignificand[i],
					&(ds[is]));
		is += 4;
	}
finish_ds:
...
void
_unpacked_to_decimal_two(_big_float *pfint, _big_float *pfrac,
			decimal_mode *pm, decimal_record *pd,
			fp_exception_field_type *ps)
{
	int	i, intdigs, fracdigs, fraczeros, fracsigs;
	int	ids = 0, idsbound, lzbound, fracnonzero;
	...
	if ((pm->ndigits >= DECIMAL_STRING_LENGTH) ||
	    ((pm->df == floating_form) && (pm->ndigits < 1))) {
		/* Gross overflow or bad spec. */
overflow:
		*ps |= 1 << fp_overflow;
		return;
	}
	if (pfint->blength > 0) {	/* Compute integer part of result. */
		if (pm->df == floating_form)
			/* Significant digits wanted for E format, */
			/* plus one for rounding. */
			nsig = pm->ndigits + 1;
		else
			/* Significant digits wanted for Format == all. */
			nsig = DECIMAL_STRING_LENGTH - 1;

		__binary_to_decimal_integer(pfint, nsig, is, &intzeros,
						&intsigs);
	} else {
		intsigs = 0;
		intzeros = 0;
	...
	if (((pm->df == fixed_form) && (pm->ndigits >= 0)) ||
	    ((pm->df == floating_form) && ((pm->ndigits + 1) > intdigs))) {
			/* Need to compute fraction part. */
		if (pm->df == floating_form) {
			/* Add two for rounding, sticky. */
			nsig = pm->ndigits + 2 - intdigs;
		...
		__binary_to_decimal_fraction(pfrac, nsig, nfrac,
					fs, &fraczeros, &fracsigs);
		fracdigs = fraczeros + fracsigs;
	}
	/* Combine integer and fraction for E format */
	if (pm->df == floating_form) {
		idsbound = intsigs;
		...
		/* Integer part had enough significant digits. */
		if (ids == pm->ndigits) {
			pd->ndigits = ids;
			...
			if (ids < intdigs) {	/* Gather rounding info. */
				...
				/* Integer part is exact - round from  */
				/* fraction. */
				if (fracnonzero) {
					int		stickystart;
					/* Fraction non-zero. */
					if (fraczeros > 0) {
						/* Round digit is zero. */
						round = '0';
						/* Stickies start with fs[0] */
						stickystart = 0;
					} else {
						/* Round digit is fs[0]. */
						round = fs[0];
						/* Stickies start with fs[1] */
						stickystart = 1;
					}
			...
			/* Need more significant digits from fraction part */
			idsbound = pm->ndigits - ids;
			if (ids == 0) {
				/* No integer part - find first */
				/* significant digit. */
				for (i = 0; fs[i] == '0'; i++);
				idsbound = i + idsbound + fraczeros;
				/* Point i at first significant digit. */
				i += fraczeros;
			} else
				i = 0;
			...
			if (fraczeros < idsbound)
			/* Compute number of leading zeros requires */
				lzbound = fraczeros;
			else
				lzbound = idsbound;
			if (lzbound > i) {
				...
				if (ncopy > 0) {
					(void) memcpy(&(pd->ds[ids]),
						&(fs[i - fraczeros]), ncopy);
					i += ncopy;
					ids += ncopy;
				}
			}
			...
			if (i < fracsigs) {	/* Gather rounding info.  */
				...
				i++;
				if (sticky == 0) {
					/* Find out if remainder is exact. */
					if (i < 0)
						i = 0;
					for (; (fs[i] == '0') &&
						(i < fracsigs); i++);
				...
				/* Fraction part is exact - add zero digits */
				/* if required. */
				int		ncount;
				ncount = pm->ndigits - ids;
				if (ncount > 0) {
					(void) memset(&(pd->ds[ids]), '0',
							ncount);
					ids += ncount;
				}
			}
		...
		if (pm->ndigits >= 0) {	/* Normal F format. */
			...
			if (idsbound > pm->ndigits)
				idsbound = pm->ndigits;
			if (fraczeros < idsbound)
			/* Compute number of leading zeros required. */
				lzbound = fraczeros;
			else
				lzbound = idsbound;
			{
			...
			/*
			 * Don't worry about leading zeros
			 * from now on, we're just rounding info
			 */
			i -= fraczeros;
			if (i < fracsigs) {
				if (i < 0)
			...
			/* Bizarre F format - round to left of point. */
			...
			if (intdigs <= roundpos) {
				...
				/* Some integer digits do not get rounded */
				/* away. */
				{
				...
				{
					int	ncopy =
						intdigs - roundpos - ids;
					if (ncopy > 0) {
						(void) memset(&(pd->ds[ids]),
							'0', ncopy);
						ids += ncopy;
					}
				}
...

{
	...
	for (zeros = 0; p->bsignificand[zeros] == 0; zeros++);
	length -= zeros;
	if (length < 0)
		length = 0;
	if ((zeros > 0) && (length > 0)) {
...
void
_split_double_m1(double_equivalence *px, _big_float *pfint,
		_big_float *pfrac)
...
void
_split_double_3(double_equivalence *px, _big_float *pfint,
		_big_float *pfrac)
...
{
	double_equivalence x = *px;
	unsigned int   fmask, imask;

	/* Mask for fraction bits */
	fmask = (1 << (-pfint->bexponent - 48)) - 1;
	imask = 0xffff & ~fmask;
	pfint->bexponent += 48;
	pfint->blength = 1;
...
void
_split_double_19(double_equivalence *px, _big_float *pfint,
		_big_float *pfrac)
...
void
_split_double_35(double_equivalence *px, _big_float *pfint,
		_big_float *pfrac)
...
void
_split_double_51(double_equivalence *px, _big_float *pfint,
		_big_float *pfrac)
...
void
_split_double_52(double_equivalence *px, _big_float *pfint,
		_big_float *pfrac)
...
void
__unpack_double_two(double *px, _big_float *pfint,
			_big_float *pfrac)
...

{
	...
	if ((pfint->bexponent < 0) && (pfint->blength > 0)) {
		/* right shift */
		__right_shift_base_two(pfint,
			(short unsigned) -pfint->bexponent, &sticky);
...
void
__k_double_to_decimal(double dd, decimal_mode *pm,
	...
	if (pm->df == fixed_form) {	/* F format */
		if ((pm->ndigits <= __TBL_TENS_MAX) && (pm->ndigits > 0)) {
			...
			if (pm->ndigits > __TBL_TENS_EXACT) {
				...
				dds = __mul_set(__dabs(dd),
				__tbl_tens[pm->ndigits], &eround);
				esum += (int)eround;
			}
		}
		/* small positive */
		else if (pm->ndigits != 0) {
		...
		/* exponent * log10(2) */
		idsexp = pm->ndigits - 1 - ((unbiasedexponent * 1233 +
		(int)__tbl_baselg[de.f.msw.significand  >> 13]) >> 12);
		retry = 0;

tryidsexp:
		esum = 0;
		/* Positive power. */
		if ((0 < idsexp) && (idsexp <= __TBL_TENS_MAX)) {
			if (idsexp > __TBL_TENS_EXACT) {
				prod = dds * __tbl_tens[idsexp];
				esum += 2;
			} else {
				prod = __mul_set(dds, __tbl_tens[idsexp],
					&eround);
				esum += (int)eround;
			}
		} else if ((0 < -idsexp) && (-idsexp <= __TBL_TENS_MAX)) {
			/* Negative power to scale */
			if (-idsexp > __TBL_TENS_EXACT) {
				prod = dds / __tbl_tens[-idsexp];
				esum += 2;
			} else {
				prod = __div_set(dds, __tbl_tens[-idsexp],
						&eround);
				esum += (int)eround;
			}
		} else if (idsexp == 0)
...
#ifdef DEBUG
				if (retry > 0)
					(void) printf("idsexp too small \
second try x %X %X nd %d idsexp %d prod %30.1f \n", dds, pm->ndigits,
					idsexp, prod);
				else
					(void) printf("idsexp too small \
first  try x %X %X nd %d idsexp %d prod %30.1f \n", dds, pm->ndigits,
						idsexp, prod);
#endif DEBUG
				if (retry > 0)
					NOTQUICKX('M')
		...
		/* prod too big - wrong power of ten or other failure */
		} else if (prod >= __tbl_tens[pm->ndigits]) {
#ifdef DEBUG
			if (retry > 0)
				(void) printf("idsexp too big second try \
x %X %X nd %d idsexp %d prod %30.1f \n", dds, pm->ndigits, idsexp, prod);
			else
				(void) printf("idsexp too big first  try \
x %X %X nd %d idsexp %d prod %30.1f \n", dds, pm->ndigits, idsexp, prod);
#endif
			if (retry > 0)
				NOTQUICKX('N')
...
#ifdef DEBUG
		if (retry == 0)
			(void) printf("idsexp ok  first try x %X %X nd %d \
idsexp %d prod %30.1f \n", dds, pm->ndigits, idsexp, prod);
#endif
...
void
double_to_decimal(double *px, decimal_mode *pm,
		decimal_record *pd, fp_exception_field_type *ps)
{
...
void
__unpack_quadruple_two(quadruple *px, _big_float *pfint,
			_big_float *pfrac)
/*
 ...
 */
{
	...
	if (exponent <= -1) {	/* exponent <= -1 - all fraction, no integer */
		...
		/*
		 * 0 <= exponent <= 111 : 1.0 <= x < 2**112 :
		 * some integer, some fraction
		 */
		_BIG_FLOAT_DIGIT u[8], fmask;
		/* transition word between int and frac */
		int	i, midword = (111 - exponent) >> 4;
		...
		while (pfrac->blength &&
		    pfrac->bsignificand[pfrac->blength - 1] == 0)
			/* remove leading zeros */
			pfrac->blength--;
...
void
__k_quadruple_to_decimal(quadruple *px, decimal_mode *pm,
		decimal_record *pd, fp_exception_field_type *ps)
{
