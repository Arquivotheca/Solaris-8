...
#pragma ident	"@(#)g_canon_name.c	1.13	97/11/12 SMI"
	...
	if (!output_name) {
		if (in_union->mech_type) {
			__gss_release_internal_name(minor_status,
							in_union->mech_type,
							&in_union->mech_name);
			gss_release_oid(minor_status, &in_union->mech_type);
...
allocation_failure:
	...
	if (output_name) {
		...
		if (out_union->name_type)
			gss_release_oid(minor_status, &out_union->name_type);
	...
	if (dest_union->mech_name) {
		__gss_release_internal_name(minor_status, dest_union->mech_type,
						&dest_union->mech_name);
	}

	if (dest_union->mech_type)
		gss_release_oid(minor_status, &dest_union->mech_type);
