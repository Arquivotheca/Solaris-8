...
#pragma ident	"@(#)mema.c	1.19	99/08/31 SMI"
...
info_set(ac_stat_t *asp, mema_bank_t *bkp, cfga_info_t info)
{
	...
	*info = NULL;
	...
	info += sprintf(info, "slot%d", board);

	if (asp->real_size == 0) {
		info += sprintf(info, " empty");
		return;
	}
	...
	if (asp->rstate == SYSC_CFGA_RSTATE_CONNECTED &&
		...
		if ((n = asp->use_size) >= 1024) {
			...
			f = "Mb";
		info += sprintf(info, " (%d%s used)", n, f);
	}
	...
	info += sprintf(info, " base 0x%llx", GRP_REALBASE(decode));
	...
	if (intlv != 1)
		info += sprintf(info, " interleaved %u-way", intlv);

	if (prom_read_disabled_list(&disab, board)) {
		if (disab != 0) {
			info += sprintf(info, " %s", "disabled at boot");
		}
...
cfga_ap_id_cmp(const cfga_ap_log_id_t ap_id1, const cfga_ap_log_id_t ap_id2)
{
	...
	if ((n = ap_drv_idx(ap_id1) - ap_drv_idx(ap_id2)) != 0)
		return (n);

	return (ap_bk_idx(ap_id1) - ap_bk_idx(ap_id2));
}
	...
	void *arg)
{
	...
	case AC_MEM_TEST_READ: {
		...
		(void) fprintf(debug_fp, "IOCTL: AC_MEM_TEST_READ handle=%#x "
		    ...
		    (u_int)tread->handle, (u_int)tread->page_buf,
	...
	case AC_MEM_TEST_WRITE: {
		...
		(void) fprintf(debug_fp, "IOCTL: AC_MEM_TEST_WRITE handle=%#x "
		    "buf=%#x page=%#llx off=%#x count=%#x\n",
		    (u_int)twrite->handle, (u_int)twrite->page_buf,
	...
	int ret_errno)
{
	...
	case AC_MEM_TEST_READ: {
		...
		if (ret_errno == EIO) {
			(void) fprintf(debug_fp, "module_id=%#llx afsr=%#llx "
			    ...
			    "udbl_error_reg=%#llx\n",
			    err->module_id, err->afsr, err->afar,
			    err->udbh_error_reg, err->udbl_error_reg);
		} else {
			(void) fprintf(debug_fp, "\n");
		}
	}
	...
	case AC_MEM_STAT: {
		...
		(void) fprintf(debug_fp, "    rstate=%u ostate=%u "
		    ...
		    (u_int)tstat->rstate, (u_int)tstat->ostate,
		    (u_int)tstat->condition, (u_long)tstat->status_time,
		    tstat->board);
		...
		(void) fprintf(debug_fp, "    page_size=%#x "
		    ...
		    tstat->page_size, tstat->phys_pages,
		    tstat->managed, tstat->nonrelocatable);
		(void) fprintf(debug_fp, "    memctl=%#llx "
		    "decode0=%#llx decode1=%#llx\n",
		    tstat->ac_memctl, tstat->ac_decode0, tstat->ac_decode1);
	}
		break;
	case AC_MEM_EXERCISE: {
		ac_cfga_cmd_t *cmdp;
