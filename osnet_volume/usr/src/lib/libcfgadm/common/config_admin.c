...
#pragma ident   "@(#)config_admin.c 1.16     99/06/21 SMI"
...
typedef struct plugin_lib {
	...
	char	ap_phys[CFGA_PHYS_EXT_LEN];	/* physical ap */
	char	ap_logid[CFGA_LOG_EXT_LEN];	/* logical ap */
 ...
 */
static rwlock_t plugin_list_lock;
...
static int find_link(const char *path, const struct stat *sbufp, int type,
    struct FTW *ftwp);

static cfga_err_t check_flags(cfga_flags_t flags);
	...
	char **errstring)
{
	...
	config_init();
	/*
	 * Check each ap_id: V1 entry points don't support dynamic
	 * attachment points
	 */
...
config_unload_libs()
{
	plugin_lib_t *libp = NULL, *libp_last = NULL;
	...
	const cfga_ap_log_id_t ap2)
{
	...
	libloc1.libp = NULL;
	if (config_get_lib(ap1, &libloc1, NULL) != CFGA_OK) {
		return (strncmp(ap1, ap2, CFGA_PHYS_EXT_LEN));
	}

	libloc2.libp = NULL;
	if (config_get_lib(ap2, &libloc2, NULL) != CFGA_OK) {
		return (strncmp(ap1, ap2, CFGA_PHYS_EXT_LEN));
	}

	if (strncmp(libloc1.pathname, libloc2.pathname, MAXPATHLEN) != 0) {
		return (strncmp(ap1, ap2, CFGA_PHYS_EXT_LEN));
	}

	return ((*libloc1.libp->cfga_ap_id_cmp_p)(ap1, ap2));
}
 ...
 * (a nop if not the first call)
 */

static void config_init()
{
	if (config_admin_inited == 0) {
		(void) rwlock_init(&plugin_list_lock, NULL, NULL);
...
static const char *
__config_strerror(int cfgerrnum)
{
	const char *ep = NULL;
	...
	lib_loc_t *liblocp)
{
	...
	assert(len <=  sizeof (libp->ap_logid));

	if (libp->plugin_vers != CFGA_HSL_V1) {
		return (CFGA_LIB_ERROR);
	}

	rc = mklog_common(node, minor, libp, len);
	if (rc == CFGA_OK) {
		(void) snprintf(liblocp->ap_logical, len, "%s", libp->ap_logid);
	}
	...
	lib_loc_t *liblocp)
{
	...
	/*
	 * look up CFGA_DEV_DIR for a symlink first.
	 * On any error, fallback to driver and instance based logical ap_ids.
	 */
	(void) mutex_lock(&config_nftw_arg.mp);
	config_nftw_arg.physpath = liblocp->ap_physical;
	config_nftw_arg.buf = buf;
	config_nftw_arg.buflen = sizeof (buf);

	/* find_link requires a buffer atleast PATH_MAX in length */
	assert(config_nftw_arg.buflen >= PATH_MAX);

	errno = 0;
	rv = nftw(CFGA_DEV_DIR, find_link, NFTW_DEPTH, FTW_PHYS);
	(void) mutex_unlock(&config_nftw_arg.mp);

	if (rv == NFTW_TERMINATE && strlen(buf) < len) {
		/* A symlink was found */
		(void) snprintf(libp->ap_logid, len, "%s", buf);
		rc = CFGA_OK;
	} else {
		rc = mklog_common(node, minor, libp, len);
	}
...
/*
 * Searches for links in CFGA_DEV_DIR
 * Since this routine is called with the nftw mutex lock held,
 * use care if acquiring other locks in this routine.
 */
/*ARGSUSED*/
static int
find_link(
	const char *path,
	const struct stat *sbufp,
	int type,
	struct FTW *ftwp)
{
	/* We want only symlinks */
	if (type != FTW_SL) {
		return (NFTW_CONTINUE);
	}

	/* realpath requires a buffer of at least PATH_MAX */
	assert(config_nftw_arg.buflen >= PATH_MAX);

	config_nftw_arg.buf[config_nftw_arg.buflen - 1] = '\0';
	if (realpath(path, config_nftw_arg.buf) == NULL) {
		return (NFTW_CONTINUE);
	}

	if (strcmp(config_nftw_arg.physpath, config_nftw_arg.buf) == 0) {
		(void) snprintf(config_nftw_arg.buf, config_nftw_arg.buflen,
		    "%s", &path[ftwp->base]);
		errno = 0;	/* terminating prematurely, but not an error */
	...
	size_t len)
{
	...
	if (drv != NULL && inst != -1 && minor_name != NULL &&
	    snprintf(libp->ap_logid, len, "%s%d:%s", drv, inst, minor_name)
	    < len) { /* snprintf return value does not include term. NULL */
		return (CFGA_OK);
	}
	...
	lib_loc_t *libloc_p)
{
	...
	if (list_libp != NULL) {
		...
		(void) rwlock_init(&libp->entry_lock, NULL, NULL);
		(void) rw_rdlock(&libp->entry_lock);
		/*
		 * get to end of list
		 */
		...
		(void) snprintf(libp->ap_phys, MAXPATHLEN, "/devices%s:%s",
		    devfs_path, di_minor_name(minor));
	 ...
	 * struct
	 */
	(void) snprintf(libloc_p->ap_physical, MAXPATHLEN, "/devices%s:%s",
	    devfs_path, di_minor_name(minor));
	...
	char **errstring)
{
	...
	di_walk_minor(rnode, "ddi_ctl:attachment_point",
	...
	void *arg)
{
	...
	(void) snprintf(phys_name, sizeof (phys_name), "/devices%s:%s",
...
list_common(list_stat_t *lstatp, const char *class)
{
	...
	di_walk_minor(rnode, nodetype, DI_CHECK_ALIAS|DI_CHECK_INTERNAL_PATH,
	    lstatp, do_list_common);
	di_fini(rnode);
	...
	list_stat_t *lstatp)
{
	...
	for (i = 0; i < num_ap_ids; i++) {
		...
		/*
		 * update the plugin library structure
		 */
		(void) strcpy(libp->ap_phys, libloc.ap_physical);
		 ...
		 */
		if ((rc = libp->vers_ops->stat_plugin(lstatp, &libloc,
		    lstatp->errstr)) != CFGA_OK) {
			break;
		}
	}
...
stat_plugin_v1(list_stat_t *lstatp, lib_loc_t *libloc_p, char **errstring)
{
	 ...
	 * update the plugin library structure
	 */
	(void) strcpy(libloc_p->libp->ap_phys, libloc_p->ap_physical);
...
check_flags(cfga_flags_t flags)
{
	...
	/* For now only 1 flag is defined: CFGA_FLAG_LIST_ALL */
	if (flags == 0 || flags ==  CFGA_FLAG_LIST_ALL) {
		return (CFGA_OK);
	} else {
		return (CFGA_ERROR);
	}
