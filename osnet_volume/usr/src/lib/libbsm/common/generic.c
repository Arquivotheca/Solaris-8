#ifndef lint
static char	sccsid[] = "@(#)generic.c 1.16 99/10/14 SMI";
#endif
...
int
aug_get_machine(char *hostname, uint32_t *buf, uint32_t *type)
{
	int	rv;
	struct hostent *hostent;
	int err;

	hostent = getipnodebyname((const void *)hostname, AF_INET6, 0, &err);
	if (hostent == 0) {
		hostent = getipnodebyname((const void *)hostname, AF_INET, 0, &err);
		if (hostent == 0) {
	...
	if (hostent->h_addrtype == AF_INET6) {
		(void) memcpy(buf, hostent->h_addr_list[0], 16);
		*type = AU_IPv6;
	} else {
		/* address is IPv4 (32 bits) */
		(void) memcpy(buf, hostent->h_addr_list[0], 4);
		*type = AU_IPv4;
	}
...
aug_save_tid_ex(dev_t port, uint32_t *machine, uint32_t type)
{
	...
	aug_tid.at_port = port;
	aug_tid.at_type = type;			/* XXX check for bogus type? */
	for (i=0; i<(type/4);i++)
...
aug_audit(void)
{
	...
	au_write(ad, au_to_subject_ex(aug_auid, aug_euid, aug_egid,
	...
	if (aug_text1 != NULL) {
		au_write(ad, au_to_text(aug_text1));
	}
	if (aug_text2 != NULL) {
		au_write(ad, au_to_text(aug_text2));
	}
	if (aug_path != NULL) {
...
#ifdef _LP64
	au_write(ad, au_to_return64((aug_sorf == 0) ? 0 : -1,
						(int64_t)aug_sorf));
#else
	au_write(ad, au_to_return32((aug_sorf == 0) ? 0 : -1,
						(int32_t)aug_sorf));
#endif
