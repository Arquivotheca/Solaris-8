...
#pragma ident	"@(#)getuserattr.c	1.1	99/06/08 SMI"
...
getuserattr()
{
	...
	memset(&user, NULL, sizeof (userattr_t));
	...
	nsc_data_t *sptr)
{
...
#ifdef	_LP64
	memcpy(buffer, (sptr->nsc_ret.nsc_u.buff + sizeof (userstr32_t)),
	...
	*result = sptr->nsc_ret.nsc_u.user;
	memcpy(buffer, (sptr->nsc_ret.nsc_u.buff + sizeof (userstr_t)),
