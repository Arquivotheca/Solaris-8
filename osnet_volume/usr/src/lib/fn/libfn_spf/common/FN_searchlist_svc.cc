...
#pragma ident	"@(#)FN_searchlist_svc.cc	1.1	96/03/31 SMI"
...
FN_searchlist_svc::FN_searchlist_svc(FN_searchset *ss, unsigned int es)
{
	search_hits = ss;
...
FN_searchlist_svc::~FN_searchlist_svc()
{
	if (search_hits)
		delete search_hits;
}
	...
	FN_status &status)
{
	...
	if (answer == 0) {
		...
		ret_answer = new FN_string(*answer);
	}
