...
#ifndef _FNSP_NISORGCONTEXT_DEFAULT_HH
#define	_FNSP_NISORGCONTEXT_DEFAULT_HH

#pragma ident	"@(#)FNSP_nisOrgContext_default.hh	1.2	96/06/19 SMI"

#include <FNSP_defaultContext.hh>

/* For context type OrganizationName */
class FNSP_nisOrgContext_default : public FNSP_defaultContext {
    protected:
	// internal functions
	FNSP_nisOrgContext_default(const FN_ref_addr&, const FN_ref&);
	const FN_ref* resolve(const FN_string&, FN_status_csvc&);

	virtual FN_ref *make_nsid_ref();
	virtual FN_ref *make_service_ref();

    public:
	~FNSP_nisOrgContext_default();

#ifdef DEBUG
	// only used for testing
	FNSP_nisOrgContext_default(const FN_identifier *addr_type,
	    const FN_string&);
	FNSP_nisOrgContext_default(const FN_ref&);
#endif /* DEBUG */

	static FNSP_nisOrgContext_default* from_address(
...
#endif /* _FNSP_NISORGCONTEXT_DEFAULT_HH */
