...
#ifndef _FNSP_DEFAULTCONTEXT_HH
#define	_FNSP_DEFAULTCONTEXT_HH

#pragma ident	"@(#)FNSP_defaultContext.hh	1.1	96/03/31 SMI"

#include <xfn/fn_spi.hh>

class FNSP_defaultContext : public FN_ctx_csvc_strong {
    protected:
	FN_ref *my_reference;
    public:
	~FNSP_defaultContext();
	static FNSP_defaultContext* from_address(
	...
	virtual int c_destroy_subcontext_nns(const FN_string& name,
	    FN_status_csvc&);
	virtual FN_attrset* c_get_syntax_attrs_nns(const FN_string&,
	    FN_status_csvc&);
	...
	virtual FN_valuelist *c_attr_get_values_nns(const FN_string&,
	    const FN_identifier&, unsigned int, FN_status_csvc&);
	virtual FN_attrset *c_attr_get_ids_nns(const FN_string&, unsigned int,
	    FN_status_csvc&);
