/*
 * ident	"@(#)SLPConfig.java	1.2	99/05/10 SMI"
 *
...
class SLPConfig {
	...
	public synchronized void load(InputStream in) throws IOException {
		...
		// Parse out equals sign, if any. Note that we do not trim any
		//  white space because the grammer doesn't allow it.
	...
	for (i = 0; i < n; i++) {
	    ...
	    if (isSA() || isDA()) {
		Assert.assert(saConfiguredScopes.contains(scope),
			      "sa_new_scope",
	...
	return parseTimeouts("net.slp.multicastTimeouts",
	...
	return parseTimeouts("net.slp.datagramTimeouts",
	...
	try {
	    ...
	    writeLog("syntax_error_prop",
		     new Object[] {"net.slp.multicastTimeouts", sTimeouts});
	    return defaults;
	...
	for (Enumeration en = timeouts.elements(); en.hasMoreElements(); ) {
		...
		writeLog("syntax_error_prop",
			 new Object[] {
		    "net.slp.multicastTimeouts", sTimeouts});
		return defaults;
