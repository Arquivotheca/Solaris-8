/*
 * Copyright (c) 1984-1991,1998-1999 by Sun Microsystems, Inc.
 * All rights reserved.
 */

#pragma ident	"@(#)xdr_rec.c	1.32	99/07/19 SMI"
...
xdrrec_endofrecord(XDR *xdrs, bool_t sendnow)
{
	...
	if (sendnow || rstrm->frag_sent ||
		((uint32_t)rstrm->out_finger + (uint_t)sizeof (uint32_t) >=
...
fill_input_buf(RECSTREAM *rstrm, bool_t do_align)
{
	...
	if (do_align) {
		...
		uint_t i = (uint_t)rstrm->in_boundry % BYTES_PER_XDR_UNIT;
...
recsz_invalid:
		 ...
		 * header. Otherwise, leave in_finger pointing to the
		 * start of the current fragment data.
		 */
	...
	if (rstrm->last_frag && rstrm->fbtbc == 0) {
		*pstat = XPRT_MOREREQS;
		...
		rstrm->fbtbc = (int)(rstrm->in_boundry - rstrm->in_finger);
		if (rstrm->in_nextrecsz == 0)
			rstrm->in_nextrec = rstrm->in_base;
		trace1(TR__xdrrec_getbytes_nonblock, 1);
		return (TRUE);
...
static uint_t
fix_buf_size(register uint_t s)
{
	uint_t dummy1;
