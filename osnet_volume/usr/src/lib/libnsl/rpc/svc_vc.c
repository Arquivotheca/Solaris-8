...
#pragma ident	"@(#)svc_vc.c	1.56	99/09/20 SMI"
...
extern int	t_getname(int, struct netbuf *, int);
    ...
    struct cf_rendezvous *r)
{
	...
	if (t_bind(destfd, (struct t_bind *)NULL, r->t_bind) == -1) {
		...
		if (t_errno == TSYSERR) {
			...
			(void) sprintf(buf, "Unknown error num %d", errno);
...
/* LINTED pointer alignment */
	 ...
	 */
		(void) t_close(destfd);
		syslog(LOG_ERR, errstring, do_accept_str, no_mem_str);
		trace1(TR_do_accept, 1);
		_exit(1);
	}
 ...
 */
static int
t_rcvnonblock(SVCXPRT *xprt, caddr_t buf, register int len)
{
	register int fd = xprt->xp_fd;
 ...
 * If there has been no activity on the fd for WAIT_PER_TRY
 * milliseconds, timeout the fd  by destroying its xprt.
