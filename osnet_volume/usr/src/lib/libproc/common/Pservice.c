...
#pragma ident	"@(#)Pservice.c	1.4	99/05/04 SMI"
 ...
 * and librtld_db.  As most of these facilities are already provided
 * by (more elegant) interfaces in <libproc.h>, we can just call those.
 *
 ...
 * causes libthread_db to create an "agent thread" in the target process.
 ...
 * return PS_OK for core files in order to allow libthread_db
 * to operate on these as well.
 */
ps_err_e
...
ps_lgetLDT(struct ps_prochandle *P, lwpid_t lwpid, struct ssd *ldt)
{
	...
	if (P->state == PS_DEAD)
		return (PS_ERR); /* No ldt from core files yet */
