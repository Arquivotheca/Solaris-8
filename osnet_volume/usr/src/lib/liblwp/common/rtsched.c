...
#pragma ident	"@(#)rtsched.c	1.2	99/11/02 SMI"
...
_init_rt_prio_ranges()
{
	...
	if ((hdl = _dlopen("librt.so", RTLD_LAZY)) == 0) {
		panic("_init_rt_prio_ranges: dlopen librt failed");
	}
	if ((fname = (int (*)(int))_dlsym(hdl, "sched_get_priority_min"))
		...
		panic("_init_rt_prio_ranges: _dlsym for symbols in librt");
	}
...
{
	(void) _pthread_once(&validateonce, _init_rt_prio_ranges);
	switch (policy) {
	case SCHED_FIFO:
