...
#pragma ident	"@(#)nis_ib_proc.c	1.53	99/05/19 SMI"
	...
	if (pid) {
		...
		if (res->cookie.n_bytes == NULL) {
			...
			add_cleanup((void (*)())XFREE,
				(char *)res->cookie.n_bytes, "callback anonid");
			res->cookie.n_len = sizeof (anonid);
			anonid = nis_add_callback_pid(pid, pname);
			memcpy(res->cookie.n_bytes, &anonid,
							sizeof (anonid));
			if (verbose)
				syslog(LOG_INFO,
		"Parent process returning, callback PID = %ld, anonid = %ld",
				pid, *((anonid_t *)res->cookie.n_bytes));
		}
		return (res);
	}
	...
	nis_result		*res;	/* Place for results.	    */
{
	...
	obj->zo_oid.ctime = (u_long)currtime;
	...
	nis_result		*res;	/* Place for results.	    */
{
	...
	mobj = *(db->objs->o);
	currtime = time(0);	/* Make sure REMOVE/ADD has same time */
	mobj.zo_oid.mtime = (u_long)currtime;
