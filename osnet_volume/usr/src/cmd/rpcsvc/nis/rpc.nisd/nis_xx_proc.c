...
#pragma ident	"@(#)nis_xx_proc.c	1.56	99/06/14 SMI"
	...
	struct svc_req *reqstp;
{
	...
	pid_t		pid;
	anonid_t	anonid;
	char		pname[1024], pid_pname[1024];
	...
	pid_pname[0] = '\0';
	pid = nis_get_callback_pid(anonid, pid_pname, sizeof(pid_pname));

	if (verbose)
		syslog(LOG_INFO,
	"CALLBACK_SVC: PID = %ld, anon id = %ld, pname = %s, pid_pname = %s",
			pid, anonid, pname, pid_pname);

	/* Verify caller principal same as pid principal */
	if (secure_level >= 2 && strcmp(pname, pid_pname) != 0 &&
	    strcmp("nobody", pid_pname) != 0) {
		res = FALSE;
		return (&res);
	}
	...
	struct svc_req 	*reqstp;
{
	...
	syslog(LOG_INFO, "nis_dump_svc: good dump of %s, %d total objects "
	    "(%d deferred).", argp->da_dir, totlobjs, skipobjs);
	if (verbose)
	...
	struct svc_req	*rqstp;
{
	...
	if (!nis_op_access("NIS_RMDIR", 0, *argp, 0, rqstp)) {
		if (verbose)
			syslog(LOG_INFO,
