...
#pragma ident	"@(#)dconf.c	1.2	98/06/15 SMI"
...
static const char DC_STR_KERNEL[] = "kernel";	/* Dump contains kernel pages */
static const char DC_STR_ALL[] = "all";		/* Dump contains all pages */
...
dconf_print(dumpconf_t *dcp, FILE *fp)
{
	...
	(void) fprintf(fp, gettext("      Dump content: %s pages\n"),
	    (dcp->dc_cflags & DUMP_ALL) ? gettext("all") : gettext("kernel"));
...
{
	return (fprintf(fp, "%s\n", (dcp->dc_cflags & DUMP_ALL) ?
	    DC_STR_ALL : DC_STR_KERNEL));
}
