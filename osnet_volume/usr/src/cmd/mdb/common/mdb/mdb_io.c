...
#pragma ident	"@(#)mdb_io.c	1.2	99/11/19 SMI"
 ...
 * connected or disconnected from an mdb_iob_t, and an IOP_ATTRSTR entry point
 * for returning terminal attribute information.
 *
 ...
 *
 * Additionally, the formatting code provides an auto-wrap facility that
 * is necessary for compatibility with adb macro formatting.  In auto-wrap
 * mode, the formatting code examines each new chunk of output to determine
...
#define	VA_ARG(alist, vat, type) ((vat == VAT_VARARGS) ? \
	va_arg(alist, type) : (type)vec_arg((const mdb_arg_t **)&alist))
...
static size_t io_promptlen = sizeof (io_prompt) - 1;
static size_t io_peraselen = sizeof (io_perase) - 1;
static size_t io_pbcksplen = sizeof (io_pbcksp) - 1;
...
iob_pager(mdb_iob_t *iob)
{
	...
	sigcb = mdb_signal_gethandler(SIGWINCH, &cbdata);

	if (setjmp(env) != 0) {
		(void) iob_write(iob, iob->iob_pgp, "\r", sizeof (char));
		(void) mdb_signal_unblock(SIGWINCH);
		winch = TRUE;
	}
	...
	(void) mdb_signal_sethandler(SIGWINCH, sigcb, cbdata);

	if (winch)
		(void) mdb_signal_raise(SIGWINCH);
...
mdb_iob_flush(mdb_iob_t *iob)
{
	...
	if (iob->iob_flags & MDB_IOB_WRONLY) {
		...
		if (pgerr == 0) {
			(void) iob_write(iob, iob->iob_iop, iob->iob_buf,
			    iob->iob_nbytes);
			iob->iob_nlines++;
		}
	}
...
static const char *
iob_int2str(va_list *ap, vatype_t vat, intsize_t size, int base,
    uint_t flags, int *zero, u_longlong_t *value)
{
	uintmax_t i;
	...
	case SZ_LONGLONG:
		if (flags & NTOS_UNSIGNED)
			i = (u_longlong_t)VA_ARG(*ap, vat, u_longlong_t);
		else
			i = (longlong_t)VA_ARG(*ap, vat, longlong_t);
		break;

	case SZ_LONG:
		if (flags & NTOS_UNSIGNED)
			i = (ulong_t)VA_ARG(*ap, vat, ulong_t);
		else
			i = (long)VA_ARG(*ap, vat, long);
		break;

	case SZ_SHORT:
		if (flags & NTOS_UNSIGNED)
			i = (ushort_t)VA_ARG(*ap, vat, uint_t);
		else
			i = (short)VA_ARG(*ap, vat, int);
		break;

	default:
		if (flags & NTOS_UNSIGNED)
			i = (uint_t)VA_ARG(*ap, vat, uint_t);
		else
			i = (int)VA_ARG(*ap, vat, int);
	}
...
{
	/*
	 * Our internal timetos() interface returns a string of the canonical
	 * ctime(3C) form "Fri Sep 13 00:00:00 1986\n\0".  We turn this into
	 * the canonical adb /y format "1986 Sep 13 00:00:00" below.
	 */
	const char *src = timetos(tmp);
	static char buf[32];
	char *dst = buf;
	int i;
...
iob_addr2str(uintptr_t addr)
{
	...
	if (mdb_tgt_lookup_by_addr(mdb.m_target, addr,
	    MDB_TGT_SYM_FUZZY, buf, sizeof (buf), &sym) == -1)
		return (NULL);
	 ...
	 * query for us with a different address.  To let us know what happened,
	 * the proc target sets the st_other field of the return symbol to non-
	 * zero to indicate we should not add an offset suffix.
	 */
	if (sym.st_value != addr && sym.st_other == 0) {
		if (sym.st_value > addr)
			offset = -(longlong_t)(sym.st_value - addr);
		else
			offset = (longlong_t)(addr - sym.st_value);

		(void) strcat(buf, numtostr(offset, mdb.m_radix,
...
static const char *
iob_att2str(mdb_iob_t *iob, const char *s, size_t nbytes)
{
	static char buf[32];
	uint_t attr;
	int req;
	...
	if (IOP_ATTRSTR(iob->iob_pgp, req, attr, buf) > 0)
		return ((const char *)buf);
...
iob_bits2str(mdb_iob_t *iob, u_longlong_t value, const mdb_bitmask_t *bmp,
    const char *str, mdb_bool_t altflag)
{
	int delim = 0;
	size_t width;
	...
	for (; bmp->bm_name != NULL; bmp++) {
		if ((value & bmp->bm_mask) == bmp->bm_bits) {
			...
			if ((iob->iob_flags & MDB_IOB_AUTOWRAP) &&
			    (width + iob->iob_nbytes > iob->iob_cols)) {
				mdb_iob_nl(iob);
				iob_indent(iob);
			}
...
out:
	if (delim == 0 && altflag == TRUE) {
		if ((iob->iob_flags & MDB_IOB_AUTOWRAP) &&
		    (strlen(str) + iob->iob_nbytes > iob->iob_cols)) {
			mdb_iob_nl(iob);
			iob_indent(iob);
		}
...
{
	static char buf[16]; /* Enough for "255.255.255.255\0" */

	(void) mdb_iob_snprintf(buf, sizeof (buf), "%u.%u.%u.%u",
 ...
 */
static void
iob_doprnt(mdb_iob_t *iob, const char *format, va_list alist, vatype_t vat)
{
	...
	while ((p = strchr(format, '%')) != NULL) {
		/*
		 * Output the format string directly up to the current special
		 * character (either %, tab, or newline)
		 */
		if (p != format) {
			len = p - format;
		...
		case 'a':
			...
			u.str = iob_int2str(&alist, vat, size, 16,
		...
		case 'A':
			...
			(void) iob_int2str(&alist, vat, size, 16,
		...
		case 'b':
			...
			iob_bits2str(iob, val, VA_ARG(alist, vat, void *),
			    u.str, f_alt);
		...
		case 'c':
			c[0] = (char)VA_ARG(alist, vat, int);
			u.str = c;
			break;

		case 'd':
		case 'i':
			u.str = iob_int2str(&alist, vat, size, 10,
			    flags, &zero, &val);
			break;

		case 'e':
		case 'E':
			u.d = VA_ARG(alist, vat, double);
		...
		case 'G':
			if (size >= SZ_LONG) {
				u.ld = VA_ARG(alist, vat, long double);
		...
		case 'I':
			u.ui32 = VA_ARG(alist, vat, uint32_t);
		...
		case 'o':
			u.str = iob_int2str(&alist, vat, size, 8,
			    NTOS_UNSIGNED, &zero, &val);
		...
		case 'p':
			u.ptr = VA_ARG(alist, vat, void *);
			u.str = numtostr((uintmax_t)u.ptr, 16, NTOS_UNSIGNED);
			break;
		...
		case 'r':
			if (f_alt)
				flags |= NTOS_SHOWBASE;
			u.str = iob_int2str(&alist, vat, size, mdb.m_radix,
		...
		case 's':
			u.str = VA_ARG(alist, vat, const char *);
			if (u.str == NULL)
		...
		case '*':
			width = (size_t)(uint_t)VA_ARG(alist, vat, int);
			goto fmt_switch;
...
{
	iob_doprnt(iob, format, alist, VAT_VARARGS);
}

void
mdb_iob_aprintf(mdb_iob_t *iob, const char *format, const mdb_arg_t *argv)
{
	iob_doprnt(iob, format, (va_list)argv, VAT_ARGVEC);
}
...
mdb_iob_nputs(mdb_iob_t *iob, const char *s, size_t nbytes)
{
	 ...
	 * recursively for each string component, followed by a call to
	 * either our newline or tab routine.  This assures that strings with
	 * these characters obey our wrapping and indenting rules.
	 */
	while ((p = strnpbrk(q, "\t\n", nleft)) != NULL) {
		if (p > q)
		...
		q = p + 1;			/* Advance s past delimiter */
	}

	if (q >= s + nbytes)
		return;	/* Nothing left; string ended with a tab or newline */

	iob_indent(iob);			/* Indent current line */
	n = iob->iob_cols - iob->iob_nbytes;	/* Bytes free on this line */

	ASSERT(iob->iob_cols != 0);
	ASSERT(n != 0);

	/*
	 * While the remaining string is longer than what we can fit on the
	 * current line, copy it into iob_buf, flush the line, indent & repeat.
	 */
	for (p = q; nleft >= n; n = iob->iob_cols - iob->iob_nbytes) {
		(void) strncpy(iob->iob_bufp, p, n);
		nleft -= n;
		p += n;
	...
	/*
	 * If there's any string left, we know it fits on the current line and
	 * has no newlines or tabs, so just copy it into the buffer and return.
	 */
	if (nleft != 0) {
		iob_indent(iob);
		(void) strncpy(iob->iob_bufp, p, nleft);
		iob->iob_bufp += nleft;
		iob->iob_nbytes += nleft;
	}
}
...
mdb_iob_fill(mdb_iob_t *iob, int c, size_t nfill)
{
	...
	if ((iob->iob_flags & MDB_IOB_AUTOWRAP) &&
	    (iob->iob_nbytes + nfill >= iob->iob_cols))
		mdb_iob_nl(iob);

	iob_indent(iob);

	while (nfill != 0) {
		size_t i, n = MIN(iob->iob_bufsiz - iob->iob_nbytes, nfill);
...
{
	if (iob->iob_flags & (MDB_IOB_RDONLY | MDB_IOB_ERR))
		return (0); /* can't write if read-only or error bit set */

	mdb_iob_flush(iob);
	return (iob_write(iob, iob->iob_iop, buf, n));
}
...
ssize_t
no_io_attrstr(mdb_io_t *io, int req, uint_t attrs, char *buf)
{
	if (io->io_next != NULL)
