...
#pragma ident	"@(#)mdb_nv.c	1.1	99/08/11 SMI"
...
typedef struct nv_walk_data {
	mdb_nv_t *nvw_nv;		/* Backpointer to hash */
	mdb_var_t *nvw_iter_elt;	/* Next element to return */
	size_t nvw_iter_bucket;		/* Current hash bucket index */
} nv_walk_data_t;
...
nv_var_alloc(const char *name, const mdb_nv_disc_t *disc,
	uintmax_t value, uint_t flags, mdb_var_t *next)
{
	size_t nbytes = (flags & MDB_NV_EXTNAME) ? sizeof (mdb_var_t) :
	    (sizeof (mdb_var_t) + MDB_NV_NAMELEN - 1);

	mdb_var_t *v = mdb_alloc(nbytes, UM_SLEEP);
...
void
mdb_nv_create(mdb_nv_t *nv)
{
	nv->nv_hash = mdb_zalloc(sizeof (mdb_var_t *) * NV_HASHSZ, UM_SLEEP);
    ...
    uintmax_t value, uint_t flags)
{
	 ...
	 */
	v = nv_var_alloc(name, disc, value, flags, nv->nv_hash[i]);
...
static void
nv_var_defn_remove(mdb_var_t *v, mdb_var_t *corpse)
{
	mdb_var_t *w = v;
...
{
	if (v->v_disc)
		return ((void *)v->v_disc->disc_get(v));
