...
#pragma ident	"@(#)mdb_pipeio.c	1.1	99/08/11 SMI"
...
pio_read(mdb_io_t *io, void *buf, size_t nbytes)
{
	...
	for (nleft = nbytes; nleft == nbytes; nleft -= n) {
		...
		if (pd->pipe_rdndx >= pd->pipe_wrndx) {
			size_t n1 = MIN(BUFSIZ - pd->pipe_rdndx, n);
			size_t n2 = MIN(pd->pipe_wrndx, n - n1);
			...
			ASSERT(pd->pipe_wrndx - pd->pipe_rdndx >= n);
...
pio_write(mdb_io_t *io, const void *buf, size_t nbytes)
{
	...
	for (nleft = nbytes; nleft != 0; nleft -= n) {
		...
		if (pd->pipe_wrndx >= pd->pipe_rdndx) {
			size_t n1 = MIN(BUFSIZ - pd->pipe_wrndx, n);
			size_t n2 = MIN(pd->pipe_rdndx, n - n1);
			...
			ASSERT(pd->pipe_rdndx - pd->pipe_wrndx >= n);
