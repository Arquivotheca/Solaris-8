...
#ident	"@(#)deroff.c	1.6	92/07/14 SMI"	/* SVr4.0 1.9	*/
...
#define TEXT_DOMAIN "SYS_TEST"
	...
	if (errflg)
		fatal(gettext("usage: deroff [ -w ] [ -m (m s l) ] [ -i ] [ file ] ... \n"),
			(char *) NULL);
	if ( optind == argc )
		infile = stdin;
	else
...
if( (fd = fopen(p, "r")) == NULL)
	fatal(gettext("Cannot open file %s\n"), p);
...
if( (t0 = t = calloc( (unsigned)(strlen(s)+1), sizeof(*t) ) ) == NULL)
	fatal(gettext("Cannot allocate memory"), (char *) NULL);
