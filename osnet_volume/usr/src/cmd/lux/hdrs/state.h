...
#pragma ident	"@(#)state.h	1.4	99/07/29 SMI"
...
typedef struct  drv_list_entry_struct {
	uchar_t	  port;		/* 0 - max number of ports */
	uchar_t	  tgt;		/* 0 - max number of targets */
} Drv_list_entry;
...
typedef struct controller_state_struct {
	...
	uchar_t		num_ports;
	uchar_t		num_tgts; /* Actual numbers as reported by the Pluto */
	uchar_t
			: 2,	/* reserved */
			: 1,	/* reserved */
			: 1,	/* reserved */
...
typedef struct drv_par_tbl_struct {
	uint_t	 state_flags;	/* drive state (bits defined above ) */
	Id_strings id;		/* drive ID as reported by the drive */
	uint_t	  num_blocks;	/* drive capacity in # of 512 byte blocks */
