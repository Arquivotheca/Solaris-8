...
#pragma ident	"@(#)devctl.c	1.15	99/07/29 SMI"
 ...
 */
int
dev_handle_insert(char *path, int force_flag)
{
	int		exit_code = -1;
	bool_t		run_xcmds = TRUE;
	devctl_hdl_t	dcp = NULL;
	char		*pp;				/* physical path */



	/* convert supplied name to physical name */
#ifdef	TWO_SIX
	if ((pp = g_get_dev_or_bus_phys_name(path)) == NULL) {
#else
	if ((pp = g_get_physical_name(path)) == NULL) {
#endif

		(void) fprintf(stderr, "%s: ", whoami);
		(void) fprintf(stdout,
			MSGSTR(112, "Error: Invalid pathname (%s)"), path);
		(void) fprintf(stdout, "\n");
	...
	/* acquire device */
#ifdef	TWO_SIX
	if (devctl_acquire(pp, force_flag ? 0 : DC_EXCL, &dcp) != 0) {
#else
	if ((dcp = devctl_device_acquire(pp, force_flag ? 0 : DC_EXCL)) ==
		NULL) {
#endif
		(void) fprintf(stderr, MSGSTR(3004,
		"%s: can't acquire \"%s\": %s\n"), whoami, pp, strerror(errno));
		goto dun;
	}

	/* take device offline */
	if ((devctl_device_offline(dcp) != 0) && !force_flag) {
		(void) fprintf(stderr, MSGSTR(3005,
			"%s: can't take offline: \"%s\": %s\n"),
	...
	if (devctl_bus_quiesce(dcp) != 0) {
		if (errno != ENOTSUP) {
			(void) fprintf(stderr, MSGSTR(3006,
	...
	if (!okay_to_proceed(MSGSTR(3007,
	...
	if (devctl_bus_unquiesce(dcp) != 0) {
		if (errno != ENOTSUP) {
			(void) fprintf(stderr, MSGSTR(3008,
	...
	if (devctl_device_online(dcp) != 0) {
		(void) fprintf(stderr, MSGSTR(3009,
 ...
 */
int
dev_handle_remove(char *path, int force_flag)
{
	int		exit_code = -1;
	bool_t		run_xcmds = TRUE;
	devctl_hdl_t	dcp = NULL;
	...
	if ((dcp = devctl_device_acquire(pp, force_flag ? 0 : DC_EXCL)) ==
		NULL) {
#endif
		(void) fprintf(stderr, MSGSTR(3010,
			"%s: can't acquire \"%s\": %s\n"),
		    whoami, pp, strerror(errno));
		goto dun;
	}
	...
	/* take device offline */
	if ((devctl_device_offline(dcp) != 0) && !force_flag) {
		(void) fprintf(stderr, MSGSTR(3011,
			"%s: can't take offline: \"%s\": %s\n"),
		    whoami, path, strerror(errno));
		goto dun;
	}
	...
	if (devctl_bus_quiesce(dcp) != 0) {
		if (errno != ENOTSUP) {
			(void) fprintf(stderr, MSGSTR(3012,
			    "%s: warning: can't quiesce \"%s\": %s\n"),
				whoami, path, strerror(errno));
		}
	}
	...
	/* tell user what to do, then wait for them to do it */
	if (!okay_to_proceed(MSGSTR(3013,
	...
	if (devctl_bus_unquiesce(dcp) != 0) {
		if (errno != ENOTSUP) {
			(void) fprintf(stderr, MSGSTR(3014,
			    "%s: warning: can't unquiesce \"%s\": %s\n"),
			    whoami, path, strerror(errno));
		}
	}
	...
	/* online device */
	if (devctl_device_online(dcp) != 0) {
		(void) fprintf(stderr, MSGSTR(3015,
		    "%s: can't put device online: \"%s\": %s\n"),
		    whoami, path, strerror(errno));
		goto dun;
	}
 ...
 */
int
dev_handle_replace(char *path, int force_flag)
{
	int		exit_code = -1;
	devctl_hdl_t	dcp = NULL;
	char		*pp;				/* physical path */
	...
	if ((dcp = devctl_device_acquire(pp, force_flag ? 0 : DC_EXCL)) ==
		NULL) {
#endif
		(void) fprintf(stderr, MSGSTR(3016,
			"%s: can't acquire \"%s\": %s\n"),
		    whoami, pp, strerror(errno));
		goto dun;
	}
	...
	/* take device offline */
	if ((devctl_device_offline(dcp) != 0) && !force_flag) {
		(void) fprintf(stderr, MSGSTR(3017,
			"%s: can't take offline: \"%s\": %s\n"),
		    whoami, path, strerror(errno));
		goto dun;
	}
	...
	if (devctl_bus_quiesce(dcp) != 0) {
		if (errno != ENOTSUP) {
			(void) fprintf(stderr, MSGSTR(3018,
			    "%s: warning: can't quiesce \"%s\": %s\n"),
				whoami, path, strerror(errno));
		}
	}
	 ...
	 * continuing
	 */
	(void) okay_to_proceed(MSGSTR(3019,
	...
	if (devctl_bus_unquiesce(dcp) != 0) {
		if (errno != ENOTSUP) {
			(void) fprintf(stderr, MSGSTR(3020,
			    "%s: warning: can't unquiesce \"%s\": %s\n"),
			    whoami, path, strerror(errno));
		}
	}
	...
	/* online device */
	if (devctl_device_online(dcp) != 0) {
		(void) fprintf(stderr, MSGSTR(3021,
		    "%s: can't put device online: \"%s\": %s\n"),
		    whoami, path, strerror(errno));
		goto dun;
	}
...
run_cmd(char *path)
{
	...
	if ((cpid = fork()) == (pid_t)-1) {
		(void) fprintf(stderr, MSGSTR(133,
	...
	if (child_exit_val != 0) {
		(void) fprintf(stderr, MSGSTR(3026,
			"%s: error running \"%s\": %s\n"),
		    whoami, path, strerror(errno));
	}
