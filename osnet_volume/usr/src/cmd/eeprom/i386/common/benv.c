...
#pragma ident	"@(#)benv.c	1.9	99/12/06 SMI"
...
void
add_item(void *item, list_t *list)
{
	list_t *entry;

	entry = (list_t *)malloc(sizeof (list_t));
	(void) memset(entry, 0, sizeof (list_t));
	entry->item = item;
 ...
 */
static void
add_bent(list_t *list, char *comm, char *cmd, char *name, char *val)
{
	benv_ent_t *bent;
...
static void
print_var(char *name, list_t *list)
{
	benv_ent_t *p;
...
static void
set_var(char *name, char *val, list_t *list)
{
	benv_ent_t *p;
 ...
 */
static int
proc_var(char *name, list_t *list)
{
	register char *val;
...
init_benv(benv_des_t *bd, char *file)
{
	...
	if (test)
		...
		boottree = strcats("/platform/", uts_buf.machine,
		    "/boot", NULL);
