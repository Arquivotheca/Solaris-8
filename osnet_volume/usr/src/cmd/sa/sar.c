...
#pragma ident	"@(#)sar.c	1.28	99/11/24 SMI"
...
main(int argc, char **argv)
{
	...
	while ((i = getopt(argc, argv, "ubdycwaqvmpgrkAo:s:e:i:f:")) != EOF)
		switch (ccc = i) {
		    case 'o':
			oflg++;
			(void) snprintf(ofile, PATH_MAX, "%s", optarg);
			break;
		    case 's':
		    ...
		    case 'f':
			fflg++;
			(void) snprintf(flnm, PATH_MAX, "%s", optarg);
			break;
		    case '?':
			usage();
			exit(1);
	...
	while (safe_read(fin, &nx, sizeof (struct sa))) {
		...
		if (niodevs != old_niodevs) {
			...
			if (SIZE_MAX / sizeof (iodevinfo_t) < niodevs)
				fail(1, "insufficient address space to hold "
				    "%lu device records\n", niodevs);
			...
			if (!input_pipe) {
				...
				size > in_stat.st_size - curr_pos)
				    fail(1, "data file corrupt; specified size"
					"exceeds actual");
			}
...
{
	(void) printf(" %7.0f %7.0f %5.0f %7.0f %7.0f %5.0f %11.0f %5.0f\n",
