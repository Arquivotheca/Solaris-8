...
#ifndef lint
#pragma ident	"@(#)modify_partition.c	1.16	96/12/31 SMI"
#endif	lint
...
struct disk_type *tptr;
{
	...
	if (label == NULL) {
		err_print("build_partition : malloc failed \n");
		fullabort();
	}
	...
	if (part == (struct partition_info *)NULL) {
		err_print("build_partition : malloc failed \n");
		fullabort();
	}
...
struct disk_type	*tptr, *oldtptr;
{
	...
	if ((oldtptr != NULL) &&
	    ...
	    part = (struct partition_info *)
			zalloc(sizeof (struct partition_info));
	    if (part == (struct partition_info *)NULL) {
			err_print("new_partitiontable : malloc failed \n");
			fullabort();
	    }
