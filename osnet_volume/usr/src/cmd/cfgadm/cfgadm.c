...
#pragma ident	"@(#)cfgadm.c	1.7	99/07/26 SMI"
...
{"info", "Information", 40, compare_info, print_info, NULL},
...
out_was_req(ap_out_t *outp, ap_arg_t *in_array, int nargs, int no_dyn)
{
	...
	(void) snprintf(physid, sizeof (physid), outp->ldatap->ap_phys_id);
	(void) snprintf(logid, sizeof (logid), outp->ldatap->ap_log_id);
