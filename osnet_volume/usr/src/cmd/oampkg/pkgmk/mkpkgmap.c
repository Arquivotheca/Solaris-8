...
#pragma ident	"@(#)mkpkgmap.c	1.23	99/02/18 SMI"	/* SVr4.0  1.11.2.1	*/
...
extern char	*errstr, *basedir, *root, *rootlist[], **environ;
...
{
	FILE	*tmpfp;
	char	*pt, *path, mybuff[PATH_MAX], temp[PATH_MAX];
	char	**envsave;
	int	c, fakebasedir;
	int	i, n;
	...
	while (!feof(fp)) {
		...
		if (!strchr("isl", entry.ftype)) {
			...
			if (strcmp(entry.ainfo.owner, NOOWNER) == 0) {
				(void) strcpy(entry.ainfo.owner, CUROWNER);
				dowarning=1;
			}

			if (strcmp(entry.ainfo.group, NOGROUP) == 0) {
				(void) strcpy(entry.ainfo.group, CURGROUP);
				dowarning=1;
			}
...
doinclude(void)
{
	char file[PATH_MAX], temp[PATH_MAX];
