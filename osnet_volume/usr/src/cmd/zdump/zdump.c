 ...
 */
#pragma ident	"@(#)zdump.c 1.9	99/06/11 SMI"
...
int	v;
{
	...
	(void) printf(gettext("%-*s  "), (int)longest, zone);
	if (v)
		(void) printf(gettext("%.24s UTC = "), asctime(gmtime(&t)));
	tmp = localtime(&t);
	(void) printf(gettext("%.24s"), asctime(tmp));
	if (*abbr(tmp) != '\0')
		(void) printf(gettext(" %s"), abbr(tmp));
	if (v) {
		(void) printf(gettext(" isdst=%d"), tmp->tm_isdst);
	}
	(void) printf(gettext("\n"));
}
