...
#ident	"@(#)lex.c	1.6	92/07/14 SMI"	/* SVr4.0 1.1	*/
#include <stdio.h>
#include <string.h>
#include <ctype.h>
#include "colltbl.h"
...
again:
	...
	if (c == '"') {
		...
		buf[clen] = '\0';
		yylval.sval = (unsigned char*)strdup(buf);
		start = TRUE;
		return (STRING);
	}
	...
	default:
		...
		do {
			...
			if (c == '\\' || c == '0') {
				...
				j = 0;
				if (c == 'x' || c == 'X') {
					while (isxdigit(c=getchar()) && j<2)
						ibuf[j++] = c;
					ibuf[j] = '\0';
					buf[clen++] = strtol(ibuf, NULL, 16);
				} else {
					ibuf[j++] = c;
					c = getchar();
					while (isodigit(c) && j<3) {
