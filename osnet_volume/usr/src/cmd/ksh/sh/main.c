#pragma ident	"@(#)main.c	1.19	99/04/30 SMI"	/* From AT&T Toolchest */
...
int 	pfkshflag = 0;	/* for pfksh, set to 1 of this is profile ksh */
...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it weren't */
	 ...
	 *
	 * 'pfkshflag' is set to 1 in env_namset if SHELL variable
	 * is set in environment and contains a 'pf' in
	 * the basename part of the value.
	 */
	rsflag=env_init();
...
secpolicy_print(int level, const char *msg)
{
	...
	case SECPOLICY_WARN:
	default:
		p_str((const char *)gettext(msg), SP);
		return;
