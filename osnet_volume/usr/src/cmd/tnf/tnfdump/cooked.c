...
#pragma ident	"@(#)cooked.c	1.7	95/07/31 SMI"
...
describe_event(tnf_datum_t datum, tnf_datum_t schedule_rec, hrtime_t evt_time)
{
	...
	if (strcmp(tidtype, K_TID) == 0) {
		...
		(void) printf("%16.6f %16.6f %5u %5u 0x%-8x %3d",
			evt_time / 1000000.0, (evt_time - last_time)/1000000.0,
			pid, lwpid, tid, cpuid);
	} else {
		/* print the OUTPUT schedule record */
		(void) printf("%16.6f %16.6f %5u %5u %10u %3s",
			evt_time / 1000000.0, (evt_time - last_time)/1000000.0,
			pid, lwpid, tid, "-");
	}
	/* print the tag */
