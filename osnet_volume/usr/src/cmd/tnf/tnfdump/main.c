...
#pragma ident	"@(#)main.c	1.12	97/10/31 SMI"
...
describe_scalar(tnf_datum_t datum)
{
	...
	case TNF_K_INT8:
		...
		break;
	case TNF_K_UINT8:
		(void) printf("%u", (tnf_uint8_t)tnf_get_int8(datum));
		break;
	case TNF_K_INT16:
		(void) printf("%d", tnf_get_int16(datum));
		break;
	case TNF_K_UINT16:
		(void) printf("%u", (tnf_uint16_t)tnf_get_int16(datum));
		break;
	case TNF_K_INT32:
	...
	case TNF_K_UINT64:
		if ((tnf_type_get_property(tnf_get_type(datum), TNF_N_OPAQUE))
			!= TNF_DATUM_NULL) {
		(void) printf("0x%llx", (tnf_uint64_t)tnf_get_int64(datum));
		} else {
		/* lint not updated, it complains: malformed format string */
		(void) printf("%llu", 	(tnf_uint64_t)tnf_get_int64(datum));
		}
		break;
	case TNF_K_FLOAT32:
