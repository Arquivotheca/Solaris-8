...
#ident	"@(#)uniq.c	1.18	95/06/27 SMI"	/* SVr4.0 1.4	*/
...
register char *s;
{
	 ...
	 * skipped, examine each (possibly m.b.) char. for each field,
	 * we first skip all blanks. then we skip any non-blank chars.
	 */
	while (nf++ < fields) {	/* while there're fields to be examined	*/
		/* while there's a (possibly m.b.) character to get:	*/
		while (clen = mbtowc(&wc, s, MB_CUR_MAX) &&
			wc != (wchar_t) NULL && iswblank(wc)) {
			s += clen;		/* move to next char	*/
		}
	...
	/* while there're any (possibly m.b.) letters to skip:		*/
	while (nl++ < letters) {		/* skip them		*/
		clen = mbtowc(&wc, s, MB_CUR_MAX);

		/* if we've hit the end, no need to continue:	*/
		if ((clen == 0) || (wc == (wchar_t) NULL)) {
			break;
		}
		s += clen;		/* move to next char	*/
	}
...
#ifdef DHL_ISBLANK
/*
 * isblank():	see if a (possibly mulitbyte) character is a blank.
...
iswblank(wint_t c)
{
	...
	if (c > 255)
		return ((int) _iswctype((wchar_t) c, _S|_B));
	else
		return (isblank(c));
}
#endif /* DHL_ISBLANK */
