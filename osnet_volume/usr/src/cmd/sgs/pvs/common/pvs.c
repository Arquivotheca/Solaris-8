 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)pvs.c	1.24	99/09/14 SMI"
 ...
 */
static int
gvers_need(Cache * cache, Cache * need, const char * file, int oflag, int vflag,
	const char * name)
{
	...
	for (_num = 1; _num <= num; _num++,
	    vnd = (GElf_Verneed *)((GElf_Addr)vnd + vnd->vn_next)) {
		GElf_Vernaux *	vnap = (GElf_Vernaux *)
					((GElf_Addr)vnd + vnd->vn_aux);
		 ...
		 */
		/* CSTYLED */
		for (vnap = (GElf_Vernaux *)((GElf_Addr)vnap + vnap->vna_next);
		    cnt;
		    cnt--,
			vnap = (GElf_Vernaux *)((GElf_Addr)vnap +
						vnap->vna_next)) {
			dep = (char *)(strs + vnap->vna_name);
...
gvers_desc(const char * name, unsigned long hash, List * lst, const char * file)
{
	...
	if ((vdp = gvers_find(name, hash, lst)) == 0) {
		if ((vdp = (GVer_desc *)calloc(sizeof (GVer_desc), 1)) == 0) {
			int err = errno;
...
static void
gvers_syms(GElf_Versym * vsp, Elf_Data * sym_data, size_t symn, char * strs,
	GVer_desc * vdp, const char * file, int oflag, int vflag)
{
	GElf_Sym	sym;
	int		_symn;
	...
	for (_symn = 0; _symn < symn; _symn++) {
		 ...
		 */
		(void) gelf_getsym(sym_data, (int)_symn, &sym);
...
static void
recurse_syms(GElf_Versym * vsp, Elf_Data * sym_data, size_t symn, char * strs,
	GVer_desc * vdp, const char * file, int oflag, int vflag)
{
	Listnode *	_lnp;
	GVer_desc *	_vdp;
	...
	for (LIST_TRAVERSE(&vdp->vd_deps, _lnp, _vdp)) {
		...
		gvers_syms(vsp, sym_data, symn, strs, _vdp, file, oflag, vflag);
...
gvers_def(Cache * cache, Cache * def, Cache * csym, const char * file,
	int nflag, int oflag, int vflag, const char * name)
{
	unsigned int	num, _num;
	char *		strs;
	GElf_Versym *	vsp;
	 ...
	 */
	for (_num = 1; _num <= num; _num++,
	    vdf = (GElf_Verdef *)((GElf_Addr)vdf + vdf->vd_next)) {
		GElf_Half	cnt = vdf->vd_cnt;
		GElf_Half	ndx = vdf->vd_ndx;
		GElf_Verdaux *	vdap = (GElf_Verdaux *)((GElf_Addr)vdf +
							vdf->vd_aux);
		const char *	_name;
		...
		vdap = (GElf_Verdaux *)((GElf_Addr)vdap + vdap->vda_next);
		for (cnt--; cnt; cnt--,
		    vdap = (GElf_Verdaux *)((GElf_Addr)vdap + vdap->vda_next)) {
			_name = (char *)(strs + vdap->vda_name);
	...
	for (LIST_TRAVERSE(&verdefs, lnp, vdp)) {
		 ...
		 * version.
		 */
		gvers_syms(vsp, sym_data, symn, strs, vdp, file, oflag, vflag);
		if (name) {
...
main(int argc, char ** argv, char ** envp)
{
	...
	for (; optind < argc; optind++) {
		 ...
		 */
		if (_cache_need)
			nerror = gvers_need(cache, _cache_need, file, oflag,
			    vflag, name);
