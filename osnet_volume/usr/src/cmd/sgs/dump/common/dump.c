...
#pragma ident	"@(#)dump.c	6.53	99/09/28 SMI"
	...
	long range, int index)
{
	...
	while (range > 0) {
		...
		if (v_flag && (type == STT_SPARC_REGISTER)) {
			/*
			 *  The strings "REG_G2" and "REG_G3" are intended
		...
		if (!v_flag) {
			...
			case SHN_COMMON:
				...
				break;
			default:
				(void) printf("%d", EC_WORD(sym.st_shndx));
			}
			(void) printf("\t");
		}
...
main(int argc, char *argv[], char *envp[])
{
	...
	while ((optchar = getopt(argc, argv, optstr)) != -1) {
		...
		case 'l':
			(void) fprintf(stderr,
			    "The -l option is not supported on SOLARIS.\n");
			break;
		case 's':
			s_flag = 1;
			x_flag = 1;
		...
		case 'D':
			(void) fprintf(stderr,
			    "The -D option is not supported on SOLARIS.\n");
			break;
		case 'V':
			V_flag = 1;
