...
#pragma ident	"@(#)cmd.c	1.15	97/11/20 SMI"
		...
		if (cmd_info->namc == 0 ||
			...
			if (f == NULL) {
				if (cmd_info->namc)
					...
					SYSTEM_ERROR, strerror(errno), gfile);
				/*
				 * Created
				 */
	...
	for (next = getfile(cmd_info); next; next = getfile(cmd_info)) {
		...
		if (f >= 0) {
			if (rawname) {
				...
				if (write(f, next->ar_contents, (unsigned)
	...
	for (i = 0; i < cmd_info->namc; i++) {
		...
		if (f == NULL)
			...
			if ((fileptr->ar_longname
			    = (char *)malloc(strlen(trim(cmd_info->namv[i]))
						* sizeof (char *)))
			    == NULL) {
				error_message(MALLOC_ERROR,
				PLAIN_ERROR, (char *)0);
			...
			if ((fileptr->ar_pathname
			    = (char *)malloc(strlen(cmd_info->namv[i]) *
				sizeof (char *)))
			    == NULL) {
				error_message(MALLOC_ERROR,
				PLAIN_ERROR, (char *)0);
