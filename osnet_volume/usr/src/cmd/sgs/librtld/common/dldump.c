 ...
 */
#pragma ident	"@(#)dldump.c	1.30	99/09/14 SMI"
 ...
 *
 * pfd is a /proc file descriptor which is only provided if the original ipath
...
rt_dldump(Rt_map * lmp, const char * opath, int pfd, int flags, Addr addr)
{
	...
	for (scn = 0; scn = elf_nextscn(ielf, scn); _icache++) {
		 ...
		 */
		if ((shdr->sh_type == M_DYNREL_SHT_TYPE) && shdr->sh_addr)
			continue;
	...
	*_shstr++ = '\0';
	...
	for (_icache = &icache[1]; _icache->c_flags != FLG_C_END; _icache++) {
		 ...
		 */
		if (shdr->sh_type == M_DYNREL_SHT_TYPE) {
			if (rel_base == (Rel *)0) {
