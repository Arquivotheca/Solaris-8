...
#pragma ident	"@(#)setupsr.c	1.22	99/03/23 SMI"
...
#define uoffset ((char *)&proc.p_uarea - (char *)&proc)
...
#ifdef	KADB
#ifdef	BOGUS
	flags = rwmap('r', (uintptr_t)Curproc + foffset, DSP, 0);
	if (errflg) {
	...
	uptr = (struct user *)rwmap('r', (uintptr_t)Curproc + uoffset, DSP, 0);
	if (errflg) {
		errflg = "cannot get u area";
		return;
	}
	(void) rwmap('r', (uintptr_t)&(uptr->u_pcb.pcb_regs), DSP, 0);
	if (errflg) {
...
#define tpcb_offset ((char *)&thread->t_pcb - (char *)thread)
	...
	tpcb_addr = (uintptr_t)Curthread + tpcb_offset;
