...
#ident	"@(#)tmlog.c	1.10	97/01/23 SMI"	/* SVr4.0 1.12	*/
...
char *msg;
{
	...
	if (Logfp) {
		...
		(void) strcpy(logf, LOGDIR);
		(void) strcat(logf, Tag);
		(void) strcat(logf, "/");
		(void) strcpy(ologf, logf);
		(void) strcpy(tlogf, logf);
		(void) strcat(logf, LOGFILE);
		(void) strcat(ologf, OLOGFILE);
		(void) strcat(tlogf, TLOGFILE);
		...
		if (!stat(ologf, &buf) && rename(ologf, tlogf)) {
		    ...
		    if (!stat(ologf, &buf) && rename(ologf, logf))
		        (void) fprintf(Logfp, "rename old to log file failed\n");
