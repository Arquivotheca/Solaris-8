...
#pragma ident	"@(#)unix_login.c	1.32	99/11/09 SMI"
...
HelperRead(pollfd_t *fdp, int nfds, int *pollretval)
{
	...
	/*
	 * fdp pollset may be compressed. Search for Master and InputSocket
	 * fds, and perform error recovery if not found, using the extra
	 * struct pollset provided in main() for this purpose.
	 */
	for (master = 0; master < nfds; master++)
		if (fdp[master].fd == Master)
			break;
	if (master == nfds)
		fdp[master].revents = 1;	/* shutdown if no Master fd */

	for (inputsocket = 0; inputsocket < nfds; inputsocket++)
		if (fdp[inputsocket].fd == InputSocket)
			break;

/*	mask = sigsetmask (sigmask (SIGCHLD));	*/
	mask = sighold(SIGCHLD);
	if (fdp[master].revents != 0) {
	...
	if (inputsocket == nfds)
		fdp[inputsocket].revents = 1;	/* shutdown if no InputSocket */
