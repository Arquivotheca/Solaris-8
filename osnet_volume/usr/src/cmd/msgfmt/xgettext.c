 ...
 */
#pragma ident   "@(#)xgettext.c 1.13     99/03/12 SMI"
...
static void add_line_to_comment();
static void add_qstring_to_str();
static void add_str_to_element_list();
static void copy_strlist_to_str();
static void end_ansi_string();
static void free_strlist();
void handle_newline();
static void initialize_globals();
static void output_comment();
static void output_msgid();
static void output_textdomain();
static void print_help();
static void read_exclude_file();
static void trim_line();
static void write_all_files();
static void write_one_file();
 ...
 */
static struct domain_st  *new_domain();
static struct strlist_st *new_strlist();
static struct element_st *new_element();
static struct exclude_st *new_exclude();
 ...
 */
static void
extract_filename_linenumber(mline)
	char	*mline;
{
	int	num;
	char	*p, *q, *r;
 ...
 */
void
add_str_to_element_list(istextdomain, domain_list)
	int	istextdomain;
	char	*domain_list;
{
...
static
void
add_node_to_polist(pohead, potail, elem)
	struct element_st **pohead, **potail, *elem;
{
	if (elem == NULL) {
		return;
	}
...
print_all_domain(dom_list)
	struct domain_st	*dom_list;
{
	struct domain_st	*p;
	struct element_st	*q;
