...
#pragma ident	"@(#)mixerctl.c	1.4	99/11/08 SMI"

/*
 * Combined mixerctl/mixeron/mixeroff command:
 *	mixerctl [-a|-d dev] [-sv] [-e|-o]
 *
...
#define	TEXT_DOMAIN "SYS_TEST"	/* Use this only if it weren't */
	...
	/*
	 * Determine command invoked (mixerctl, mixeron, mixeroff)
	 */
	...
	if (info.all) {
		 ...
		 *	dev_number{function}
		 *
		 * Currently there are only enteries like /dev/sound/0 (audio
		...
		while ((dp = readdir(dirp)) != NULL) {
			...
			/* if next char is NULL then not and audio ctl device */
			if (*cp == '\0') {
				if (info.debug) {
					(void) printf(gettext("Skipping %s\n"),
					    dp->d_name);
		...
		if (info.device == 0) {
			if ((info.device = getenv(ENV)) == NULL) {
			    if (info.debug) {
				(void) printf(
				    gettext("using default device: %s\n"),
				    DEFAULT_DEVICE);
			    }
			    info.device = DEFAULT_DEVICE;
			} else {
			    /* using ENV value, add "ctl" to it */
			    (void) strcpy(dev_buffer, info.device);
			    (void) strcat(dev_buffer, CONTROL);
			    info.device = dev_buffer;
			    if (info.debug) {
				(void) printf(gettext("using ENV device: %s\n"),
				    info.device);
			    }
			}
		}
...
doit(info_t *info, char *dev)
{
	...
	if (info->verbose || info->info) {
		if (ioctl(fd, AUDIO_GETDEV, &dev_info) < 0) {
			(void) fprintf(stderr, gettext(
			    "%s: AUDIO_GETDEV ioctl() for device %s fialed"),
			    info->pgm, dev);
			(void) close(fd);
			return (1);
		}
	...
	if (ioctl(fd, AUDIO_MIXERCTL_GET_MODE, &mode) < 0) {
		...
		if (!info->disable) {
			...
			if (ioctl(fd, AUDIO_MIXERCTL_SET_MODE, &new_mode) < 0) {
			    (void) fprintf(stderr, gettext(
				"%s: couldn't disable audio mixer for %s\n"),
				info->pgm, dev);
			    rc = 1;
			}
		}
	...
	if (info->verbose) {
		if (ioctl(fd, AUDIO_GETINFO, &audio_info) < 0) {
			(void) fprintf(stderr, gettext(
			    "%s: AUDIO_GETINFO ioctl() for device %s fialed"),
			    info->pgm, dev);
			(void) close(fd);
			return (1);
		}
