...
#pragma ident   "@(#)postio.c	1.15	95/10/31 SMI"	/* SVr4.0 1.2   */
...
int	ecppmode = 0;			/* flag for ecpp support */
...
main(int agc, char *agv[])
{
	...
	ecppmode = get_ecpp_status(1);

	if (is_a_parallel_bpp(1) || (ecppmode == ECPP_CENTRONICS)) {
		initialize_parallel();
		x_stat = parallel_comm(1, bpp_state);
	} else if (is_a_parallel_spif(1)) {
		initialize_parallel();
		x_stat = parallel_comm(1, spif_state);
	} else if (isatty(1) || (ecppmode == ECPP_ECP_MODE)) {
		if (ecppmode == ECPP_ECP_MODE) {
			close(1);
		}
