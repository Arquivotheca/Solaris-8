...
#pragma ident	"@(#)Kstat.xs	1.2	99/11/24 SMI"
...
#define	PERL_ASSERT(EXP) \
	((void)((EXP) || (croak(DEBUG_ID #EXP), 0), 0))
...
static void
save_cpu_stat(HV *self, kstat_t *kp)
{
	cpu_stat_t    *statp;
...
static void
save_flushmeter(HV *self, kstat_t *kp)
{
...
static void
save_ncstats(HV *self, kstat_t *kp)
{
...
static void
save_sysinfo(HV *self, kstat_t *kp)
{
	sysinfo_t *sysinfop;
...
static void
save_vminfo(HV *self, kstat_t *kp)
{
	vminfo_t *vminfop;
...
#ifdef __sparc
static void
save_sfmmu_global_stat(HV *self, kstat_t *kp)
{
	...
	SAVE_INT32(self, sfmmugp, sf_slow_tsbmiss);
	...
	SAVE_INT32(self, sfmmugp, sf_hblk8_dalloc);
	SAVE_INT32(self, sfmmugp, sf_hblk8_dfree);
	SAVE_INT32(self, sfmmugp, sf_hblk1_nalloc);
	SAVE_INT32(self, sfmmugp, sf_hblk1_dalloc);
	SAVE_INT32(self, sfmmugp, sf_hblk1_dfree);
	SAVE_INT32(self, sfmmugp, sf_hblk8_startup_use);
	...
	SAVE_INT32(self, sfmmugp, sf_mlist_enter);
	SAVE_INT32(self, sfmmugp, sf_mlist_exit);
	...
	SAVE_INT32(self, sfmmugp, sf_tsb_resize);
...
#ifdef __sparc
static void
save_simmstat(HV *self, kstat_t *kp)
{
	uchar_t	*simmstatp;
	SV	*list;
	int	i;
...
static void
save_temperature(HV *self, kstat_t *kp)
{
...
#ifdef __sparc
static void
save_temp_over(HV *self, kstat_t *kp)
{
	short *shortp;
...
#ifdef __sparc
static void
save_ps_shadow(HV *self, kstat_t *kp)
{
	uchar_t *ucharp;
...
#ifdef __sparc
static void
save_fault_list(HV *self, kstat_t *kp)
{
	struct ft_list	*faultp;
	int		i;
	char		name[KSTAT_STRLEN + 4];	/* room for 999 faults */

	/* PERL_ASSERT(kp->ks_ndata == 1); */
	/* PERL_ASSERT(kp->ks_data_size == sizeof (struct ft_list)); */

	for (i = 1; i <= kp->ks_data_size / sizeof (struct ft_list);
	i++, faultp++) {
		snprintf(name, KSTAT_STRLEN + 4, "unit_%d", i);
		hv_store(self, name, strlen(name), newSViv(faultp->unit), 0);
		snprintf(name, KSTAT_STRLEN + 4, "type_%d", i);
		hv_store(self, name, strlen(name), newSViv(faultp->type), 0);
		snprintf(name, KSTAT_STRLEN + 4, "fclass_%d", i);
		hv_store(self, name, strlen(name), newSViv(faultp->fclass), 0);
		snprintf(name, KSTAT_STRLEN + 4, "create_time_%d", i);
		hv_store(self, name, strlen(name),
		NEW_UIV(faultp->create_time), 0);
		snprintf(name, KSTAT_STRLEN + 4, "msg_%d", i);
		...
		while (entry2 = hv_iternext(hash2)) {
			...
			while (entry3 = hv_iternext(hash3)) {
				...
				if (((KstatInfo_t *)SvPVX(
					...
					hv_delete(hash3, HePV(entry3, klen),
					    klen, G_DISCARD);
					if (del) {
						av_push(del,
						    newSVpvf("%s:%s:%s",
			...
			if (HvKEYS(hash3) == 0) {
				hv_delete(hash2, HePV(entry2, klen), klen,
				    G_DISCARD);
			}
		}
...
save_named(HV *self, kstat_t *kp)
{
	...
	for (n = kp->ks_ndata, knp = KSTAT_NAMED_PTR(kp); n > 0; n--, knp++) {
		...
		case KSTAT_DATA_CHAR:
			value = newSVpv(knp->value.c, sizeof (knp->value.c));
			break;
...
read_kstats(HV *self, int refresh)
{
	...
	if (! kstat_read(kip->kstat_ctl, kip->kstat, 0)) {
		return (0);
	}
...
CODE:
	...
	for (kp = kc->kc_chain; kp != 0; kp = kp->ks_next) {
		...
		if (kp->ks_type == KSTAT_TYPE_RAW &&
			...
			printf("Unknown kstat type %s:%d:%s - %d of size %d\n",
...
PPCODE:
	...
	/* Keep calling kstat_chain_update until it doesn't change any more */
	updated = 0;
	while ((ret = kstat_chain_update(kc)) > 0) { updated = 1; }
	if (ret == -1) {
		if (GIMME_V == G_ARRAY) {
			EXTEND(SP, 2);
			PUSHs(sv_newmortal());
	 ...
	 */
		...
		for (kp = kc->kc_chain; kp != 0; kp = kp->ks_next) {
			...
			if (kp->ks_type == KSTAT_TYPE_RAW &&
			    ...
			    == 0) {
#ifdef REPORT_UNKNOWN
				printf("Unknown kstat type %s:%d:%s - %d "
				    "of size %d\n", kp->ks_module,
			...
			/* If the stats already exist, just update them */
				...
				mg = mg_find((SV *)tie, '~');
				PERL_ASSERTMSG(mg != 0, "update: lost ~ magic");
				((KstatInfo_t *)SvPVX(mg->mg_obj))->valid =
				    TRUE;
