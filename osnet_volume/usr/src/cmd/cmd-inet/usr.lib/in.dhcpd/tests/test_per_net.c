#ident	"@(#)test_per_net.c	1.8	99/10/13 SMI"
...
main()
{
	...
	PER_NET_DB	pndb;
	PN_REC		pn;
	u_char buf[MAX_CID_LEN] = {0x1, 0x0, 0x0, 0xc0, 0xee, 0xe, 0x4c };
	...
	if (recs < 0)
		...
		if (recs > 0) {
			...
			len = MAX_CID_LEN;
			octet_to_ascii((u_char *)&pn.lease, 4, tbuf, &len);
	...
	if (recs < 0)
		...
		if (recs > 0) {
			...
			for (i = 0; i < recs; i++) {
				...
				len = MAX_CID_LEN;
				octet_to_ascii((u_char *)&pn.lease, 4, tbuf,
				    &len);
				printf("lease is %s, 0x%x\n", tbuf, pn.lease);
				printf("macro is %s\n", pn.macro);
			}
