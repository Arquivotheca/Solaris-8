/*
 * @(#)MacroView.java	1.4	99/05/05 SMI
 *
...
class MacroLoader extends SwingWorker {
	...
	MacroView.macroTreeModel.getRootNode().setMacros((Macro [])get());
    }
}
...
public class MacroView implements View {
	...
	macroHelp = new JMenuItem(ResourceStrings.getString("on_macros_item"));
	...
	ints[0] = new Integer(macroTreeModel.getRootNode().getChildCount());
	String s = MessageFormat.format(
	    ResourceStrings.getString("macro_status_message"), ints);
	...
	if (state) {
		...
		try {
			...
			Class.forName("com.sun.dhcpmgr.ui.MainFrame"),
			macroTree);
