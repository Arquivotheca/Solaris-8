...
#pragma ident	"@(#)packet.c	1.3	99/09/21 SMI"
...
static float	fuzzify(uint32_t, float);
static void 	retransmit(tq_t *, void *);
...
init_pkt(struct ifslist *ifsp, uchar_t type)
{
	...
	(void) memcpy(dpkt->pkt->chaddr, ifsp->if_hwaddr, ifsp->if_hwlen);

	dpkt->pkt->op    = BOOTREQUEST;
	dpkt->pkt->xid   = rand() ^ (rand() << 10) ^ (rand() << 20);
 ...
 *	    void *: the value of that option
 *	    uchar_t: the length of the value of the option
 *  output: void
 */

void
add_pkt_opt(dhcp_pkt_t *dpkt, uchar_t opt_type, void *opt_val, uchar_t opt_len)
{
 ...
 *	    float: the approximate percentage of that time to return
 *  output: float: a number approximating (time * approx_fraction)
 */

static float
fuzzify(uint32_t sec, float approx_fraction)
{
	float	noise[] = { -.02, -.01, 0, .01, .02 };

	return (sec * (approx_fraction + noise[rand() % 5]));
}
...
send_pkt_internal(struct ifslist *ifsp)
{
	...
	if (ifsp->if_send_timeout != 0) {
		...
		if ((ifsp->if_retrans_timer = tq_schedule_timer(tq,
	...
	if (n_bytes != dpkt->pkt_cur_len) {
		...
		if (ifsp->if_retrans_timer == -1)
			...
			dhcpmsg(MSG_WARNING, "send_pkt_internal: cannot send "
			    "%s packet to server (will retry in %d seconds)",
			    pkt_name, ifsp->if_send_timeout);
		return (0);
	}
...
retransmit(tq_t *tqp, void *arg)
{
	...
	if (ifsp->if_send_stop_func(ifsp, ifsp->if_packet_sent) == B_TRUE)
		return;
...
{
	if (ifsp->if_retrans_timer != -1) {
		if (tq_cancel_timer(tq, ifsp->if_retrans_timer, NULL) == 1) {
			(void) release_ifs(ifsp);
			ifsp->if_retrans_timer = -1;
		}
	}
    ...
    boolean_t chain)
{
	 ...
	 * calloc the PKT_LIST since _dhcp_options_scan() relies on it
	 * being zeroed.
	 */
	...
	if (plp->pkt->xid != ifsp->if_send_pkt.pkt->xid) {
		dhcpmsg(MSG_VERBOSE, "received wrong packet xid (%#x "
 ...
 *
 *   input: uint32_t: the number of seconds until the last retransmission
 *  output: uint32_t: the number of seconds until the next retransmission
 */

static uint32_t
next_retransmission(uint32_t last_retrans)
{
	uint32_t	timeout;
	int		noise[] = { -1, 0, 1 };

	/*
	 * start at 4, and increase by a factor of 2 up to 64, then
	 * spice it up with a little randomness.
	 */

	timeout = (last_retrans == 0) ? 4 : MIN(last_retrans << 1, 64);

	return (timeout + noise[rand() % (sizeof (noise) / sizeof (*noise))]);
}
