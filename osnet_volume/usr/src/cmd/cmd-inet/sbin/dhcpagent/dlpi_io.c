...
#pragma ident	"@(#)dlpi_io.c	1.2	99/04/15 SMI"
...
dlpi_open(const char *if_name, dl_info_ack_t *dlia, size_t dlia_size,
    t_uscalar_t dl_sap)
{
	char		device_name[sizeof ("/dev/") + IFNAMSIZ] = "/dev/";
	char		*ppa_ptr = strpbrk(if_name, "0123456789");
	int		ppa;
	int		fd;

	if (ppa_ptr == NULL) {
		dhcpmsg(MSG_ERROR, "dlpi_open: no ppa in interface name");
		return (-1);
	}

	if (strchr(ppa_ptr, ':') != NULL) {
		dhcpmsg(MSG_ERROR, "dlpi_open: virtual interfaces not "
		    "supported");
		return (-1);
	}
	...
	(void) strncat(device_name, if_name, ppa_ptr - if_name);
	if ((fd = open(device_name, O_RDWR)) == -1) {
		dhcpmsg(MSG_ERR, "dlpi_open: open on `%s'", device_name);
		return (-1);
	}
