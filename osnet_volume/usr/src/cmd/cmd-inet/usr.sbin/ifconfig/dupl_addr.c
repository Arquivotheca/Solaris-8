 ...
 * and IPv6 address.
 */
#ident	"@(#)dupl_addr.c 1.2	99/03/26 SMI"
...
do_dad(char *ifname, struct sockaddr_in6 *testaddr)
{
	...
	/*
	 * Determine interface index (for IPV6_BOUND_IF) and
	...
	ret = run_dad(s, phyname, testaddr, &solicited_mc);
 ...
 */
static int
recv_dad(int s, char *phyname, struct sockaddr_in6 *testaddr)
{
	struct sockaddr_in6 from;
