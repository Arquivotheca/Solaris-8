...
#ident	"@(#)snoop_rip.c	1.5	99/03/21 SMI"	/* SunOS	*/
	...
	int fraglen;
{
	...
	if (flags & F_SUM) {
		...
		case RIPCMD_POLL:
			len = fraglen - 4;
			count = 0;
			for (n = rip->rip_nets; len >= sizeof (struct netinfo);
			    n++) {
				count++;
				len -= sizeof (struct netinfo);
			}
			(void) sprintf(get_sum_line(),
			    "RIP %s (%d destinations)", p, count);
			break;
		case RIPCMD_TRACEON:
	...
	if (flags & F_DTAIL) {
		...
		case RIPCMD_POLL:
			...
			for (n = rip->rip_nets; len >= sizeof (struct netinfo);
				...
				(void) sprintf(get_line((char *)n - dlc_header,
				    ...
				    "(default)" :
				    addrtoname(AF_INET, &sin->sin_addr),
		...
		case RIPCMD_POLLENTRY:
			...
			(void) sprintf(get_line((char *)sin - dlc_header,
			    sizeof (struct sockaddr)), "Destination = %s %s",
			    inet_ntoa(sin->sin_addr),
			    addrtoname(AF_INET, &sin->sin_addr));
			sin = (struct sockaddr_in *)&ep->rtu_router;
			(void) sprintf(get_line((char *)sin - dlc_header,
			    sizeof (struct sockaddr)), "Router      = %s %s",
			    inet_ntoa(sin->sin_addr),
			    addrtoname(AF_INET, &sin->sin_addr));
			(void) sprintf(get_line((char *)&ep->rtu_flags -
			    dlc_header, 2), "Flags = %4x", ep->rtu_flags);
			(void) sprintf(get_line((char *)&ep->rtu_state -
			    dlc_header, 2), "State = %d", ep->rtu_state);
			(void) sprintf(get_line((char *)&ep->rtu_timer -
			    dlc_header, 4), "Timer = %d", ep->rtu_timer);
			(void) sprintf(get_line((char *)&ep->rtu_metric -
			    dlc_header, 4), "Metric = %d", ep->rtu_metric);
			(void) sprintf(get_line((char *)&ep->int_flags -
			    dlc_header, 4), "Int flags = %8x", ep->int_flags);
			(void) sprintf(get_line((char *)ep->int_name -
			    dlc_header, 16), "Int name = \"%s\"", ep->int_name);
			break;

		case RIPCMD_TRACEON:
		case RIPCMD_TRACEOFF:
			(void) sprintf(get_line((char *)rip->rip_tracefile -
			    dlc_header, 2), "Trace file = %s",
			    rip->rip_tracefile);
			break;
		}
	}
