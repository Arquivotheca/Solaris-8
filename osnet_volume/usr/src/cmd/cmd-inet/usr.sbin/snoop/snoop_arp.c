...
#ident	"@(#)snoop_arp.c	1.6	99/03/21 SMI"	/* SunOS	*/
	...
	int alen;
{
	...
	if (flags & F_SUM) {
		...
		case ARPOP_REPLY:
			(void) sprintf(line, "ARP R %s is %s",
			    printip(ap->arp_spa), printether(&ap->arp_sha));
		...
		case REVARP_REPLY:
			(void) sprintf(line, "RARP R %s is %s",
			    printether(&ap->arp_tha), printip(ap->arp_tpa));
			dst_name = addrtoname_align(ap->arp_tpa);
			break;
		}
	}
	...
	if (flags & F_DTAIL) {
		...
		show_space();
		(void) sprintf(get_line(0, 0), "Hardware type = %d",
		    ntohs(ap->arp_hrd));
		(void) sprintf(get_line(0, 0), "Protocol type = %04x (%s)",
		    ntohs(ap->arp_pro), print_ethertype(ntohs(ap->arp_pro)));
		(void) sprintf(get_line(0, 0),
		    "Length of hardware address = %d bytes", ap->arp_hln);
		(void) sprintf(get_line(0, 0),
		    "Length of protocol address = %d bytes", ap->arp_pln);
		(void) sprintf(get_line(0, 0), "Opcode %d (%s)",
		    ntohs(ap->arp_op), opname[ntohs(ap->arp_op)]);
