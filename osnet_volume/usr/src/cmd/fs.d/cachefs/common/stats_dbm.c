...
#pragma ident "@(#)stats_dbm.c   1.15     97/11/05 SMI"
    ...
    ino64_t fileno, u_int count)
{
	...
	(void) sprintf(keystring, "%s.%lld", cacheid,
	    fileno / (ino64_t)mi->mi_filegrp_size);
	gfileno = (u_int)(fileno % (ino64_t)mi->mi_filegrp_size);
