 ...
 */
#ident   "@(#)cfsd_maptbl.c 1.4     98/02/03 SMI"
...
maptbl_setup(cfsd_maptbl_object_t *maptbl_object_p, const char *filename)
{
	...
	strcpy(maptbl_object_p->i_name, filename);
	...
	xx = stat(maptbl_object_p->i_name, &sinfo);
	if (xx) {
		xx = errno;
		dbug_print(("error",
	...
	/* open the file */
	dbug_assert(maptbl_object_p->i_fid == -1);
	maptbl_object_p->i_fid = open(maptbl_object_p->i_name, O_RDWR);
