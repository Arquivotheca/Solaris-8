 ...
 *
 */
#ident   "@(#)cfsd_kmod.c 1.9     97/11/03 SMI"
/*
...
kmod_setup(cfsd_kmod_object_t *kmod_object_p, const char *path)
{
	...
	if (kmod_object_p->i_fd == -1) {
		...
		xx = 0;
		strcpy(kmod_object_p->i_path, path);
	...
	cred_t *credp, vattr_t *vattrp)
{
	...
	len = sizeof (cred_t) + ((credp->cr_ngroups - 1) * sizeof (gid_t));
...
kmod_getattrname(cfsd_kmod_object_t *kmod_object_p, cfs_fid_t *dirp,
	const char *name, cred_t *credp, vattr_t *vattrp, cfs_fid_t *filep)
{
...
kmod_format_fid(cfsd_kmod_object_t *kmod_object_p, const cfs_fid_t *fidp)
{
	...
	for (index = 0; index < (int)fidp->fid_len; index += sizeof (u_int)) {
		...
		strcat(kmod_object_p->i_fidbuf, format);
	}
}
 ...
 */
void
kmod_print_cred(const cred_t *credp)
{
	char buf[100];
	char format[10];
	int xx;
...
kmod_print_attr(const vattr_t *vp)
{
	...
	if (vp->va_mask & AT_VCODE)
		dbug_print(("ioctl", "  vcode %d", vp->va_vcode));
