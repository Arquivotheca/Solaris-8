 ...
 *
 */
#ident   "@(#)cfsd_subr.c 1.9     97/12/06 SMI"
...
subr_cache_setup(cfsd_all_object_t *all_object_p)
{
	...
	int lockfd;
	DIR *dirp;
	char pathname[MAXPATHLEN + MAXNAMELEN + 1];
	int len;
	...
	for (index = 0; index < all_object_p->i_cachecount; index++) {
		...
		strcpy(pathname, cache_object_p->i_cachedir);
		strcat(pathname, "/");
		len = strlen(pathname);
...
cfsd_sleep(int sec)
{
	...
	if (sec > 0) {
		...
		tval = time(NULL);
		abstime.tv_sec = tval + sec;
		abstime.tv_nsec = 0;

		mutex_lock(&mt);
		cond_timedwait(&cv, &mt, &abstime);
		mutex_unlock(&mt);
