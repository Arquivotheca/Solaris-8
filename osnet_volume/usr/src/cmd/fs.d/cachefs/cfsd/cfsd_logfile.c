 ...
 */
#ident   "@(#)cfsd_logfile.c 1.3     96/03/26 SMI"
	...
	const char *filename, int maxmap)
{
	...
	strcpy(logfile_object_p->i_name, filename);
	...
	xx = stat(logfile_object_p->i_name, &sinfo);
	if (xx) {
		xx = errno;
		if (xx == ENOENT) {
	...
	/* open the file */
	dbug_assert(logfile_object_p->i_fid == -1);
	logfile_object_p->i_fid = open(logfile_object_p->i_name, O_RDWR);
