...
#pragma ident	"@(#)ff.c	1.2	97/04/06 SMI"
/*
...
char	*usage =
	"s5fs Usage\nff [-F s5fs] [generic_options] [-Ilsu] [-p prefix] [-a n] [-m n] [-c n] [-n file] [-i i-node-list] special...\n";
...
long	atime,		/* abs(number) of days from -a option */
	mtime,
	crtime;		/* "         "           "  -c option */
time_t	ntime, today;		/* today as returned by time() */
main(argc,argv)
int argc;
char **argv;
{
...
int super()
{
	...
	if (fstblk<=0 ||lstblk<=0 ||g_ilb<=0 ||fstblk>lstblk ||g_ilb>lstblk) 
		errorx("is not an s5fs file system\n",g_dev);
...
char *s;
{
	...
	fprintf(stderr,"s5fs %s: %s: ", g_cmd, g_dev);
