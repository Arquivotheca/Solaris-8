...
#pragma ident	"@(#)ns_ldap.c	1.1	99/07/07 SMI"
 ...
 *
 * nisMapName: mapname i.e. auto_home, etc.
 * cn: contains the key i.e. the mount point
 * nisMapEntry: contains the mount options and remote mount location
 ...
 *
 * Then this would map to the LDAP attributes as follows:
 *
 ...
 *
 */
#define	_LDAP_ATTR_NISMAPNAME   "nismapname"
#define	_LDAP_ATTR_CN		"cn"
#define	_LDAP_ATTR_NISMAPENTRY  "nismapentry"
#define	_LDAP_ATTR_DESCRIPTION  "description"

#define	CAPCHAR '%'
#define MAXERROR 4000

/*
 * automount LDAP filter definition.
 *
 * nisMapName: auto_master, auto_home, auto_shared, auto_direct, etc.
 * cn: directory to mount i.e. the key or mount point
 */
#define	_LDAP_AUTOMAP_F "(&(objectClass=nisObject)(nisMapName=%s)(cn=%s))"

static const char *automap_attr[] = {
		_LDAP_ATTR_NISMAPNAME,
		_LDAP_ATTR_CN,
		_LDAP_ATTR_NISMAPENTRY,
		_LDAP_ATTR_DESCRIPTION,
		(char *)NULL
};
...
char **stack, char ***stkptr, bool_t *iswildcard)
{
	char *ldap_line = NULL;
	char *lp;
	int ldap_len, len;
	...
	(void) strcpy(ml->linebuf, ldap_line);
...
ldap_match(char *map, char *key, char **ldap_line, int *ldap_len)
{
	 ...
	 * searches are case insensitive.  Note, key = attribute _LDAP_ATTR_CN.
	 */
	...
	sprintf(searchfilter, _LDAP_AUTOMAP_F, map, ldapkey);

	if (trace > 1) {
		trace_prt(1, "ldap_match: searchfilter =[ %s ]\n",
		    searchfilter);
	}

	if (trace > 1)
		trace_prt(1, "  ldap_match: Requesting list for %s\n",
		    searchfilter);

	res = __ns_ldap_list(map, searchfilter, NULL,
	    NULL, NULL, 0, &result, &errp,
	    NULL, NULL);
	...
	for (i = 0; i < entry->attr_count; i++) {
		...
		if (strcmp(attr->attrname, _LDAP_ATTR_NISMAPENTRY) == 0) {
			char *attrval;
...
loadmaster_ldap(char *mapname, char *defopts, char **stack, char ***stkptr)
{
	...
	sprintf(searchfilter, _LDAP_AUTOMAP_F, mapname, "*");

	if (trace > 1)
		trace_prt(1, "  loadmaster_ldap: Requesting list for %s\n",
		    searchfilter);

	if (trace > 1)
		trace_prt(1,
			"loadmaster_ldap: Just about to call __ns_ldap_list\n");

	res = __ns_ldap_list(mapname, searchfilter, automap_attr,
	    NULL, NULL, 0, &result, &errp,
	    mastermap_callback_ldap, (void *) &master_cbdata);
...
char **stack, char ***stkptr)
{
	...
	/* filter gets all the entries for the specified mapname */
	sprintf(searchfilter, _LDAP_AUTOMAP_F, nsmap, "*");

	if (trace > 1) {
		trace_prt(1, "loaddirect_ldap: searchfilter=[ %s ]\n",
			searchfilter);
	}

	res = __ns_ldap_list(nsmap, searchfilter, automap_attr,
	    NULL, NULL, 0, &result, &errp,
...
mastermap_callback_ldap(ns_ldap_entry_t *entry, void *udata)
{
	...
	char cont_temp[256], key_temp[256];
	...
	for (i = 0; i < entry->attr_count; i++) {
		...
		if (trace > 1) {
			trace_prt(1,
			"mastermap_callback_ldap: attr->attrname=[ %s ]\n",
			    attr->attrname);
		}
		if (strcmp(attr->attrname, _LDAP_ATTR_NISMAPENTRY) == 0) {
			strcpy(cont_temp, tounix_str(attr->attrvalue[0]));
			contents = cont_temp;
	...
	"mastermap_callback_ldap: contents=[ %s ], contents_len=[ %d ]\n",
		...
		if (strcmp(attr->attrname, _LDAP_ATTR_CN) == 0) {
			strcpy(key_temp, tounix_str(attr->attrvalue[0]));
			key = key_temp;
			key_len = strlen(key);
			if (trace > 1) {
				trace_prt(1,
	...
	if (strcspn(opts, " 	") == strlen(opts)) {
		...
		/* get the value for the nisMapName attribute */
		for (i = 0; i < entry->attr_count; i++) {
			ns_ldap_attr_t *attr;

			attr = entry->attr_pair[i];
			if (strcmp(attr->attrname, _LDAP_ATTR_NISMAPNAME)
								== 0) {
				map_name = attr->attrvalue[0];
				break;
			}
		}
		pr_msg(
	"Warning: invalid entry for %s in ldap server map %s ignored.\n",
		    dir, map_name);
	}
	if (trace > 1)
...
int *cache_time, char **stack, char ***stkptr)
{
	...
	*error = 0;
	...
	if (trace > 1)
		trace_prt(1, "  getmapkeys_ldap: Requesting list for %s\n",
		    searchfilter);
