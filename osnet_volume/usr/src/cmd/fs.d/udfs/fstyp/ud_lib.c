...
#pragma ident	"@(#)ud_lib.c	1.9	99/05/28 SMI"
...
int32_t ud_get_ecma_ver(int32_t);
int32_t ud_get_fs_bsize(int32_t, uint32_t *);
...
ud_fill_udfs_info(int32_t fd)
{
	...
	udfs.ecma_version = ud_get_ecma_ver(fd);
	if (udfs.ecma_version == UD_ECMA_UNKN) {
		return (1);
	}

	udfs.lbsize = ud_get_fs_bsize(fd, &udfs.avdp_loc);
	if (udfs.lbsize == 0) {
		return (2);
	}
...
ud_get_ecma_ver(int32_t fd)
{
	 ...
	 */
	for (off = UD_VOL_REC_START; off < UD_VOL_REC_END;
			off += UD_VOL_REC_BSZ) {
...
int32_t
ud_get_fs_bsize(int32_t fd, uint32_t *avd_loc)
{
	uint64_t off;
...
print_iuvd(struct iuvd_desc *iuvd)
{
	...
	print_regid("iid ", &iuvd->iuvd_iid, REG_UDF_ID);
}
