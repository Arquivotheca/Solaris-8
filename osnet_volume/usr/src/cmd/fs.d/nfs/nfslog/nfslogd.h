...
#pragma ident	"@(#)nfslogd.h	1.35	99/05/25 SMI"
...
struct nfsl_log_file {
	...
	int	bufoffset;	/* current offset in buffer */
...
struct nfslog_buf {
	...
	intptr_t next_rec;			/* address of next record */
	...
	unsigned int last_record_offset;	/* value last written to hdr */
...
struct nfslog_lr {
	...
	unsigned int f_offset;			/* offset for ondisk file */
