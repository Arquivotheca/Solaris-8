...
#pragma ident	"@(#)dbtab.c	1.36	99/09/03 SMI"
	...
	int datasize, char *str)
{
	...
	if (dbm_store(dbp->db, key, data, DBM_REPLACE) < 0) {
		...
		if (debug) {
			debug_print_key(stderr, str, "dbm_store:\n    ",
				key.dptr, key.dsize);
			(void) fprintf(stderr, "Error %s\n", strerror(error));
		} else {
			syslog(LOG_ERR, gettext(
				"dbm_store failed: Error %s"),
				strerror(error));
		}
		if (error == 0)	/* should not happen but sometimes does */
			error = -1;
		return (error);
	}
	return (0);
	...
	int *errorp, char *str)
{
	...
	if (data.dptr == NULL) {
		...
		*errorp = dbm_error(dbp->db);
		if (debug > 3) {
			debug_print_key(stderr, str, "dbm_fetch:\n    ",
				key.dptr, key.dsize);
			(void) fprintf(stderr, "Error %s\n", strerror(*errorp));
		}
		if (*errorp == 0)   /* should not happen but sometimes does */
			*errorp = -1;
		return (NULL);
	}
	...
	if (debug > 3) {
		debug_print_key_and_data(stdout, str, "dbm_fetch:\n    ",
			key.dptr, key.dsize, dataaddr, data.dsize);
	}
	*errorp = 0;
	return (dataaddr);
}
...
delete_record(struct db_list *dbp, void *keyaddr, int keysize, char *str)
{
	...
	key.dsize = keysize;
	if (debug > 2) {
		debug_print_key(stdout, str, "dbm_delete:\n    ",
			key.dptr, key.dsize);
	}
	if (dbm_delete(dbp->db, key) < 0) {
		error = dbm_error(dbp->db);
		if (debug) {
			debug_print_key(stderr, str, "dbm_delete:\n    ",
				key.dptr, key.dsize);
			if (error == 0)	/* returned if rec not in db */
				(void) fprintf(stderr, "not in database\n");
			else
				(void) fprintf(stderr, "Error %s\n",
						strerror(error));
		} else if (error != 0) {
			syslog(LOG_ERR, gettext(
				"dbm_delete failed: Error %s"),
				strerror(error));
		}
	}
	return (error);
