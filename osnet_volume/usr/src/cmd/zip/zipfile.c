...
local int zqcmp OF((ZCONST zvoid *, ZCONST zvoid *));
   ...
   local int readzipfile2 OF((void));
...
# else
  q = LastDir(t);
  if(strrchr(q, '_') == NULL && strrchr(q, '.') == NULL)
  {
      strcat(t, "_zip");
  }
# endif /* QDOS */
...
/*
 * readzipfile2 is called with zip -F or zip -FF
...
/*
 * readzipfile starts searching for the End Signature at the end of the file
   ...
   destroy() or deletedir() is ignored. */
{
  ...
  for (z = zfiles; z != NULL; z = z->nxt)
    ...
    {
      z->mark = 1;
      if (z->iname[z->nam - 1] != '/') { /* don't unlink directory */
        if (verbose)
  ...
  if (n)
  {
    ...
    n = 0;
    for (z = zfiles; z != NULL; z = z->nxt) {
      if (z->mark && z->nam > 0 && z->iname[z->nam - 1] == '/'
