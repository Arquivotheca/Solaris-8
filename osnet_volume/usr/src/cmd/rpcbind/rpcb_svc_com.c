...
#ident	"@(#)rpcb_svc_com.c	1.30	99/07/18 SMI"
	...
	ulong_t verstype;
{
	...
	if (uaddr && uaddr[0])
		free((void *) uaddr);
	/*
	 * It's wrong. Same mistake as the getaddr_list.
	 * Also it's not consistent with getaddr_list.
	...
	if (fnd && ((verstype == RPCB_ALLVERS) ||
		...
		if (!(uaddr = mergeaddr(transp, regp->r_netid,
	...
	SVCXPRT *xprt;
{
	...
	fi = forward_find((u_long) reply_msg.rm_xid, uaddr);
	if (fi == NULL)
		goto done;
#ifdef	SVC_RUN_DEBUG
	if (debugging) {
	...
	if (debugging) {
		...
		(void) free_slot_by_xid((u_long) reply_msg.rm_xid, fi->uaddr);
}
