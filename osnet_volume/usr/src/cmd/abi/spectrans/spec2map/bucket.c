...
#pragma ident	"@(#)bucket.c	1.1	99/01/25 SMI"
...
add_by_name(const char *version, const Interface *interface)
{
	...
	/* Add to bucket */
	(void) snprintf(buffer, sizeof (buffer), "%s %s", interface->IF_name,
		(interface->IF_type == FUNCTION) ? "= FUNCTION" : "");

	b->b_interface_table = add_to_stringtable(b->b_interface_table,
		buffer); /* Assign in case of realloc. */
	errlog(END, "}");
	return (0);
}
...
sort_buckets(void)
{
	...
	for (l = first_list(); l != NULL; l = next_list()) {
		for (b = first_from_list(l); b != NULL; b = next_from_list()) {
			sort_stringtable(b->b_interface_table);
			if (b->b_uncles) {

				if (b->b_uncles->bl_bucket) {
		sort_stringtable(b->b_uncles->bl_bucket->b_interface_table);
				}
			}
		}
	}
...
print_bucket(const bucket_t *b)
{
	...
	errlog(TRACING, "    int        b_weak = %d;",
		b->b_weak);
	errlog(TRACING, "    table_t  *b_interface_table = 0x%p;",
		(void *)b->b_interface_table);
	errlog(TRACING, "}");
}
...
print_all_buckets(void)
{
	...
	for (i = 0, l = first_list(); l != NULL; l = next_list(), ++i) {
		...
		for (j = 0, b = first_from_list(l);
			...
			print_bucket(b);
			errlog(TRACING, "interfaces = {");
			print_stringtable(b->b_interface_table);
			errlog(TRACING, "}");
			...
			if (b->b_uncles) {
				...
				errlog(TRACING, "interfaces = {");
				print_stringtable(
				    b->b_uncles->bl_bucket->b_interface_table);
				errlog(TRACING, "}");
			}
		}
	}
...
new_bucket(const char *name, int weak)
{
	...
	b->b_uncles = NULL;
	b->b_interface_table = create_stringtable(TABLE_INITIAL);
	b->b_weak = weak;
	return (b);
}
...
print_iface(const Interface * p)
{
	...
	errlog(TRACING, "%s (%s, %s, %s)", p->IF_name,
