...
#pragma ident	"@(#)frontend.c	1.2	99/02/26 SMI"
...
#ifdef NO_READLINE
	while (fgets(buf, BUFSIZ, spec_fp)) {
		meta_info.mi_line_number++;
		Curlineno = meta_info.mi_line_number;
		meta_info.mi_nlines = 1;
	...
	while (meta_info.mi_nlines = readline(&buf2, spec_fp)) {
		...
		case XLATOR_KW_DATA:	 /* Data keyword */
			...
			case XLATOR_SUCCESS: /* OK */
				start_if_fail = 0;
				extends_err = check4extends(filename, value,
				    T_info->ti_archtoken, spec_fp);
				switch (extends_err) {
				case -1:	/* Error */
 ...
 *         0 if outside the known set {i386, sparc, sparcv9, ia64}.
 */
int
