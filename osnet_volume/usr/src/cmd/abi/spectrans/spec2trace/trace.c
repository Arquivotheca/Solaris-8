...
#pragma ident	"@(#)trace.c	1.4	99/05/14 SMI"
...
{
	char ifname[BUFSIZ];
	seterrline(LINE, m.mi_filename, Keywords[token-FIRST_TOKEN].key, value);
 ...
 */
	...
	(void) strpqcpy(ifname, value, nextsep(value));
...
collect_prototype(char *p, int line, int extcnt)
{
	...
	if (type_IsVarargs(decl_GetType(pp))) {
		symtab_set_skip(YES);
		decl_Destroy(pp);
		return (SUCCESS_RC);
	}
