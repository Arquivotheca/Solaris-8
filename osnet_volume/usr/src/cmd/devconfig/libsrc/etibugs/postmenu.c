/*	Copyright (c) 1984, 1986, 1987, 1988 AT&T	*/
/*	  All Rights Reserved  	*/

/*	THIS IS UNPUBLISHED PROPRIETARY SOURCE CODE OF AT&T	*/
/*	The copyright notice above does not evidence any   	*/
/*	actual or intended publication of such source code.	*/

#include "private.h"

void
_post_item (m, k)
MENU *m;
register ITEM *k;
{
  int foreon = FALSE;
  int backon = FALSE;
  int greyon = FALSE;
  chtype c;
  register int i;

  /* Display the mark region of the item */

  if (!Selectable(k)) {
    wattron (Win(m), Grey(m));
    greyon = TRUE;
    for (i=Marklen(m); i>0; i--) {
	  waddch (Win(m), (chtype)' ');
    }
  }
  else {

/* highlight only the current item, and not all the selected items */

#ifdef WAS
    if (Value(k) || k==Current(m)) {
#else
	if (k==Current(m)) {
#endif
      wattron (Win(m), Fore(m)); 
      foreon = TRUE;
    }
    else {
      wattron (Win(m), Back(m));
      backon = TRUE;
    }

    /* Display the mark */
    if (Value(k) || (OneValue(m) && k==Current(m))) {
      if (Marklen(m)) {
	waddstr (Win(m), Mark(m));
      }
    }
    else {
      for (i=Marklen(m); i>0; i--) {
	waddch (Win(m), (chtype)' ');
      }
    }
  }

  /* Display the name */

  waddnstr (Win(m), Name(k), MaxName(m));
  if (ShowDesc(m) && MaxDesc(m) != 0) {
    c = Pad(m);
  }
  else {
    c = ' ';
  }
  for (i=MaxName(m)-NameLen(k); i>0; i--) {
	waddch (Win(m), (chtype)c);
  }

  /* Display the description */

  if (ShowDesc(m) && MaxDesc(m) != 0) {
	waddch (Win(m), (chtype)Pad(m));
    if (DescriptionLen(k) != 0) {
      waddstr (Win(m), Description(k));
    }
    for (i=MaxDesc(m)-DescriptionLen(k); i>0; i--) {
	  waddch (Win(m), (chtype)' ');
    }
  }
  if (foreon) {
    wattroff (Win(m), Fore(m)); 
  }
  if (backon) {
    wattroff (Win(m), Back(m)); 
  }
  if (greyon) {
    wattroff (Win(m), Grey(m)); 
  }
}

void
_move_post_item (m, k)
register MENU *m;
register ITEM *k;
{
  wmove (Win(m), Y(k), X(k)*(Itemlen(m)+1));
  _post_item (m, k);
}

int
unpost_menu (m)
register MENU *m;
{
  if (!m) {
    return E_BAD_ARGUMENT;
  }
  if (Indriver(m)) {
    return E_BAD_STATE;
  }
  if (!Posted(m)) {
    return E_NOT_POSTED;
  }
  Iterm(m);
  Mterm(m);
  werase (US(m));
  wsyncup (US(m));
  delwin (Sub(m));
  Sub(m) = (WINDOW *)0;
  delwin (Win(m));
  Win(m) = (WINDOW *)0;
  ResetPost(m);
  return E_OK;
}

/* This routine draws the item indicated by oldcur first and then */
/* draws the item indicated by Current.  This will have the affect */
/* of unselecting the first item and selecting the next. */
void
_movecurrent (m, oldcur)
register MENU *m;
register ITEM *oldcur;
{
  if (oldcur != Current(m)) {
    _move_post_item (m, oldcur);
    _move_post_item (m, Current(m));
  }
}

/* Draw the entire menu into the super window */
/* This routine assumes all items have been linked and */
/* that the menu is in at least a pre-post state. */

void
_draw (m)
register MENU *m;
{
  register int k;
  register ITEM *i, *j;
  register ITEM *si, *sj;

  k = 0;		/* Line number */
  i = IthItem(m, 0);
  si = Cyclic(m) ? i : (ITEM *)0;
  do {
    wmove (Win(m), k++, 0);
    j = i;
    sj = Cyclic(m) ? j : (ITEM *)0;
    do {
      _post_item (m, j);
      if ((j = Right(j)) != sj) {
	waddch (Win(m), (chtype)' ');
      }
    } while (j != sj);
  } while ((i = Down(i)) != si);
}

int 
post_menu (m)
register MENU *m;
{
  register ITEM **ip;
  int r, c;		/* visible # of rows and cols */

  if (!m) {
    return E_BAD_ARGUMENT;
  }
  if (Indriver(m)) {
    return E_BAD_STATE;
  }
  if (Posted(m)) {
    return E_POSTED;
  }
  /* Make sure there is at least one item present */
  if (Items(m) && IthItem(m, 0)) {
    getmaxyx (US(m), r, c);
    /* Make sure the menu fits into the window horizontally */
    if (c < Width(m) || r < Height(m)) {
      return E_NO_ROOM;
    }

    /* Create the menu window and derived windows */

    if ((Win(m) = newwin (Rows(m), Width(m), 0, 0)) == (WINDOW *)0) {
      return E_SYSTEM_ERROR;
    }
    /* Take the minimum of the height of the menu (Height), the physical
       height of the window (r), and the number of rows in the menu (Rows). */
    r = min (min (r, Rows(m)), Height(m));
    if ((Sub(m) = derwin (Win(m), r, Width(m), 0, 0)) == (WINDOW *)0) {
      return E_SYSTEM_ERROR;
    }

    /* If needed, link all items in the menu */

    if (LinkNeeded(m)) {
      _link (m);
    }

    SetPost(m);

    /* If only one value can be set then unset all values to start. */
    if (OneValue(m)) {
      for (ip=Items(m); *ip; ip++) {
	Value(*ip) = FALSE;
      }
    }

    /* Go do the drawing of the menu */
    _draw (m);

    Minit(m);
    Iinit(m);
    _show (m);		/* Display the menu */
    return E_OK;
  }
  return E_NOT_CONNECTED;
}
