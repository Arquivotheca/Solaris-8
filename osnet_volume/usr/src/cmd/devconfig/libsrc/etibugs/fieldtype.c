...
char * arg;
{
	...
	*/
	...
	if (Status (t, LINKED))
	{
		/* don't dereference null arg pointer */
		/* and return immediately if first check fails! */
		
		if(arg)
		{
			if(!_checkfield (TypeL (t), f, ArgL (arg)))
				return(FALSE);
			else
				return(_checkfield (TypeR (t), f, ArgR (arg)));
		}
		else  /* arg is NULL - don't dereference! */
		{
				if(!_checkfield (TypeL (t), f, arg))
				return(FALSE);
			else
				return(_checkfield (TypeR (t), f, arg));
...
char * arg;
{
	...
	*/
	...
	if (Status (t, LINKED))
	{
		...
		if(arg)
		{
			if(!_checkchar (TypeL (t), c, ArgL (arg)))
				return(FALSE);
			else
				return(_checkchar (TypeR (t), c, ArgR (arg)));
		}
		else  /* arg is NULL - don't dereference! */
		{
			if(!_checkchar (TypeL (t), c, arg))
				return(FALSE);
			else
				return(_checkchar (TypeR (t), c, arg));
		}
	} 
	else   /* not linked */
	{
