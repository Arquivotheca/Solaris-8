...
#ident	"@(#)volmissing.c	1.12	95/05/18 SMI"
...
#define	VOLMISSING_POPUP_PATH	"/usr/lib/vold/volmissing_popup"
...
mail_msg(char *to)
{
	...
	(void) sprintf(namebuf, "/bin/mail %s", to);
	if ((fp = popen(namebuf, "w")) == NULL) {
		perror(namebuf);
		return;
	}
