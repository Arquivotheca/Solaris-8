...
#pragma ident	"@(#)vold_node.c	1.50	98/12/11 SMI"
...
node_fattr(vvnode_t *vn, struct fattr *fat)
{
	...
	case VV_DIR:
		fat->type = NFDIR;
		fat->mode = (u_int) vn->vn_obj->o_mode;
		fat->size = 512;
		break;
	...
	case VV_CHR:
		...
		fat->mode = (u_int) node_mapmode(vn->vn_vol);
		...
		fat->rdev = (u_int) vn->vn_vol->v_devmap[vn->vn_num].dm_voldev;
		ASSERT(fat->rdev != 0);
	...
	case VV_PART:
		...
		fat->mode = (u_int) node_mapmode(vn->vn_parent->vn_vol);
	...
	fat->uid = (u_int) vn->vn_obj->o_uid;
	fat->gid = (u_int) vn->vn_obj->o_gid;
...
node_fhtovn(nfs_fh *fh)
{
	...
	hash_line = (u_int) (ifh->fh_id % FH_HASH_SIZE);
