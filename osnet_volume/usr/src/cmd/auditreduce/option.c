...
#pragma ident	"@(#)option.c	1.18	99/12/06 SMI"
...
char	**argv;
{
	...
	static char	*options = "ACD:M:NQR:S:VO:a:b:c:d:e:g:j:m:o:r:u:";
...
char	*optarg;
{
	...
	case OBJ_SOCK:
		...
		he = getipnodebyname((const void *)obj_val, AF_INET6, 0,
			&err);
		if (he == 0) {
			he = getipnodebyname((const void *)obj_val,
				AF_INET, 0, &err);
			if (he == 0) {
				(void) sprintf(errbuf,
...
char	*dir;
{
	...
	(void) sprintf(dname, "%s/files", dir);
...
int	do_check;
{
	 ...
	 */
	namep = (char *)a_calloc(1, strlen(slp));
