...
#pragma ident	"@(#)main.c	1.12	99/07/30 SMI"
...
init_sig()
{
	...
	if (signal(SIGPOLL, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGPOLL signal failed"));
		return (-1);
	}
	...
	if (signal(SIGWAITING, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGWAITING signal failed"));
		return (-1);
	}
	if (signal(SIGLWP, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGLWP signal failed"));
		return (-1);
	}
	if (signal(SIGFREEZE, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGFREEZE signal failed"));
		return (-1);
	}
	if (signal(SIGTHAW, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGTHAW signal failed"));
		return (-1);
	}
	if (signal(SIGCANCEL, int_handler) == SIG_ERR) {
		perror(gettext("auditreduce: SIGCANCEL signal failed"));
		return (-1);
	}
