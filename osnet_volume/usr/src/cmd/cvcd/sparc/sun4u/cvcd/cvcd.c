...
#pragma	ident	"@(#)cvcd.c	1.26	99/03/05 SMI"
...
main(int argc, char **argv)
{
	...
	/*
	 * Set the SO_REUSEADDR and SO_DONTROUTE options for this TLI endpoint.
	 */
...
cvcd_setopt(int fd, int name)
{
	...
	tropt = (struct t_optmgmt *)t_alloc(fd, T_OPTMGMT, T_OPT);
	if (tropt == NULL) {
		cvcd_err(LOG_ERR, "t_alloc: %s", t_errlist[t_errno]);
		exit(1);
	}
