...
#ident  "@(#)passmgmt.c 1.20     99/08/25 SMI"        /* SVr4.0 1.23 */
...
#define AUTH_MASK	010000
#define PROF_MASK	020000
#define ROLE_MASK	040000
#define TYPE_MASK	0100000
#define UA_KEYS		4
...
kvopts_t ua_opts[UA_KEYS];	/* mapping of extensible keywords and options*/
...
static void
assign_attr(userattr_t *user, char *newkey, char *val) {
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	ua_opts[0].mask = AUTH_MASK;
	ua_opts[1].mask = PROF_MASK;
	ua_opts[2].mask = ROLE_MASK;
	ua_opts[3].mask = TYPE_MASK;
	...
	ua_opts[0].key = USERATTR_AUTHS_KW;
	ua_opts[1].key = USERATTR_PROFILES_KW;
	ua_opts[2].key = USERATTR_ROLES_KW;
	ua_opts[3].key = USERATTR_TYPE_KW;
	
	/* parse the command line */

	while ((c = getopt(argc, argv, "ml:c:h:u:g:s:f:e:k:A:P:R:T:oad")) != -1) {
	...
	if (optind == argc || argc > (optind+1) ||
		...
		E_MASK|AUTH_MASK|PROF_MASK|ROLE_MASK|TYPE_MASK))))
	...
	if ((D_MASK|L_MASK|AUTH_MASK|ROLE_MASK|PROF_MASK|TYPE_MASK) & optn_mask)
	...
	if (info_mask & UATTR_FILE) {
		...
		if ((fp_uatemp = fopen(USERATTR_TEMP, "w")) == NULL) {
			rid_tmpf();
			file_error();
		}

		if (chown(USERATTR_TEMP, statbuf.st_uid, statbuf.st_gid) != NULL) {
			rid_tmpf();
			file_error();
		}
	}
	...
	if (info_mask & UATTR_FILE) {
		...
		while (!end_of_file) {
			...
			if (strcmp(lognamp, ua_ptr1p->name) == 0) {
				...
				if (optn_mask & M_MASK) {
				    ...
				    ua_ptr1p->name = userattr_st.name;
				    for (j = 0; j < UA_KEYS; j++){
					if (ua_opts[j].mask & optn_mask){
					    value = 
						kva_match(userattr_st.attr,
						    ua_opts[j].key);
					    assign_attr(ua_ptr1p,
						    ua_opts[j].key,
						    value);
					}
				    }
		...
		/*
		 * Add entry in user_attr if masks is one of -
		 *   AUTH_MASK|ROLE_MASK|PROF_MASK|TYPE_MASK
