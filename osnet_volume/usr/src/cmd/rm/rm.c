...
/*	Portions Copyright (c) 1988,1996,1997 by Sun Microsystems, Inc. */
/*	All Rights Reserved. 					*/

#ident	"@(#)rm.c	1.28	97/05/08 SMI"	/* SVr4.0 1.19	*/
/*
 * rm [-fiRr] file ...
 */
...
mypath(dev_t dev, ino_t ino)
{
	 ...
	 */
	if (parent_err) {
		(void) fprintf(stderr, gettext("rm: cannot determine if this "
		    "is an ancestor of the current working directory\n%s\n"),
		    fullpath);
