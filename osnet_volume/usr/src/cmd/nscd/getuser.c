...
#pragma ident	"@(#)getuser.c	1.1	99/04/07 SMI"
...
static getuser_namekeepalive(int keep, int interval);
static int update_user_bucket(nsc_bucket_t **old, nsc_bucket_t *new,
    int callnumber);
...
getuser_lookup(nsc_return_t *out, int maxsize, nsc_call_t * in, time_t now)
{
	...
	while (1) {
		...
		logit("getuser_lookup: Name too long from pid %d uid %d\n",
...
getout:
	mutex_unlock(&db_lock);
	if ((current_admin.user.nsc_secure_mode != 0) &&
