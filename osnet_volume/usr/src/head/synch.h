...
#pragma ident	"@(#)synch.h	1.46	99/11/15 SMI"
 ...
 * of the correponding structure of pthread_rwlock_t in sys/types.h. See NOTE
 * above for mutex_t/cond_t.
 */
typedef struct _rwlock {
	int32_t		readers;	/* -1 == writer else # of readers */
	uint16_t	type;
	uint16_t	magic;
	uint16_t	pad1[4];	/* internal consistency mutex */
	upad64_t	pad10[2];	/* internal consistency mutex */
	uint16_t	pad2[4];	/* really cond_t for readers */
	upad64_t	pad20;		/* really cond_t for readers */
	uint16_t	pad3[4];	/* really cond_t for writers */
	upad64_t	pad30;		/* really cond_t for writers */
} rwlock_t;
 ...
 */
#define	DEFAULTMUTEX	{{0, 0, 0, {USYNC_THREAD}, MUTEX_MAGIC}, \
				{{{0, 0, 0, 0, 0, 0, 0, 0}}}, 0}
#define	SHAREDMUTEX	{{0, 0, 0, {USYNC_PROCESS}, MUTEX_MAGIC}, \
				{{{0, 0, 0, 0, 0, 0, 0, 0}}}, 0}
...
#define	DEFAULTRWLOCK	{0, USYNC_THREAD, RWL_MAGIC, \
				{0, 0, USYNC_THREAD, MUTEX_MAGIC}, { 0, 0}, \
				{0, 0, USYNC_THREAD, MUTEX_MAGIC}, 0, \
				{0, 0, USYNC_THREAD, MUTEX_MAGIC}, 0}
#define	SHAREDRWLOCK	{0, USYNC_PROCESS, RWL_MAGIC, \
				{0, 0, USYNC_PROCESS, MUTEX_MAGIC}, { 0, 0}, \
				{0, 0, USYNC_PROCESS, MUTEX_MAGIC}, 0, \
				{0, 0, USYNC_PROCESS, MUTEX_MAGIC}, 0}
