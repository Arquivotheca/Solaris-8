...
#pragma ident	"@(#)invlib.c	1.1	99/01/11 SMI"
...
invopen(INVCONTROL *invcntl, char *invname, char *invpost, int stat)
{
	...
	if (stat > invcntl->param.filestat) {
		...
		(void) fwrite((char *)&invcntl->param,
		    sizeof (invcntl->param), 1, invcntl->invfile);
	}
	return (1);
}
...
invstep(INVCONTROL *invcntl)
{
	...
	(void) fseek(invcntl->invfile,
	    ...
	    0);
	(void) fread(invcntl->logblk, (int)invcntl->param.sizeblk, 1,
	    invcntl->invfile);
	invcntl->keypnt = 0;
}
...
invdump(INVCONTROL *invcntl, char *term)
{
	...
	if (*term == '-') {
		...
		invcntl->numblk = j;
		(void) fseek(invcntl->invfile,
		    (j * invcntl->param.sizeblk) + invcntl->param.cntlsize, 0);
		(void) fread(invcntl->logblk, (int)invcntl->param.sizeblk, 1,
		    invcntl->invfile);
	} else
	...
	case OR:
		...
		for (set1c = 0, set2c = 0;
			...
			if (set1p->lineoffset < posting.lineoffset) {
				...
				*newsetp++ = posting;
				(void) fread((char *)&posting,
				    (int)sizeof (posting), 1, file);
				set2c++;
