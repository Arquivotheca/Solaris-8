...
#pragma ident	"@(#)smcp.c	1.9	99/08/11 SMI"
...
SMCG_attach(dev_info_t *devinfo, ddi_attach_cmd_t cmd)
{
	...
	if ((pAd = (Adapter_Struc *)kmem_zalloc(
	    sizeof (Adapter_Struc), KM_NOSLEEP)) == NULL) {
		gld_mac_free(macinfo);
		return (DDI_FAILURE);
	}
	if ((smcg = (smcg_t *)kmem_zalloc(
	    sizeof (smcg_t), KM_NOSLEEP)) == NULL) {
		gld_mac_free(macinfo);
		kmem_free((caddr_t)pAd, sizeof (Adapter_Struc));
		return (DDI_FAILURE);
	}
...
attach_fail_cleanup:
	kmem_free((caddr_t)pAd, sizeof (Adapter_Struc));
	kmem_free((caddr_t)smcg, sizeof (smcg_t));
	gld_mac_free(macinfo);
	return (DDI_FAILURE);
}
...
SMCG_set_mac_addr(gld_mac_info_t *macinfo, unsigned char *macaddr)
{
	...
	mutex_enter(&smcg->lm_lock);
	bcopy((caddr_t)macaddr, (caddr_t)pAd->node_address, ETHERADDRL);
...
{
	register smcg_t		*smcg = (smcg_t *)macinfo->gldm_private;
	Adapter_Struc		*pAd = smcg->smcg_pAd;
	...
	if (SMCG_debug & SMCGTRACE)
		...
		while (1) {
			...
			dbuf.fragment_list[j].fragment_ptr =
			    (unsigned char *)cookie.dmac_address;
			j++;
			if (--ncookies == 0)
				break;
			ddi_dma_nextcookie(
	...
	Adapter_Struc *pAd, int status, Data_Buff_Structure **pDataBuff)
{
	...
	if (((bdesc = smcg->rx_freelist) != NULL) && !smcg->detaching_flag) {
		...
		bcopy(smcg->bdesc[smcg->rx_ring_index]->buf,
		    (caddr_t)mp->b_wptr, length);
	}
...
UM_PCI_Services(Adapter_Struc *pAd, union REGS *pregs)
{
	...
	switch (func) {
		...
		case PCI_WRITE_CONFIG_BYTE:
			regnum = (unsigned long) pregs->h.di;
			pci_config_putb((ddi_acc_handle_t)pAd->pcihandle,
			    regnum, pregs->h.cl);
			pregs->x.cflag = 0;
			pregs->h.ah = PCI_SUCCESSFUL;
			break;
