...
#pragma ident	"@(#)msm.c	1.21	99/05/20 SMI"
...
static u_int msmintr(caddr_t arg);
...
msmattach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	ddi_set_driver_private(dip, (caddr_t)msmptr);

	if (ddi_add_intr(dip, (u_int)0, &msmptr->iblock,
    ...
    struct cred *cred_p)
{
	...
	if (!(msmptr = (struct strmseinfo *)ddi_get_driver_private(dip)))
