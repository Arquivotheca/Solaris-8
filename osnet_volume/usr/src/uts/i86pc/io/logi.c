...
#pragma ident	"@(#)logi.c	1.18	99/05/20 SMI"
...
logiattach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		...
		ddi_set_driver_private(dip, (caddr_t)logiptr);


		if (ddi_add_intr(dip, (u_int)0, &logiptr->iblock,
	...
	case DDI_DETACH:
		...
		for (instance = 0; instance < LOGI_MAXUNIT; instance++) {
			if (((ldevi = logiunits[instance]) == NULL) ||
			    !(logiptr = (struct strmseinfo *)
			    ddi_get_driver_private(ldevi))) {
				continue;
			}
		}
 ...
 */
		...
		if ((instance >= LOGI_MAXUNIT) ||
		    !(logiptr =
		    (struct strmseinfo *)ddi_get_driver_private(dip)))
			return (DDI_FAILURE);
...
logiopen(queue_t *q, dev_t *devp, int flag, int sflag, cred_t *cred_p)
{
	...
	if (!mse_config.present)
		return (EIO);
	if (!(logiptr = (struct strmseinfo *)ddi_get_driver_private(dip)))
		return (EIO);
