...
#pragma ident	"@(#)glmvar.h	1.8	99/04/19 SMI"
...
#define	STATIC_SGL_SIZE	17	/* handle up to 64K without malloc */


typedef struct	glm_scsi_cmd {
	uint32_t		cmd_flags;	/* flags from scsi_init_pkt */
	ddi_dma_handle_t	cmd_dmahandle;	/* dma handle */
	ddi_dma_cookie_t	cmd_cookie;
	uint_t			cmd_nwin;	/* number of windows */
	uint_t			cmd_cwin;	/* current window number */
	uint_t			cmd_wcc;	/* window cookie count */
	uint_t			cmd_woff;	/* cookie offset in window */
	off_t			cmd_offset;	/* byte offset for window */
	size_t			cmd_length;	/* window size in bytes */
	uint_t			cmd_scc;	/* scripts cookie count */
	uchar_t			cmd_saved_cookie; /* save/restore cmd_scc */
	int			cmd_pkt_flags;
	struct buf		*cmd_bp;
	...
	glmti_t			cmd_sgl[STATIC_SGL_SIZE];
...
#ifdef i86pc
	int		nt_total_sectors;  /* total # of sectors on device */
 ...
 */
#define	HBAOFFSET(label) ((long)&(((struct glm_hba_dsa *)0)->label))
...
#define	NSS_FUNCS	8	/* number of defined SCRIPT functions */
	...
	ulong_t		g_dsa_addr;	/* addr of table indirects */
	...
	/*
	 * These ushort_ts are bit maps for targets
	 */
	...
	uchar_t		g_arq_mask[NTARGETS_WIDE];
...
#define	GLM_QUIESCE_TIMEOUT	1	/* 1 second */
