...
#pragma ident	"@(#)ac_test.c	1.11	99/06/30 SMI"
...
ac_mapin(uint64_t pa, caddr_t va)
{
	...
	ac_trace(0xc1, (uint_t)va, pa, tte.ll);
...
ac_mem_test_start(ac_cfga_pkt_t *pkt, int flag)
{
	...
	test->info.line_size = ecache_linesize;
...
ac_mem_test_read(ac_cfga_pkt_t *pkt, int flag)
{
	...
	case DDI_MODEL_ILP32: {
		...
		t_read.page_buf = (void *)t_read32.page_buf;
		t_read.address = t_read32.address;
		t_read.error_buf =
		    (sunfire_processor_error_regs_t *)t_read32.error_buf;
		break;
	}
	case DDI_MODEL_NONE:
	...
	ac_trace(0xc3, page_offset, page_pa, (u_longlong_t)dst_va);
	...
	ac_trace(0xc5, (uint_t)src_va,
	    (u_longlong_t)t_read.address.line_count,
	    (u_longlong_t)test->info.line_size);
	...
	ac_trace(0xc7, 0, error_buf.afsr, error_buf.afar);
...
ac_mem_test_write(ac_cfga_pkt_t *pkt, int flag)
{
	...
	case DDI_MODEL_ILP32: {
		...
		t_write.page_buf = (void *)t_write32.page_buf;
	...
	ac_trace(0xc9, page_offset, page_pa, (u_longlong_t)src_va);
	...
	ac_trace(0xcb, (uint_t)dst_va,
	    (u_longlong_t)t_write.address.line_count,
	    (u_longlong_t)test->info.line_size);
