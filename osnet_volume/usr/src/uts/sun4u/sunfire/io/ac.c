...
#pragma ident	"@(#)ac.c	1.29	99/09/26 SMI"
...
void ac_mem_test_stop_on_close(u_int, u_int);
/*
...
ac_pkt_init(ac_cfga_pkt_t *pkt, intptr_t arg, int flag)
{
	...
	if (ddi_model_convert_from(flag & FMODELS) == DDI_MODEL_ILP32) {
		...
		pkt->cmd_cfga.outputstr =
			(char *)ac_cmd32.outputstr;
		pkt->cmd_cfga.private =
			(void *)ac_cmd32.private;
	} else
...
ac_timeout(void *arg)
{
	...
	while (board != NULL) {
		...
		*softsp->ac_counter = 0x80000000LL - 0x9ac4 -
		    (softsp->board << 3);
	 ...
	 */
	cmn_err(CE_WARN, "This machine is now in hot-plug mode.");
