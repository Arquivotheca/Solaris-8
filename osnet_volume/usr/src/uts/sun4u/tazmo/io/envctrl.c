...
#pragma ident	"@(#)envctrl.c	1.50	99/07/29 SMI"
...
extern prom_getprop();
extern prom_getproplen();
extern	void	prom_printf(const char *fmt, ...);
extern void (*abort_seq_handler)();
extern void ddi_putb(ddi_acc_handle_t, uint8_t *, uint8_t);
...
static struct modldrv envctrlmodldrv = {
	...
	"I2C ENVCTRL_driver: 1.50 99/07/29",
	&envctrl_ops,
};
...
envctrl_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	pshotplug_timeout_hz = drv_usectohz(BLINK_TIMEOUT_USEC * 2);
	...
	status = ddi_prop_lookup_byte_array(DDI_DEV_T_ANY, dip,
		    DDI_PROP_DONTPASS, "cpu-fan-speeds", &creg_prop, &len);
	...
	status = ddi_prop_lookup_byte_array(DDI_DEV_T_ANY, dip,
		    DDI_PROP_DONTPASS, "ps-fan-speeds", &creg_prop, &len);
...
envctrl_init_bus(struct envctrlunit *unitp)
{
	...
	ENVCTRL_BUS_INIT0);
	(void) ddi_putb(unitp->ctlr_handle, &unitp->bus_ctl_regs->s0,
	ENVCTRL_BUS_INIT1);
...
retry0:
		...
		if (status != DDI_SUCCESS) {
			...
			if (retrys < envctrl_max_retries) {
				...
				mutex_enter(&unitp->umutex);
				cmn_err(CE_WARN,
				"envctrl_xmit: Write to TDA8444 failed\n");
				return (DDI_FAILURE);
			}
		}
...
retry:
		...
		if (status != DDI_SUCCESS) {
			...
			if (retrys < envctrl_max_retries) {
				...
				envctrl_init_bus(unitp);
				mutex_enter(&unitp->umutex);
				cmn_err(CE_WARN, "Write to PCF8574 failed, \
					addr = %x\n", slave_addr);
				if (envctrl_debug_flags)
...
retry:
		...
		if (status != DDI_SUCCESS) {
			...
			if (retrys < envctrl_max_retries) {
				...
				envctrl_init_bus(unitp);
				mutex_enter(&unitp->umutex);
				cmn_err(CE_WARN, "Read from PCF8591 failed, \
					slave_addr = %x\n", slave_addr);
			}
		}
...
ioport->chip_num);
		...
		if (status != DDI_SUCCESS) {
			...
			if (retrys < envctrl_max_retries) {
				...
				envctrl_init_bus(unitp);
				mutex_enter(&unitp->umutex);
				cmn_err(CE_WARN, "Read from PCF8574 failed, \
					addr = %x\n", slave_addr);
				if (envctrl_debug_flags)
...
retry:
	...
	if (status == DDI_FAILURE) {
		...
		if (retrys < envctrl_max_retries) {
			...
			envctrl_init_bus(unitp);
			mutex_enter(&unitp->umutex);
			cmn_err(CE_WARN, "envctrl CPU TEMP read failed\n");
...
retry:
	...
	if (status != DDI_SUCCESS) {
		...
		if (retrys < envctrl_max_retries) {
			...
			envctrl_init_bus(unitp);
			mutex_enter(&unitp->umutex);
			cmn_err(CE_WARN, "Read from PCF8574 (FPM) failed\n");
		}
	}
	recv_data = ~recv_data;
