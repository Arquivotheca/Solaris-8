...
#pragma ident	"@(#)iommu.c	1.81	99/10/22 SMI"
...
/*
 * function to get from preallocated list of spare tsbs.
 *      Look at startup.c for details as to where spares are kept.
 *      Since this is an sbus impl, we get the smallest tsb available.
 */

static int
iommu_tsb_spare_alloc(struct sbus_soft_state *softsp)
{
	uint8_t i, smallest_i;
	int smallest = 0;

	for (i = 0, smallest = 0, smallest_i = 0; i < MAX_UPA; i++) {
		if (iommu_tsb_spare[i] == IOMMU_TSB_ISASPARE) {
			if (!smallest) {
				smallest_i = i;
				smallest = iommu_tsb_alloc_size[smallest_i];
			} else if (iommu_tsb_alloc_size[i] < smallest) {
				smallest_i = i;
				smallest = iommu_tsb_alloc_size[smallest_i];
			}
		}
	...
	if (smallest) {
		iommu_tsb_spare[smallest_i] |= IOMMU_TSB_ISUNAVAILABLE;
		softsp->spare_slot = smallest_i;
	}
	return (smallest);
}
...
#undef REG_ADDR
	...
	softsp->iommu_dvma_size = iommu_tsb_alloc_size[softsp->upa_id] <<
	    IOMMU_TSB_TO_RNG;
	if ((softsp->iommu_dvma_size == 0) ||
		((iommu_tsb_spare[softsp->upa_id] & IOMMU_TSB_ISASPARE) ==
			IOMMU_TSB_ISASPARE)) {
		softsp->iommu_dvma_size = iommu_tsb_spare_alloc(softsp)
			<< IOMMU_TSB_TO_RNG;
	}
	 ...
	 */
	if ((iommu_tsb_vaddr[softsp->upa_id] == 0) ||
		((iommu_tsb_spare[softsp->upa_id] & IOMMU_TSB_ISASPARE) ==
			IOMMU_TSB_ISASPARE)) {
		if (softsp->spare_slot < MAX_UPA) {
			softsp->soft_tsb_base_addr =
				(uint64_t *)iommu_tsb_vaddr[softsp->spare_slot];
		} else {
		...
		softsp->soft_tsb_base_addr =
			(uint64_t *)iommu_tsb_vaddr[softsp->upa_id];
	}
...
iommu_uninit(struct sbus_soft_state *softsp)
{
	...
	*softsp->iommu_ctrl_reg &=
	...
	if (softsp->spare_slot != -1) {
		iommu_tsb_spare[softsp->spare_slot] &= ~IOMMU_TSB_ISUNAVAILABLE;
		softsp->spare_slot = (uint8_t)MAX_UPA;
	} else {
		iommu_tsb_spare[softsp->upa_id] = (uchar_t)IOMMU_TSB_ISASPARE;
	}
