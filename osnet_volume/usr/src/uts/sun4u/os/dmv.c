...
#pragma ident "@(#)dmv.c   1.5     99/04/26 SMI"
 ...
 */
caddr_t
ndata_alloc_dmv(caddr_t dmv_alloc_base)
{
	size_t alloc_sz;
	...
	if (dmv_totalints != 0) {
		...
		alloc_sz = sizeof (struct dmv_disp) *
		    (dmv_totalints + 1);

		dmv_dispatch_table = (struct dmv_disp *)
		    roundup((uintptr_t)dmv_alloc_base,
		    sizeof (struct dmv_disp));

		dmv_alloc_base += alloc_sz;
