...
#pragma ident	"@(#)x_call.c	1.59	99/08/13 SMI"
...
xc_init(void)
{
	...
	xc_serv_inum = add_softintr(XCALL_PIL, (intrfunc)xc_serv, 0);
	xc_loop_inum = add_softintr(XCALL_PIL, (intrfunc)xc_loop, 0);
	 ...
	 */
	xc_tick_limit =
	    ((uint64_t)cpu_tick_freq * XC_SEND_MONDO_MSEC) / 1000;
}
...
xc_one(int cix, xcfunc_t *func, uint64_t arg1, uint64_t arg2)
{
	...
	XC_SETUP(cix, func, arg1, arg2);
	init_mondo((xcfunc_t *)xc_serv_inum, 0, 0);
	send_mondo(CPUID_TO_UPAID(cix));	/* does membar_sync */
	fini_mondo();
