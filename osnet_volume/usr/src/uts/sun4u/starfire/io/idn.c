...
#pragma ident	"@(#)idn.c	1.35	99/10/08 SMI"
...
static int	idngetinfo(dev_info_t *, ddi_info_cmd_t, void *, void **);
static int	idnidentify(dev_info_t *);
...
static uint_t	idn_sigbhandler_wakeup(caddr_t arg);
...
static uint_t	idn_handler(caddr_t unused);
/*
 * ioctl services
 */
...
static int	idn_param_get(queue_t *q, mblk_t *mp, caddr_t cp);
...
static struct dev_ops idnops = {
	DEVO_REV,		/* devo_rev */
	0,			/* devo_refcnt */
	idngetinfo,		/* devo_getinfo */
	idnidentify,		/* devo_identify */
	nulldev,		/* devo_probe */
 ...
 */
static int
idnidentify(dev_info_t *dip)
{
	if (strcmp(ddi_get_name(dip), IDNNAME) == 0)
...
idnattach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	switch (cmd) {
	case DDI_RESUME:
		sip = (struct idn *)ddi_get_driver_private(dip);
		/*
	...
	sip->si_dip = dip;
	ddi_set_driver_private(dip, (caddr_t)sip);
 ...
 */
static int
idngetinfo(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg, void **result)
{
	...
	case DDI_INFO_DEVT2DEVINFO:
		rw_enter(&idn.struprwlock, RW_READER);
		for (stp = idn.strup; stp; stp = stp->ss_nextp)
			if (stp->ss_minor == instance)
				break;
		if (stp && stp->ss_sip)
			dip = stp->ss_sip->si_dip;
		rw_exit(&idn.struprwlock);
		if (dip) {
			*result = (void *)dip;
			error = DDI_SUCCESS;
...
idnioctl(register queue_t *wq, register mblk_t *mp)
{
	...
	} else if (!VALID_NDOP(cmd) && (iocp->ioc_count != sizeof (idnop_t))) {
		PR_STR("%s: idnioc(cmd = 0x%x) argsize (%d) != expected (%d)\n",
			proc, cmd, iocp->ioc_count, sizeof (idnop_t));
		error = EINVAL;
		goto done;
	}
	...
	if (!VALID_NDOP(cmd) && (argsize < sizeof (idnop_t))) {
		/*
		 * Gotta have our struct in one message block.
		 */
		PR_STR("%s: REG ioc: pullup needed (1st blk = %d bytes)\n",
			proc, argsize);
		(void) pullupmsg(mp->b_cont, sizeof (idnop_t));
		argsize = sizeof (idnop_t);
	}
...
idn_sigbhandler_create()
{
	...
	sbp = &idn.sigbintr;
	sbp->sb_busy = IDNSIGB_INACTIVE;
	idn.sigb_threadp = thread_create(NULL, 2*PAGESIZE,
					idn_sigbhandler_thread,
					(caddr_t)&sbp, sizeof (sbp),
...
/*ARGSUSED0*/
static uint_t
idn_sigbhandler_wakeup(caddr_t arg)
{
	mutex_enter(&idn.sigbintr.sb_mutex);
...
/*ARGSUSED*/
static int
idn_param_get(queue_t *q, mblk_t *mp, caddr_t cp)
{
...
/*ARGSUSED*/
static int
idn_param_set(queue_t *q, mblk_t *mp, char *value, caddr_t cp)
{
	char		*end;
	ulong_t		new_value;
	idnparam_t	*idnpa = (idnparam_t *)cp;
...
/*ARGSUSED*/
static int
idn_set_net_binding(queue_t *q, mblk_t *mp, char *value, caddr_t cp)
{
	char		*end, *cpup;
	long		net;
	processorid_t	cpuid;
...
/*ARGSUSED*/
static int
idn_get_net_binding(queue_t *q, mblk_t *mp, caddr_t cp)
{
	int	c;
...
idn_retrytask_init()
{
	...
	idn.retryqueue.rq_cache = kmem_cache_create("idn retryjob cache",
...
idn_timercache_init()
{
	...
	idn.timer_cache = kmem_cache_create("idn timer cache",
					sizeof (idn_timer_t),
					0, NULL, NULL, NULL,
					NULL, NULL, 0);
}
...
/*ARGSUSED*/
static int
idn_slabpool_report(queue_t *wq, mblk_t *mp, caddr_t cp)
{
...
/*ARGSUSED*/
static int
idn_buffer_report(queue_t *wq, mblk_t *mp, caddr_t cp)
{
	smr_slab_t	*sp;
	register int	d, cnt;
...
/*ARGSUSED*/
static int
idn_mboxtbl_report(queue_t *wq, mblk_t *mp, caddr_t cp)
{
...
/*ARGSUSED2*/
static int
idn_mainmbox_report(queue_t *wq, mblk_t *mp, caddr_t cp)
{
	int	domid;
	int	header = 0;
...
/*ARGSUSED*/
static int
idn_global_report(queue_t *wq, mblk_t *mp, caddr_t cp)
{
...
/*ARGSUSED*/
static int
idn_domain_report(queue_t *wq, mblk_t *mp, caddr_t cp)
{
	int		d, nchan;
	uint_t		domset;
...
#define	RANDOM(a, b)	\
	...
	if (rw == 1) {
		...
		for (segp = &ktextseg; segp; segp = segp->s_next.list) {
...
idn_prom_getsmr(uint_t *smrsz, uint64_t *paddrp, uint64_t *sizep)
{
	 ...
	 */
	pstk = prom_stack_init(sp, sizeof (sp));
	nodeid = prom_findnode_byname(prom_nextnode((dnode_t)0),
					"memory", pstk);
	prom_stack_fini(pstk);
	...
	if (obpsize == 0) {
		cmn_err(CE_WARN, "!IDN: 139: OBP region for SMR is 0 length");
