...
#pragma ident	"@(#)idn_proto.c	1.35	99/10/21 SMI"
...
idn_recv_con(int domid, idn_msgtype_t *mtp, idn_xdcargs_t xargs)
{
	...
	if (!(msg & IDNP_NACK) && (contype == IDNCON_QUERY)) {
		...
		if ((msg & IDNP_MSGTYPE_MASK) == 0) {
			if (dp->dsync.s_qout == 0) {
				/*
				 * Can't really trust his ready_set,
				 * so reset it to 0 and continue through.
				 * If we really need to query this guy
				 * again, then it will be caught in
				 * sync_register and return a non-zero
				 * query_set.
				 */
				PR_PROTO("%s:%d: unexpected ACK to query\n",
					proc, domid);
				ready_set = 0;
			} else {
				dp->dsync.s_qout--;
...
idn_recv_fin(int domid, idn_msgtype_t *mtp, idn_xdcargs_t xargs)
{
	...
	if (!(msg & IDNP_NACK) && (fintype == IDNFIN_QUERY)) {
		...
		if ((msg & IDNP_MSGTYPE_MASK) == 0) {
			if (dp->dsync.s_qout == 0) {
				/*
				 * Can't really trust his
				 * ready_set.  Reset it to 0
				 * all continue through.  If
				 * we really need to query this
				 * guy, will get a non-zero
				 * query set.
				 */
				PR_PROTO("%s:%d: unexpected ACK "
					"to query\n", proc, domid);
				ready_set = 0;
			} else {
				dp->dsync.s_qout--;
...
idn_deconfig(int domid)
{
	 ...
	 * domain going down is the master.
	 */
	if ((domid == masterid) && (idn.smr.rempfn != PFN_INVALID))  {
...
idn_protocol_init(int nservers)
{
	...
	idn.protocol.p_jobpool = kmem_cache_create("idn protocol jobcache",
	 ...
	 */
	sema_init(&idn.protocol.p_morgue, 0, "idn protocol morgue",
			SEMA_DEFAULT, NULL);
	/*
	 * Alloc server queues.
	 */
	...
	for (i = 0; i < nservers; protoq++, i++) {
		 ...
		 */
		protoq->q_threadp = thread_create(NULL, 2*PAGESIZE,
						idn_protocol_server,
						(caddr_t)&i,
						sizeof (i),
						&p0, TS_RUN, maxclsyspri);
		if (protoq->q_threadp == NULL) {
			idn.nservers = i;
			idn_protocol_deinit();
			return (-1);
		}
...
idn_chanserver_bind(int net, int cpuid)
{
	...
	cp = cpu_get(cpuid);
	if ((cpuid != -1) && ((cp == NULL) || (cpu_status(cp) != P_ONLINE))) {
		mutex_exit(&cpu_lock);
		cmn_err(CE_WARN,
...
idn_chan_server(idn_chansvr_t **cspp)
{
	...
	if ((cpuid = csp->ch_bound_cpuid_pending) != -1) {
		 ...
		 * a particular cpu.
		 */
		if ((cp == NULL) || (cpu_status(cp) != P_ONLINE)) {
			/*
...
idn_activate_channel(idn_chanset_t chanset, idn_chanop_t chanop)
{
	...
	for (c = 0; c < IDN_MAX_NETS; c++) {
		...
		if ((mainhp = idn_chan_server_syncheader(c)) != NULL) {
...
idn_activate_channel_services(int channel)
{
	...
	(void) sprintf(str, "idn chansvr (c=%d) morgue", channel);

	csp->ch_recv_morguep = GETSTRUCT(ksema_t, 1);
	sema_init(csp->ch_recv_morguep, 0, str, SEMA_DRIVER, NULL);
	...
	csp->ch_recv_threadp = thread_create(NULL, 2*PAGESIZE,
					idn_chan_server,
					(caddr_t)&csp, sizeof (csp),
					&p0, TS_RUN, idn_chansvr_pri);
