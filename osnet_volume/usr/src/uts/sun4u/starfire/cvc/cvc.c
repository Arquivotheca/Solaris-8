...
#pragma ident	"@(#)cvc.c	1.28	99/07/14 SMI"
...
static int	cvc_identify(dev_info_t *);
...
static void	cvc_ack(mblk_t *, mblk_t *, uint);
static void	cvc_sendnak(queue_t *, mblk_t *, int);
static void	cvc_reioctl(void *);
static void	cvc_copy(mblk_t *, mblk_t *, uint, unchar);
...
DDI_DEFINE_STREAM_OPS(cvcops, cvc_identify, nulldev, cvc_attach, cvc_detach, \
...
static int
cvc_identify(dev_info_t *devi)
{
	if (strcmp(ddi_get_name(devi), "cvc") == 0)
...
cvc_open(register queue_t *q, dev_t *devp, int flag, int sflag, cred_t *crp)
{
	...
	if (!input_daemon_started) {
		...
		if ((cvc_input_daemon_thread = thread_create(NULL, NULL,
			cvc_input_daemon, 0, 0, &p0, TS_RUN, 60)) == NULL) {
			cmn_err(CE_WARN,
			    "cannot start cvc BBSRAM input daemon\n");
		}
	} else {
		mutex_exit(&cvcmutex);
	}
	...
	if (error < 0) {
		...
		case TIOCMBIC:
			if (iocp->ioc_count != TRANSPARENT) {
				cvc_ack(mp, (mblk_t *)NULL, 0);
			} else {
				cvc_copy(mp, (mblk_t *)NULL, sizeof (int),
				    M_COPYIN);
			}
			/* qreply done below */
			break;
		...
		case TIOCMGET:
			...
			if (tmp == NULL) {
				cvc_sendnak(q, mp, EAGAIN);
				return;
			}
			if (iocp->ioc_count != TRANSPARENT) {
				cvc_ack(mp, (mblk_t *)tmp, 0);
				*(int *)mp->b_cont->b_rptr = 0;
			} else {
				cvc_copy(mp, (mblk_t *)tmp, sizeof (int),
					M_COPYOUT);
				*(int *)mp->b_cont->b_rptr = 0;
			}
			/* qreply done below */
	...
	if (cvcinput_q) {
		...
		if (mp->b_datap->db_type == M_IOCTL) {
			if (iocp->ioc_cmd == TIOCSWINSZ) {
				...
				/*
				 * Its a CVC_DISCONNECT, send hangup.
				 */
				if (cvc_hangup_ok)
		...
		/*
		 * Need to let caller know that it may be necessary
		 * for them to free the message buffer.
		 */
		rv = 0;
	}
...
static void
cvc_ack(mblk_t *mp, mblk_t *dp, uint size)
{
...
static void
cvc_copy(mblk_t *mp, mblk_t *dp, uint size, unchar type)
{
...
static void
cvc_sendnak(queue_t *q, mblk_t  *mp, int err)
{
	register struct iocblk  *iocp = (struct iocblk *)mp->b_rptr;
...
cvc_iobuf_mapin(processorid_t cpu_id)
{
	...
	num_pages = mmu_btopr(((u_int)(cvc_iobuf_physaddr
	...
	return ((caddr_t)(cvaddr + (u_int)(cvc_iobuf_physaddr
		& MMU_PAGEOFFSET)));
}
