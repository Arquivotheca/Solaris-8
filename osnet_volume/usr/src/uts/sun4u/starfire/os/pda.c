...
#pragma ident	"@(#)pda.c	1.12	99/06/02 SMI"
...
pda_get_mem_size(pda_handle_t ph, int boardnum)
{
	...
	basepa &= ~(endpa - 1);
	endpa += basepa;
	basepfn = (uint_t)(basepa >> PAGESHIFT);
	endpfn = (uint_t)(endpa >> PAGESHIFT);

	npages = 0;

	for (c = 0; c < p2o->p2o_memtotal.Memt_NumChunks; c++) {
		uint_t	c_basepfn, c_endpfn;

		c_basepfn = (uint_t)p2o->p2o_mchunks[c].Memc_StartAddress;
		c_endpfn = (uint_t)p2o->p2o_mchunks[c].Memc_Size;
		c_endpfn += c_basepfn;
...
pda_mem_add_span(pda_handle_t ph, uint64_t basepa, uint64_t nbytes)
{
	...
	a_pfn = (uint_t)(basepa >> PAGESHIFT);
	a_npgs = (uint_t)(nbytes >> PAGESHIFT);
...
pda_mem_del_span(pda_handle_t ph, uint64_t basepa, uint64_t nbytes)
{
	...
	d_pfn = (uint_t)(basepa >> PAGESHIFT);
	d_npgs = (uint_t)(nbytes >> PAGESHIFT);
...
cpu_p2o_mapin(int cpuid)
{
	...
	pfn = (uint_t) (cpu_p2o_physaddr >> MMU_PAGESHIFT);
