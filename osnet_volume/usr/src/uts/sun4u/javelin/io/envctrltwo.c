...
#ident	"@(#)envctrltwo.c	1.8	99/04/05 SMI"
...
static u_char _cpu_temps[256];
static u_char _cpu_fan_speeds[256];
 ...
 */
static enum levels {green, yellow, red};
...
static struct modldrv envctrlmodldrv = {
	...
	"I2C ENVCTRLTWO_driver: 1.4 98/01/12",
	&envctrltwo_ops,
};
...
{
	if (strcmp(ddi_get_name(dip), "SUNW,envctrltwo") == 0) {
...
envctrl_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	tblsz = (sizeof (_cpu_temps) / sizeof (u_char));
	...
	tblsz = (sizeof (_cpu_fan_speeds) / sizeof (u_char));
