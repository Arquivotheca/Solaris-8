...
#pragma ident	"@(#)mi.c	1.59	99/11/19 SMI"
...
mi_close_unlink(void **mi_headp, IDP ptr)
{
	...
	if ((dev != OPENFAIL) && (dev != 0) && (dev <= MAXMIN))
		vmem_free(mi_head->mh_arena, (void *)dev, 1);
    ...
    cred_t *credp)
{
	...
	if (mi_head == NULL) {
		...
		*mi_headp = (void *)mi_head;
		...
		mi_head->mh_maxminor = MIN(MAXMIN32, mi_maxminor);
		mi_head->mh_arena = vmem_create(arena_name,
		    (void *)MI_MIN_DEV, mi_head->mh_maxminor - MI_MIN_DEV + 1,
		    1, NULL, NULL, NULL, 1, VM_SLEEP);
	}
	ASSERT(ptr != NULL);
	mi_o = (MI_OP)ptr;
	mi_o--;
	...
	if (sflag == MODOPEN) {
		...
		while ((dev = (dev_t)vmem_alloc(mi_head->mh_arena, 1,
		    VM_NOSLEEP)) == 0) {
			if (mi_head->mh_maxminor >= mi_maxminor)
				return (EBUSY);
			if (vmem_add(mi_head->mh_arena,
			    (void *)(mi_head->mh_maxminor + 1),
			    mi_maxminor - mi_head->mh_maxminor,
			    VM_NOSLEEP) == NULL)
				return (EBUSY);
			mi_head->mh_maxminor = mi_maxminor;
		}
	}
