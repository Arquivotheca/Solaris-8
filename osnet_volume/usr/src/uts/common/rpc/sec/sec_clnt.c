...
#pragma ident	"@(#)sec_clnt.c	1.51	99/10/25 SMI"	/* SVr4.0 1.14	*/
...
gss_clnt_loadinfo(caddr_t usrdata, caddr_t *kdata, model_t model)
{
	...
	if (error == 0) {
		...
		elements = (caddr_t) kmem_alloc(data->mechanism.length,
						KM_SLEEP);
...
sec_clnt_loadinfo(struct sec_data *in, struct sec_data **out, model_t model)
{
	...
	secdata = (struct sec_data *) kmem_alloc(sizeof (*secdata), KM_SLEEP);
	...
	*ap = (AUTH *)NULL;
	...
	for (;;) {
		...
		case AUTH_DES:
			...
			if (desauthtab == NULL) {
				desauthtab = (struct desauthent *) kmem_zalloc(
						...
						model_t model)
{
	...
	case RPCSEC_GSS: {
		...
		if (model != DATAMODEL_NATIVE) {
		    ...
		    if (copyin(mechanism, &mech32, sizeof (gss_OID_desc32))) {
			kmem_free(mech, sizeof (rpc_gss_OID_desc));
			return (EFAULT);
		    }
		    mech->length = mech32.length;
		    mech->elements = (caddr_t)mech32.elements;
		}
		else
#endif _SYSCALL32_IMPL
