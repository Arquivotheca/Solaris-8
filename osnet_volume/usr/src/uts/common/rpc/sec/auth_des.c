...
#ident	"@(#)auth_des.c	1.27	98/02/02 SMI"	/* SVr4.0 1.9	*/
	...
	AUTH **retauth)
{
	...
	*retauth = NULL;
	...
	if ((stat = kgetnetname(namebuf)) != 0) {
		cmn_err(CE_NOTE,
	"authdes_create: unable to get client's netname, rpc status %d", stat);
		goto failed;
	}

	ad->ad_fullnamelen = (u_int) RNDUP(strlen(namebuf));
	ad->ad_fullname = mem_alloc(ad->ad_fullnamelen + 1);

	ad->ad_servernamelen = (u_int) strlen(servername);
	...
	if (ckey == NULL) {
		if ((stat = key_gendes(&auth->ah_key)) != RPC_SUCCESS) {
			cmn_err(CE_NOTE,
	"authdes_create: unable to gen conversation key, rpc status %d", stat);
			if (stat == RPC_INTR)
				error = EINTR;
...
" rpc status %d", (int)cr->cr_uid, (int)stat);
		return (FALSE);
	}
