...
#pragma ident	"@(#)pcgld.h	1.1	99/03/11 SMI"
...
struct gld_mac_info {
	...
	long		gldm_GLD_flags;	/* GLD PRIVATE */
	...
	long		gldm_flags;	/* PRIVATE TO DRIVER */
	long		gldm_state;	/* PRIVATE TO DRIVER */
	long		gldm_maxpkt;
	long		gldm_minpkt;
	char		*gldm_ident;
	long		gldm_type;
	...
	long		gldm_nprom;	/* num streams in promiscuous */
					/* mode--GLD PRIVATE */
	...
	long		gldm_reg_index;	/* SET BY DRIVER FOR GLD TO USE */
	off_t		gldm_reg_len;	/* used to specify length of RAM */
	long		gldm_irq_index;	/* SET BY DRIVER FOR GLD TO USE */
	unsigned long	gldm_options;	/* Identify features to use */
	...
	uint_t		(*gldm_intr_hi)();	/* inform driver of event */
...
extern ulong_t  pcgldcrc32(uchar_t *);
#endif
