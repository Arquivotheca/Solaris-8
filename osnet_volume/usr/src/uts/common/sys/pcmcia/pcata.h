...
#pragma ident	"@(#)pcata.h	1.6	99/10/28 SMI"
...
/*
 * Since the pccards are removable and do not have a hardware lock
 * the cards may be removed while an applicatio has the device open
 *
 * if MAINTAIN_OPEN_STATUS is defined
 *	when a device is removed and reinserted all I/O will be blocked
 *	until the open count reaches 0
 * if MAINTAIN_OPEN_STATUS is not defined
 *	I/O may occur anytime while the device is inserted
 *
 * in either case, if the device is not inserted,
 * all active, queued, or new I/O requests will complete in error (EIO)
 *
 * #define	MAINTAIN_OPEN_STATUS
 *
 */
...
#define	ENABLE_INTERRUPT	8	/* enable interrupt(not part of spec) */
...
typedef struct ata_soft  {
	...
	int		card_invalid;	/* Card is invalid		*/
	int		open_flag;	/* Card is open			*/
...
/*
 * values for intr_pending
 */
#define	IPEND_NONE	0	/* No interrupt is expected */
#define	IPEND_SOFT	1	/* lo priority interrupt expected */
#define	IPEND_HARD	2	/* hi priotity interrupt expected */
...
void pcata_readywait_timeout(void *timeout_arg);
