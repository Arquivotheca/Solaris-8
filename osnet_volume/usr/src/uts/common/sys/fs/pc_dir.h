...
#pragma ident	"@(#)pc_dir.h	1.16	98/01/06 SMI"
...
struct pcdir {
	...
	int	pcd_size;		/* file size (little endian) */
};
...
extern int pc_read_long_fn(struct vnode *dvp, struct uio *uiop,
    struct pc_dirent *ld, struct pcdir **epp, int *offset, struct buf **bp);
extern int pc_read_short_fn(struct vnode *dvp, struct uio *uiop,
    struct pc_dirent *ld, struct pcdir **epp, int *offset, struct buf **bp);
extern int pc_match_long_fn(struct pcnode *pcp, char *namep, struct pcdir **ep,
    struct slot *slotp, int *offset);
extern int pc_match_short_fn(struct pcnode *pcp, char *namep,
    struct pcdir **epp, struct slot *slotp, int *offset);
extern uchar_t pc_checksum_long_fn(char *name, char *ext);
extern void set_long_fn_chunk(struct pcdir_lfn *ep, char *buf, int len);
extern int pc_valid_long_fn(char *namep);
extern int pc_extract_long_fn(struct pcnode *pcp, char *namep,
    struct pcdir **epp, int *offset, struct buf **bp);
extern int pc_fname_ext_to_name(char *namep, char *fname, char *ext,
    int foldcase);

extern pc_cluster32_t pc_getstartcluster(struct pcfs *fsp, struct pcdir *ep);
extern void pc_setstartcluster(struct pcfs *fsp, struct pcdir *ep,
    pc_cluster32_t cln);
