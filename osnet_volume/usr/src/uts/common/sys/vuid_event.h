...
#pragma ident	"@(#)vuid_event.h	1.25	98/01/06 SMI"
/*	From:	SunOS4.1.2	1.17	sundev/vuid_event.c	*/
...
#else
/* For x86, these numbers conflict with VT_?????? ioctl numbers */
#define	VUIDSFORMAT   (VUIOC|11) /* Set input device byte stream format */
#define	VUIDGFORMAT   (VUIOC|12) /* Get input device byte stream format */
#endif
...
#else
/* For x86, these numbers conflict with VT_?????? ioctl numbers */
#define	VUIDSADDR   (VUIOC|13)	/* Set vuid address */
#define	VUIDGADDR   (VUIOC|14)	/* Get vuid address */
#endif
