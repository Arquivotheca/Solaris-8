...
#pragma ident	"@(#)udf_vfsops.c	1.23	99/08/07 SMI"
...
int32_t ud_val_get_vat(struct udf_vfs *, dev_t, daddr_t, struct ud_map *);
...
static int32_t udf_swapvp(struct vfs *, struct vnode **, char *);
...
/* ARGSUSED */
static int32_t
udf_swapvp(struct vfs *vfsp,
	struct vnode **vpp, char *nm)
{
	ud_printf("udf_swapvp\n");
	return (ENOSYS);
}
	...
	struct cred *cr, int32_t isroot)
{
	...
	if (vfsp->vfs_flag & VFS_REMOUNT) {
		...
		for (index = 0; index < count; index ++) {
			...
			if (ud_verify_tag(ttag, UD_LOG_VOL_INT,
					udf_vfsp->udf_iseq_loc + (index >>
					udf_vfsp->udf_l2d_shift), 1) == 0) {
				struct log_vol_int_desc *lvid;
...
remountout:
	...
	udf_vfsp->udf_fsmnt = kmem_zalloc(strlen(name), KM_SLEEP);
...
ud_validate_and_fill_superblock(dev_t dev, int32_t bsize, uint32_t avd_loc)
{
	...
	for (index = 0; index < count; index ++) {
		ttag = (struct tag *)(addr + index * DEV_BSIZE);
		if (ud_verify_tag(ttag, UD_PRI_VOL_DESC,
				vds_loc + (index >> shift), 1) == 0) {
		...
		} else if (ud_verify_tag(ttag, UD_LOG_VOL_DESC,
				vds_loc + (index >> shift), 1) == 0) {
		...
		} else if (ud_verify_tag(ttag, UD_PART_DESC,
				vds_loc + (index >> shift), 1) == 0) {
			int32_t i;
		...
		} else if (ud_verify_tag(ttag, UD_TERM_DESC,
				vds_loc + (index >> shift), 1) == 0) {
			break;
		}
loop_end:
	...
	for (index = 0; index < count; index ++) {
		ttag = (struct tag *)(addr + index * DEV_BSIZE);
		if (ud_verify_tag(ttag, UD_LOG_VOL_INT,
			udf_vfsp->udf_iseq_loc + (index >> shift), 1) == 0) {
	...
	if (ud_verify_tag(&fsd->fsd_tag, UD_FILE_SET_DESC,
			udf_vfsp->udf_fsd_loc, 1) != 0) {
		secbp->b_flags = B_AGE | B_STALE;
		brelse(secbp);
		goto out;
	}
...
ud_destroy_fsp(struct udf_vfs *udf_vfsp)
{
	...
	if (udf_vfsp != NULL) {
		if (udf_vfsp->udf_maps != NULL) {
			map = udf_vfsp->udf_maps;
			for (i = 0; i < udf_vfsp->udf_nmaps; i++) {
				if (map->udm_flags & UDM_MAP_VPM) {
					for (j = 0; j < map->udm_nent; j++) {
						if (map->udm_bp[j]) {
							brelse(map->udm_bp[j]);
						}
					}
					kmem_free(map->udm_count,
					sizeof (uint32_t) * map->udm_nent);
					kmem_free(map->udm_bp,
					sizeof (struct buf *) * map->udm_nent);
			...
			kmem_free(udf_vfsp->udf_maps,
				udf_vfsp->udf_nmaps * sizeof (struct ud_map));
		}
		if (udf_vfsp->udf_parts != NULL) {
			kmem_free(udf_vfsp->udf_parts,
				udf_vfsp->udf_npart * sizeof (struct ud_part));
		}
		if (udf_vfsp->udf_iseq != NULL) {
			udf_vfsp->udf_iseq->b_flags |= (B_STALE|B_AGE);
			brelse(udf_vfsp->udf_iseq);
		}
		if (udf_vfsp->udf_vds != NULL) {
		...
		kmem_free(udf_vfsp, sizeof (struct udf_vfs));
	}
}
	...
	daddr_t blkno, struct ud_map *udm)
{
	...
	for (i = 0; i < ud_sub_count; i++) {
		...
		if (ud_verify_tag(&fe->fe_tag, UD_FILE_ENTRY, 0, 0) == 0) {
			if (ud_verify_tag(&fe->fe_tag, UD_FILE_ENTRY,
				SWAP_32(fe->fe_tag.tag_loc), 1) == 0) {
	...
	ad_type = fe->fe_icb_tag.itag_flags & 0x3;
	...
	udm->udm_count = kmem_zalloc(sizeof (uint32_t) * udm->udm_nent,
				KM_SLEEP);
	udm->udm_bp = kmem_zalloc(sizeof (struct buf *) * udm->udm_nent,
				KM_SLEEP);
	udm->udm_addr = kmem_zalloc(sizeof (uint32_t *) * udm->udm_nent,
				KM_SLEEP);
	for (i = 0; i < udm->udm_nent; i++) {
		if (ad_type == ICB_FLAG_SHORT_AD) {
			...
			if ((blk = ud_xlate_to_daddr(udf_vfsp,
				SWAP_16(lad->lad_ext_prn),
	...
	for (i = 0; i < udm->udm_nent; i++) {
		if (udm->udm_bp[i] != NULL) {
			brelse(udm->udm_bp[i]);
		}
	}
	kmem_free(udm->udm_count, sizeof (uint32_t) * udm->udm_nent);
	kmem_free(udm->udm_bp, sizeof (struct buf *) * udm->udm_nent);
	kmem_free(udm->udm_addr, sizeof (uint32_t *) * udm->udm_nent);
	secbp->b_flags |= B_AGE | B_STALE;
	brelse(secbp);
	return (EINVAL);
}
