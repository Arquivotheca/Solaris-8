...
#pragma ident	"@(#)udf_inode.c	1.22	99/10/06 SMI"
...
#define	UD_END		0x1
#define	UD_UNKN		0x2
struct inode *udf_ifreeh, *udf_ifreet;
...
int32_t ud_updat_ext4(struct inode *, struct file_entry *);
int32_t ud_updat_ext4096(struct inode *, struct file_entry *);
void ud_make_sad(struct icb_ext *, struct short_ad *, int32_t);
void ud_make_lad(struct icb_ext *, struct long_ad *, int32_t);
void ud_trunc_ext4(struct inode *, u_offset_t);
void ud_trunc_ext4096(struct inode *, u_offset_t);
void ud_add_to_free_list(struct inode *, uint32_t);
void ud_remove_from_free_list(struct inode *, uint32_t);


int32_t udf_fstype = -1;

int
udfinit(struct vfssw *vswp, int32_t fstype)
{
	int32_t index;
...
ud_iget(struct vfs *vfsp, uint16_t prn, uint32_t ploc,
	struct inode **ipp, struct buf *pbp, struct cred *cred)
{
...
tryagain:
	...
	if (ud_cur_inodes > ud_max_inodes) {
		...
		while (udf_ifreeh == NULL || ITOV(udf_ifreeh)->v_pages) {
			/*
	 ...
	 */
	if (udf_ifreeh &&
		(nomem || ITOV(udf_ifreeh)->v_pages == NULL ||
		 ...
		 */
		ip = (struct inode *)kmem_zalloc(sizeof (struct inode),
				KM_NOSLEEP);
		if (ip == NULL) {
			mutex_enter(&udf_ifree_lock);
			if (udf_ifreeh) {
				...
				mutex_exit(&udf_ifree_lock);
				ip = (struct inode *)
					kmem_zalloc(sizeof (struct inode),
					KM_SLEEP);
			}
		}
		vp = ITOV(ip);
		...
		ip->i_marker4 = (uint32_t)0xDDDDDDDD;
		ip->i_marker5 = (uint32_t)0xEEEEEEEE;
...
read_de:
	...
	if ((bp->b_flags & B_ERROR) ||
		(ud_verify_tag(&fe->fe_tag, UD_FILE_ENTRY,
			ip->i_icb_block, 1) != 0)) {
...
ie_error:
		...
		if (ud_verify_tag(&ie->ie_tag, UD_INDIRECT_ENT,
				ip->i_icb_block + 1, 1) == 0) {
			struct long_ad *lad;
...
end_4096:
	...
	if (ea_len && (ud_verify_tag(&eah->eah_tag, UD_EXT_ATTR_HDR,
			ip->i_icb_block, 1) == 0)) {
		...
		while (ea_off < ea_len) {
			...
			if ((ah->ahdr_atype == SWAP_32(12)) &&
				...
				ip->i_major =
					SWAP_32(ds->ds_major_id);
				ip->i_minor =
					SWAP_32(ds->ds_minor_id);
			}

			/*
			 * Impl Use EA
			 */
			if ((ah->ahdr_atype == SWAP_32(2048)) &&
				(ah->ahdr_astype == 1)) {
				struct iu_ea *iuea;
				struct copy_mgt_info *cmi;

				iuea = (struct iu_ea *)ah;
				if (strcmp(iuea->iuea_ii.reg_id,
					UDF_FREEEASPACE) == 0) {
					/* skip it */
					iuea = iuea;
				} else if (strcmp(iuea->iuea_ii.reg_id,
					UDF_CGMS_INFO) == 0) {
			...
			/* ??? PARANOIA */
			if (SWAP_32(ah->ahdr_length) == 0) {
				break;
			}
			ea_off += SWAP_32(ah->ahdr_length);
		}
	}
	...
	if (ip->i_desc_type == ICB_FLAG_SHORT_AD) {
		...
		while (ndesc --) {
			...
			offset += iext->ib_count;

			iext->ib_flags = length >> 30;
...
void
ud_iinactive(struct inode *ip, struct cred *cr)
{
	int32_t busy = 0;
	struct vnode *vp;
	vtype_t type;
	...
	if ((ip->i_udf->udf_flags & UDF_FL_RDONLY) == 0) {
		if (ip->i_nlink <= 0) {
			ip->i_marker5 = (uint32_t)0xFFFF0000;
...
tryagain:
	...
	mutex_exit(&ip->i_tlock);
		...
		kmem_free(addr, size);
		kmem_free(addr1, size1);
		ip->i_marker5 = (uint32_t)0xFFFFFFFF;
		kmem_free(ip, sizeof (struct inode));
	}
}
...
void
ud_iupdat(struct inode *ip, int32_t waitfor)
{
	...
	if ((flag & (IUPD|IACC|ICHG|IMOD|IMODACC)) != 0) {
		...
		crc_len = 0xAC + SWAP_32(fe->fe_len_ear);
...
int32_t
ud_updat_ext4(struct inode *ip, struct file_entry *fe)
{
	uint32_t dummy;
...
/* ARGSUSED */
int32_t
ud_updat_ext4096(struct inode *ip, struct file_entry *fe)
{
	return (ENXIO);
}
...
/* ARGSUSED */
int
ud_itrunc(struct inode *oip, u_offset_t length, int32_t flags, struct cred *cr)
{
	int32_t error, boff;
	off_t bsize;
	mode_t mode;
...
void
ud_trunc_ext4(struct inode *ip, u_offset_t length)
{
	int32_t index, l2b, count, ecount;
	int32_t elen, ndent, nient, nused;
	...
	if (ip->i_con) {
		...
		for (nused = 0, index = 0; index < ip->i_con_used; index++) {
			icon = &ip->i_con[index];
			nient = icon->ib_count -
				(sizeof (struct alloc_ext_desc) + elen);
...
void
ud_trunc_ext4096(struct inode *ip, u_offset_t length)
{
	/*
 ...
 */
int
ud_iaccess(struct inode *ip, int32_t mode, struct cred *cr)
{
/*
...
*/
	...
	if ((ip->i_perm & mode) == mode)
		return (0);
	return (EACCES);
}

void
ud_imark(struct inode *ip)
{
	ud_printf("ud_imark\n");
	if (ip->i_flag & IACC) {
		ip->i_atime.tv_sec = hrestime.tv_sec;
		ip->i_atime.tv_nsec = hrestime.tv_nsec;
	}
	if (ip->i_flag & IUPD) {
		ip->i_mtime.tv_sec = hrestime.tv_sec;
		ip->i_mtime.tv_nsec = hrestime.tv_nsec;
		ip->i_flag |= IMODTIME;
	}
	if (ip->i_flag & ICHG) {
		ip->i_diroff = 0;
		ip->i_ctime.tv_sec = hrestime.tv_sec;
		ip->i_ctime.tv_nsec = hrestime.tv_nsec;
	}
}


void
ud_itimes_nolock(struct inode *ip)
{
...
ud_idrop(struct inode *ip)
{
	...
	mutex_enter(&ip->i_tlock);
	mutex_enter(&udf_ifree_lock);
	if (vp->v_pages == NULL || ip->i_perm == 0) {
		ud_add_to_free_list(ip, UD_BEGIN);
	} else {
		/*
...
void
ud_add_to_free_list(struct inode *ip, uint32_t at)
{
	ASSERT(ip);
...
void
ud_remove_from_free_list(struct inode *ip, uint32_t at)
{
	ASSERT(ip);
	ASSERT(mutex_owned(&udf_ifree_lock));
