...
#pragma ident	"@(#)udf_alloc.c	1.19	99/08/10 SMI"
...
#ifdef	DEBUG
extern struct inode *ud_search_icache(struct vfs *, uint16_t, uint32_t);
#endif
...
/* ARGSUSED */
int32_t
ud_ialloc(struct inode *pip,
	struct inode **ipp, struct vattr *vap, struct cred *cr)
{
	int32_t err;
	...
	if ((vap->va_mask & AT_GID) &&
		((cr->cr_uid == 0) || (vap->va_gid == pip->i_gid) ||
		...
		fe->fe_gid = (pip->i_char & ISGID) ?
				SWAP_32(pip->i_gid) : SWAP_32(cr->cr_gid);
	}
	...
	time.tv_sec = hrestime.tv_sec;
	time.tv_nsec = hrestime.tv_nsec;
	...
	if ((pip->i_char & ISGID) && (vap->va_type == VDIR)) {
		...
		if ((ichar & ISGID) &&
			(!groupmember((uid_t)SWAP_32(fe->fe_gid), cr)) &&
			(cr->cr_uid != 0)) {
			ichar &= ~ISGID;
		}
	}
...
void
ud_ifree(struct inode *ip, vtype_t type)
{
	struct udf_vfs *udf_vfsp;
	struct buf *bp;
