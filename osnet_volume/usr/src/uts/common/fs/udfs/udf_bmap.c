...
#pragma ident	"@(#)udf_bmap.c	1.17	99/06/02 SMI"
...
int32_t ud_break_create_new_icb(struct inode *, int32_t, uint32_t);
int32_t ud_bump_ext_count(struct inode *, int32_t);
void ud_remove_ext_at_index(struct inode *, int32_t);
int32_t ud_last_alloc_ext(struct inode *, uint64_t, uint32_t, int32_t);
int32_t ud_create_ext(struct inode *, int32_t, uint32_t,
	int32_t, uint64_t, uint64_t *);
int32_t	ud_zero_it(struct inode *, uint32_t, uint32_t);
...
int32_t
ud_bmap_has_holes(struct inode *ip)
{
	int32_t i, error = 0;
	struct icb_ext *iext;
...
int32_t
ud_bmap_read(struct inode *ip, u_offset_t off, daddr_t *bnp, int32_t *lenp)
{
	struct icb_ext *iext;
	daddr_t bno;
...
int32_t
ud_read_next_cont(struct inode *ip)
{
	int32_t error = 0, ndesc, count;
	uint32_t length, dummy;
	struct buf *bp;
	caddr_t addr;
	daddr_t bno;
	u_offset_t offset;
	struct icb_ext *cont, *iext;
	...
	if (ud_verify_tag(&aed->aed_tag, UD_ALLOC_EXT_DESC,
			cont->ib_block, 1) != 0) {
		error = EINVAL;
		goto end;
	}
	if (ip->i_desc_type == ICB_FLAG_SHORT_AD) {
		...
		sad = (struct short_ad *)
			(bp->b_un.b_addr + sizeof (struct alloc_ext_desc));
		ndesc = SWAP_32(aed->aed_len_aed) / sizeof (struct short_ad);


		count = (((ip->i_ext_used + ndesc) /
				EXT_PER_MALLOC) + 1) * EXT_PER_MALLOC;

		addr = kmem_zalloc(count *
				sizeof (struct icb_ext), KM_SLEEP);
		bcopy(ip->i_ext, addr,
			sizeof (struct icb_ext) * ip->i_ext_used);
		kmem_free(ip->i_ext, sizeof (struct icb_ext) * ip->i_ext_count);
		ip->i_ext = (struct icb_ext *)addr;
		...
		ndesc = SWAP_32(aed->aed_len_aed) / sizeof (struct long_ad);
...
int32_t
ud_read_icb_till_off(struct inode *ip,
	u_offset_t offset)
{
	int32_t error = 0;
	struct icb_ext *iext;
	...
	if (ip->i_desc_type == ICB_FLAG_ONE_AD) {
		...
		while ((iext->ib_offset + iext->ib_count) < offset) {
			...
			if ((error = ud_read_next_cont(ip)) != 0) {
				return (error);
			}
			ip->i_con_read++;
			iext = &ip->i_ext[ip->i_ext_used - 1];
	...
	/*
	 * we donot support any other allocation strategies
	 */
	return (EINVAL);
}
 ...
 */
int32_t
ud_last_alloc_ext(struct inode *ip, uint64_t off,
 ...
 */
int32_t
ud_break_create_new_icb(struct inode *ip,
...
void
ud_remove_ext_at_index(struct inode *ip, int32_t index)
{
	int32_t i;
...
int32_t
ud_bump_ext_count(struct inode *ip, int32_t sleep_flag)
{
	int32_t error = 0;
	struct icb_ext *iext;
...
int32_t
ud_create_ext(struct inode *ip, int32_t index, uint32_t flags,
...
int32_t
ud_zero_it(struct inode *ip, uint32_t start_block, uint32_t block_count)
{
	struct udf_vfs *udf_vfsp;
	uint32_t bno, dummy;
	int32_t error;
	...
	bp->b_flags = B_KERNBUF | B_WRITE | B_BUSY;
	bp->b_edev = ip->i_dev;
