...
#pragma ident	"@(#)udf_dir.c	1.16	99/08/10 SMI"
...
int32_t ud_dircheckforname(struct inode *,
	char *, int, struct slot *, struct inode **, uint8_t *, struct cred *);
int32_t ud_dirempty(struct inode *, uint64_t, struct cred *);
int32_t str2cmp(char *, int32_t, char *, int32_t, char *, int32_t);
int32_t ud_dircheckpath(int32_t, struct inode *, struct cred *);
int32_t ud_dirmakeinode(struct inode *, struct inode **,
	struct vattr *, enum de_op, struct cred *);
int32_t ud_diraddentry(struct inode *, char *,
	enum de_op, int, struct slot *, struct inode *,
	struct inode *, struct cred *);
int32_t ud_dirmakedirect(struct inode *, struct inode *, struct cred *);
int32_t ud_dirrename(struct inode *, struct inode *,
	struct inode *, struct inode *, char *, uint8_t *,
	struct slot *, struct cred *);
int32_t ud_dirprepareentry(struct inode *,
	struct slot *, uint8_t *, struct cred *);
int32_t ud_dirfixdotdot(struct inode *, struct inode *, struct inode *);
int32_t ud_write_fid(struct inode *, struct slot *, uint8_t *);

int
ud_dirlook(struct inode *dip,
	char *namep, struct inode **ipp, struct cred *cr, int32_t skipdnlc)
{
...
restart:
	...
	if ((namelen == 1) &&
		...
		VN_HOLD(dvp);
		*ipp = dip;
		dnlc_enter(dvp, namep, ITOV(*ipp), NOCRED);
		return (0);
	}

	if ((!skipdnlc) && (vp = dnlc_lookup(dvp, namep, NOCRED))) {
...
int
ud_direnter(struct inode *tdp,
	char *namep, enum de_op op, struct inode *sdp, struct inode *sip,
	struct vattr *vap, struct inode **ipp, struct cred *cr)
{
	struct udf_vfs *udf_vfsp;
	struct inode *tip;
	struct slot slot;
...
*/
int
ud_dirremove(struct inode *dp,
	char *namep, struct inode *oip, struct vnode *cdir,
	enum dr_op op, struct cred *cr)
{
	 ...
	 */
	if ((dp->i_char & ISVTX) && cr->cr_uid != 0 &&
		cr->cr_uid != dp->i_uid && cr->cr_uid != ip->i_uid &&
		(err = ud_iaccess(ip, IWRITE, cr))) {
...
int
ud_dircheckforname(struct inode *tdp,
	char *namep, int32_t namelen, struct slot *slotp,
	struct inode **ipp, uint8_t *buf, struct cred *cr)
{
	struct udf_vfs *udf_vfsp;
...
/* ARGSUSED */
int
ud_dirempty(struct inode *ip, uint64_t ino, struct cred *cr)
{
	offset_t off;
	...
	addr = kmem_zalloc(1024, KM_SLEEP);
	...
	for (off = 0; off < ip->i_size; off += entry_len) {
		...
		if (ud_verify_tag(&fid->fid_tag, UD_FILE_ID_DESC,
				tbno, 0) != 0) {
			/* Corrupted directory */
			empty = 0;
			break;
...
ud_dircheckpath(int32_t blkno,
	struct inode *target, struct cred *cr)
{
	int32_t err = 0;
	struct vfs *vfsp;
	struct udf_vfs *udf_vfsp;
...
int
ud_dirmakeinode(struct inode *tdp, struct inode **ipp,
 ...
 */
int
ud_diraddentry(struct inode *tdp, char *namep,
...
/* ARGSUSED2 */
int
ud_dirmakedirect(struct inode *ip,
	struct inode *dp, struct cred *cr)
{
	int32_t err;
...
int
ud_dirrename(struct inode *sdp, struct inode *sip,
	struct inode *tdp, struct inode *tip, char *namep,
	uint8_t *buf, struct slot *slotp, struct cred *cr)
{
	 ...
	 */
	if (error = ud_iaccess(tdp, IWRITE, cr)) {
		goto out;
	}
	 ...
	 */
	if ((tdp->i_char & ISVTX) && cr->cr_uid != 0 &&
	    cr->cr_uid != tdp->i_uid && cr->cr_uid != tip->i_uid &&
	    (error = ud_iaccess(tip, IWRITE, cr))) {
		goto out;
	}
 ...
 */
int
ud_dirprepareentry(struct inode *dp,
 ...
 */
int
ud_dirfixdotdot(struct inode *dp,
	struct inode *opdp, struct inode *npdp)
{
	int32_t err = 0;
	struct fbuf *fbp;
	struct file_id *fid;
...
int32_t
ud_write_fid(struct inode *dp, struct slot *slot, uint8_t *buf)
{
	struct udf_vfs *udf_vfsp;
	struct fbuf *lfbp;
	struct file_id *fid;
