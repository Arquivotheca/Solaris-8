...
#pragma ident	"@(#)hsfs_rrip.c	1.12	97/04/05 SMI"
...
static void name_parse(int, u_char *, size_t, u_char *, int *, u_long *, int);
 ...
 */
static void
form_time(int time_length, u_char *file_time, struct timeval *tvp)
{
...
name_parse(
	...
	u_char		*SUA_string,	/* string from SUA */
	size_t		SUA_string_len, /* length of SUA string */
	u_char		*to_string,	/* string to copy to */
...
rrip_sym_link(sig_args_t *sig_args_p)
{
	...
	for (comp_ptr = RRIP_sl_comp(sl_ptr);
		...
		name_parse((int)RRIP_COMP_FLAGS(comp_ptr),
		    RRIP_comp(comp_ptr),
		    (size_t)RRIP_COMP_NAME_LEN(comp_ptr), (u_char *)sym_link,
	...
	hdp->ext_size = (u_int) strlen(tmp_sym_link);
