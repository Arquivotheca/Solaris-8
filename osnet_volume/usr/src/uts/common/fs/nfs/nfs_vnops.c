...
#pragma ident	"@(#)nfs_vnops.c	1.220	99/11/11 SMI"
...
static int	nfs_rdwrlbn(vnode_t *, page_t *, uint_t, size_t, int, cred_t *);
...
static int	nfsasyncreaddir(vnode_t *, rddir_cache *, cred_t *);
 ...
 */
static int
nfs_rdwrlbn(vnode_t *vp, page_t *pp, uint_t off, size_t len, int flags,
	cred_t *cr)
{
	struct buf *bp;
	int error;
...
nfssetattr(vnode_t *vp, struct vattr *vap, int flags, cred_t *cr)
{
	...
	if ((mask & AT_MTIME) && !(flags & ATTR_UTIME)) {
		...
		if (NFS_TIME_T_OK(vap->va_mtime.tv_sec) &&
			...
			if (NFS_TIME_T_OK(hrestime.tv_sec)) {
		...
		/* either don't set times, or use client times */
		error = vattr_to_sattr(vap, &args.saa_sa);
	}
	if (error) {
	...
	if (!error) {
		...
		if (!error) {
			...
			/* If we had time overflow, exit here */
			IF_NOT_NFS_TIME_OK(
			    NFS2_TIME_OK(ns.ns_attr.na_ctime.tv_sec) &&
			    NFS2_TIME_OK(ns.ns_attr.na_mtime.tv_sec),
			{
				error = EOVERFLOW;
				PURGE_ATTRCACHE(vp);
				return (error);
			 ...
			 */
			NFS_TIME_T_CONVERT(ctime.tv_sec,
				ns.ns_attr.na_ctime.tv_sec);
			ctime.tv_nsec = (uint32_t)(ns.ns_attr.na_ctime.tv_usec
							 * 1000);
			NFS_TIME_T_CONVERT(mtime.tv_sec,
				ns.ns_attr.na_mtime.tv_sec);
			mtime.tv_nsec = (uint32_t)(ns.ns_attr.na_mtime.tv_usec
							 * 1000);
...
static int
nfsasyncreaddir(vnode_t *vp, rddir_cache *rdc, cred_t *cr)
{
	int error;
	rnode_t *rp;
...
#define	NFS_FIRST_NRA	1
static int nfs_first_nra = NFS_FIRST_NRA;
...
reread:
	...
	if ((off & (MAXBSIZE - 1)) == 0 &&
		...
		/*
		 * nfs_first_nra is to control number
		 * of reahaheads when offset == 0.
		 */

		if (off == 0)
			readahead = nfs_first_nra;
	...
	size_t len, uint_t prot, uint_t maxprot, uint_t flags, cred_t *cr)
{
	...
	if (vp->v_pages != NULL && !(vp->v_vfsp->vfs_flag & VFS_RDONLY) &&
		...
		mutex_exit(&rp->r_statelock);
		if (VTOMI(vp)->mi_flags & MI_NOCTO)
			error = nfs_putpage(vp, (u_offset_t)off, len,
							B_ASYNC, cr);
		else
			error = nfs_putpage(vp, (u_offset_t)off, len, 0, cr);
		if (!error) {
			mutex_enter(&rp->r_statelock);
			error = rp->r_error;
			rp->r_error = 0;
