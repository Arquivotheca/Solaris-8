...
#pragma ident "@(#)nfs_dlinet.c 1.76	99/07/18 SMI"
...
static int		dhcpinfo(char *, char *, int, struct netbuf *, char *,
			    int);
static int		dlifinit(TIUSER *, struct in_addr *, struct in_addr *,
			    struct in_addr *);
static int		dlbind(vnode_t *);
...
static char rootopts[256];		/* root file system mount options */
	...
	char *server_name, struct netbuf *server_address, char *server_path)
{
	...
	/*
	 * Call dhcpinfo() to see whether we can satisfy this request by using
	 * the information cached in memory by the boot program's DHCP
	 * implementation rather than consult BOOTPARAMS, but while preserving
	 * the semantics of getfile(). We know that the server name is
	 * SYS_NMLN in length, and server_path is MAXPATHLEN (pn_alloc).
	 */
	if (strcmp(fileid, "root") == 0) {
		if (dhcpinfo(fileid, server_name, SYS_NMLN, server_address,
		    server_path, MAXPATHLEN)) {
			using_dhcp = TRUE;
			return (0);
		}
		root = TRUE;
	...
	/*
	 *	If using DHCP, dhcpinfo() will already have the information
	 *	necessary to fill in rootopts[] from the VS_NFSMNT_ROOTOPTS
	 *	vendor option.
	 */
	if (strcmp(fileid, "rootopts") == 0 && using_dhcp == TRUE) {
		(void) dhcpinfo(fileid, server_name, SYS_NMLN, server_address,
		    rootopts, sizeof (rootopts));
		return (0);
	}
 ...
 * encoded at that location. In this case, we simply verify that the necessary
 * information is present and place the data in the buffers and return 1.
 *
 * If no such property exists (or the information is incomplete or garbled),
 * the function returns 0.
 */
static int
dhcpinfo(char *fileid, char *server_name, int server_len,
    struct netbuf *server_address, char *server_data, int data_len)
{
	int rc, i;
	static int if_initialized = FALSE;
	char *p;
	...
	TIUSER *tiptr;
	struct sockaddr_in *sin = (struct sockaddr_in *)server_address->buf;

	if (dhcack == NULL) {
		if (dldebug)
			printf("dhcp: dhcack %x\n", (int)dhcack);
		return (0);
	}
	...
	if (pl == NULL) {
		int	true_dhcacklen;
		char	*ackp = (char *)(dhcack + IFNAMSIZ);

		ASSERT(strcmp(fileid, "root") == 0);

		true_dhcacklen = strlen(ackp);

		/*
		 *	Since we expect the "bootp-response" property to have
		 *	been encoded via octet_to_ascii(), its length should
		 *	always be even.
		 */
		ASSERT((true_dhcacklen % 2) == 0);

		if (dldebug) {
			printf("dhcp:  dhcack %x, len %d\n", (int)dhcack,
		...
		if (ascii_to_octet(ackp, true_dhcacklen, (uchar_t *)pl->pkt,
		    (int *)&pl->len) != 0) {
			cmn_err(CE_WARN,
			    "dhcp: boot dhcp cache is corrupted.\n");
			kmem_free((void *)pl->pkt, pl->len);
			kmem_free((void *)pl, sizeof (PKT_LIST));
			pl = NULL;
			return (0);
		}

		bzero((caddr_t)pl->opts, (DHCP_LAST_OPT + 1) *
		    sizeof (DHCP_OPT *));
		bzero((caddr_t)pl->vs, (VS_OPTION_END - VS_OPTION_START + 1) *
		    sizeof (DHCP_OPT *));
		...
		if (pl->opts[CD_HOSTNAME] != NULL) {
			...
			if (i > SYS_NMLN) {
				cmn_err(CE_WARN,
				    "dhcp: Notice: Hostname is too long\n");
			} else {
				bcopy((caddr_t)doptp->value,
				    (caddr_t)utsname.nodename, i);
		...
		if (p != NULL) {
			if (i > SYS_NMLN) {
				cmn_err(CE_WARN,
				    "dhcp: Notice: NIS domainname too long.\n");
			} else {
				bcopy(p, (caddr_t)srpc_domain, i);
				srpc_domain[i] = '\0';
				if (dldebug)
		...
		/* extract root path, record in server_data */
		if (pl->vs[VS_NFSMNT_ROOTPATH] == NULL) {
			cmn_err(CE_WARN, "dhcp:  missing required root path\n");
			kmem_free((void *)pl->pkt, pl->len);
			kmem_free((void *)pl, sizeof (PKT_LIST));
			pl = NULL;
			return (0);
			...
			if (len > (data_len - 1)) {
				cmn_err(CE_WARN,
				    "dhcp: root pathname is too long (> %d)\n",
				    data_len);
				kmem_free((void *)pl->pkt, pl->len);
				kmem_free((void *)pl, sizeof (PKT_LIST));
				pl = NULL;
			...
			bcopy((caddr_t)doptp->value, (caddr_t)server_data, len);
			server_data[len] = '\0';
			if (dldebug)
	...
	/* fetch root file system mount options and save in server_data[] */
	if (strcmp(fileid, "rootopts") == 0) {
		if (pl->vs[VS_NFSMNT_ROOTOPTS] == NULL) {
			if (dldebug)
				printf("dhcp:  no rootopts\n");
			/* not an error */
			server_data[0] = '\0';
		} else {
			int len;
			...
			len = doptp->len;
			if (len > (data_len - 1)) {
				cmn_err(CE_WARN, "dhcp: root mount options are "
				    "too long (> %d)\n", data_len);
				kmem_free((void *)pl->pkt, pl->len);
				kmem_free((void *)pl, sizeof (PKT_LIST));
				pl = NULL;
	...
	/* set server_name */
	if (pl->vs[VS_NFSMNT_ROOTSRVR_NAME] == NULL) {
		char *unknown_hostname = "unknown_rootfs_host";

		cmn_err(CE_WARN, "dhcp: missing root server name, using '%s'\n",
		    unknown_hostname);
		bcopy(unknown_hostname, (caddr_t)server_name,
		    strlen(unknown_hostname) + 1);
	} else {
		int len;
		...
		if (len > (server_len - 1)) {
			cmn_err(CE_WARN,
			    "dhcp: root server name is too long (> %d)\n",
			    server_len);
			kmem_free((void *)pl->pkt, pl->len);
			kmem_free((void *)pl, sizeof (PKT_LIST));
			pl = NULL;
		...
		bcopy((caddr_t)doptp->value, (caddr_t)server_name, len);
		server_name[len] = '\0';
		if (dldebug)
	...
	sin->sin_family = AF_INET;
	server_address->len = sizeof (struct sockaddr_in);
	if (pl->vs[VS_NFSMNT_ROOTSRVR_IP] == NULL) {
		cmn_err(CE_WARN, "dhcp:  missing root server IP address\n");
		kmem_free((void *)pl->pkt, pl->len);
		kmem_free((void *)pl, sizeof (PKT_LIST));
		pl = NULL;
		return (0);
		...
		if (doptp->len != sizeof (struct in_addr)) {
			cmn_err(CE_WARN,
			    "dhcp:  root server IP address len bad %d\n",
			    doptp->len);
			pl->vs[VS_NFSMNT_ROOTSRVR_IP] = NULL;
			kmem_free((void *)pl->pkt, pl->len);
			kmem_free((void *)pl, sizeof (PKT_LIST));
			pl = NULL;
			return (0);
		}
		bcopy((caddr_t)doptp->value, (caddr_t)&sin->sin_addr,
		    sizeof (struct in_addr));
		if (dldebug) {
	...
	/*
	 *	If we've already initialized the interface, return.
	 *	Otherwise, continue to compute the netmask, bring the
	 *	interface up, and add routes.
	 */
	if (if_initialized) {
		kmem_free((void *)pl->pkt, pl->len);
		kmem_free((void *)pl, sizeof (PKT_LIST));
	...
	if (pl->opts[CD_BROADCASTADDR] != NULL) {
		...
		if (doptp->len != sizeof (struct in_addr)) {
			...
			bcopy((caddr_t)doptp->value, (caddr_t)&braddr,
			    sizeof (struct in_addr));
			if (dldebug)
	...
	if ((rc = t_kopen((file_t *)NULL, dl_udp_netconf.knc_rdev,
				FREAD|FWRITE, &tiptr, CRED())) != 0) {
		nfs_perror(rc, "dhcp: t_kopen udp failed: %m.\n");
	}
	if (rc = dlifinit(tiptr, &pl->pkt->yiaddr, &subnet, &braddr)) {
		nfs_perror(rc, "dhcp: dlifinit failed: %m\n");
		kmem_free((void *)pl->pkt, pl->len);
		kmem_free((void *)pl, sizeof (PKT_LIST));
		pl = NULL;
		(void) t_kclose(tiptr, 0);
	...
	if (pl->opts[CD_ROUTER] != NULL) {
		...
		if ((doptp->len % sizeof (struct in_addr)) != 0) {
			...
			for (tp = doptp->value, i = 0; i < nrouters; i++) {
				struct in_addr defr;
				struct rtentry	rtentry;
...
static int
dlifinit(TIUSER *tiptr, struct in_addr *myIPaddr, struct in_addr *mymask,
    struct in_addr *mybraddr)
{
	...
	if (dldebug)
		printf("dlifinit:  entered\n");

	if (rc = kstr_open(clone_maj, ndev, &vp, NULL)) { /* XXX */
		nfs_perror(rc, "dlifinit: kstr_open failed: %m\n");
		return (rc);
	}
	if (rc = dlbind(vp)) {
	...
	if (rc = dlifconfig(tiptr, myIPaddr, mymask, mybraddr)) {
		(void) kstr_close(vp, -1);
		return (rc);
	}
	...
	if (dldebug)
		printf("dlifinit:  returned\n");
	return (0);
}

/*
 *	Perform DLPI attach and bind.
 */
static int
dlbind(vnode_t *vp)
{
    ...
    struct in_addr *mybraddr)
{
	...
	bcopy((caddr_t)myIPaddr, (caddr_t)&sin.sin_addr,
	    sizeof (struct in_addr));
	sin.sin_family = AF_INET;
	...
	if (mybraddr->s_addr != INADDR_BROADCAST) {
		bcopy((caddr_t)mybraddr, (caddr_t)&sin.sin_addr,
		    sizeof (struct in_addr));
		sin.sin_family = AF_INET;
		sbuf.buf = (caddr_t)&sin;
		sbuf.maxlen = sbuf.len = sizeof (sin);
	...
	bcopy((caddr_t)mymask, (caddr_t)&sin.sin_addr, sizeof (struct in_addr));
	sin.sin_family = AF_INET;
	sbuf.buf = (caddr_t)&sin;
	sbuf.maxlen = sbuf.len = sizeof (sin);
...
static int
dl_bind(vnode_t *vp, uint_t sap, uint_t max_conn, uint_t service,
	uint_t conn_mgmt)
{
