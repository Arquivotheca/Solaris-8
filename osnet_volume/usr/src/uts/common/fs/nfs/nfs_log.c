...
#pragma ident	"@(#)nfs_log.c	1.45	99/03/19 SMI"
...
static struct flush_thread_params {
...
log_buffer_rele(struct log_buffer *lbp)
{
	...
	if (lbp->lb_refcnt < 0)
		cmn_err(CE_PANIC, "log_rele: log_buffer refcnt < 0");
	...
	if (lbp->lb_refcnt == 1) {
		...
		mutex_enter(&lbp->lb_lock);
		/*
		 * If the ref count is > 1 then someone has stepped in to
		 * use this log buffer.  unlock and return.
		 */
		if (lbp->lb_refcnt > 1) {
			mutex_exit(&lbp->lb_lock);
			rw_exit(&nfslog_buffer_list_lock);
...
log_file_create(caddr_t origname, struct log_file **lfpp)
{
	...
	if (error = vn_open(name, UIO_SYSSPACE, FCREAT|FWRITE, LOG_MODE,
	    &vp, CRCREAT, 0)) {
		cmn_err(CE_WARN,
			"log_file_create: Can not open %s - error %d",
			name, error);
			goto out;
	}
	...
	if (error) {
		cmn_err(CE_WARN,
			"log_file_create: Can not stat %s - error %d",
			name, error);
		goto out;
	}
...
out:
	if (vp != NULL) {
		int error1;
		error1 = VOP_CLOSE(vp, FCREAT|FWRITE, 1, (offset_t)0, CRED());
		if (error1) {
			cmn_err(CE_WARN,
				"log_file_create: Can not close %s - error %d",
				name, error1);
		}
		VN_RELE(vp);
	}
...
log_file_rele(struct log_file *lfp)
{
	...
	if (lfp->lf_refcnt < 0)
		cmn_err(CE_PANIC, "log_file_rele: log_file refcnt < 0");
	...
	if (error = VOP_CLOSE(lfp->lf_vp, FCREAT|FWRITE, 1, (offset_t)0,
		CRED())) {
		cmn_err(CE_WARN,
			"NFS: Could not close log buffer %s - error=%d",
			lfp->lf_path, error);
	} else {
		LOGGING_DPRINT((3,
	...
	int flags)
{
	...
	*cookie = (void *)lrp;
	...
	LOGGING_DPRINT((3,
		"nfslog_record_alloc(log_buffer=%p mem=%p size=%d)\n",
...
{
	struct log_file *lfp = lbp->lb_logfile;
	...
	struct lr_alloc *lrp_writers, int num_recs)
{
	...
	uio.uio_resid = len;
	uio.uio_llimit = (rlim64_t)MAXOFF_T;
	...
	if ((error = VOP_GETATTR(vp, &va, 0, CRED())) == 0) {
		VOP_RWLOCK(vp, 1);		/* UIO_WRITE */
		error = VOP_WRITE(vp, &uio, ioflag, CRED());
		VOP_RWUNLOCK(vp, 1);
		if (uio.uio_resid)
			error = ENOSPC;
		if (error)
			(void) VOP_SETATTR(vp, &va, 0, CRED());
	}
...
/*
 * Rename lp->lf_logfile to reflect the true name requested by 'share'
 */
static int
...
nfslog_record_append2all(struct lr_alloc *lrp)
{
	...
	for (nlbp = NULL; lbp != NULL; lbp = nlbp) {
		...
		mutex_exit(&lbp->lb_lock);
		error = nfslog_records_flush_to_disk(lbp);
...
/* ARGSUSED */
int
nfsl_flush(struct nfsl_flush_args *args, model_t model, cred_t *cr)
{
	...
	if (nfsl_args->directive & NFSL_SYNC) {
		 ...
		 */
		if (thread_create(NULL, NULL, nfslog_do_flush,
		    (caddr_t)tparams, 0, &p0, TS_RUN, 60) != NULL) {
			error = 0;
		} else {
			LOGGING_DPRINT((10,
				"nfsl_flush: Couldn't create async thread \n"));
			error = EIO;
		}
	}
...
create_buffer_header(caddr_t *loghdr, size_t *reclen, size_t *freesize)
{
	...
	*freesize = NFSLOG_SMALL_RECORD_SIZE;
	...
	lh.bh_flags = 0;
	lh.bh_offset = 0;
	TIMESPEC_TO_TIMESPEC32(&lh.bh_timestamp, &hrestime);
