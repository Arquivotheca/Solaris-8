...
#pragma ident	"@(#)quotacalls.c	1.64	99/08/07 SMI"
	...
	struct cred *cr)
{
	...
	if ((ufsvfsp->vfs_qflags & MQ_ENABLED) != 0) {
		...
		if (qip->i_size < qlen) {
			...
			VOP_PUTPAGE(vp, (offset_t)0, (size_t)qip->i_size,
				    B_INVAL, kcred);
		} else {
			rw_exit(&qip->i_contents);
		}
	...
	if (error == 0) {
		...
		vfs_setmntopt(&ufsvfsp->vfs_vfs->vfs_mntopts, MNTOPT_QUOTA,
			NULL, 0);
		dqput(dqp);
...
closedq(struct ufsvfs *ufsvfsp, struct cred *cr)
{
	...
	vfs_setmntopt(&ufsvfsp->vfs_vfs->vfs_mntopts, MNTOPT_NOQUOTA,
		NULL, 0);
