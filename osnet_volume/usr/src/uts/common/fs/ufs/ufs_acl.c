/*
 * Copyright 1992,1996-1999 by Sun Microsystems, Inc.
 *	All rights reserved.
 */
#pragma	ident	"@(#)ufs_acl.c	1.76	99/08/25 SMI"
...
switchshadows:
	 ...
	 *
	 *		if an ACL mask exists, the effective group rights
	 *		should be the intersection of the mask and the
	 *		object group bits (bug 4091882).
	 */
	if (sp->aowner) {				/* Owner */
...
ufs_acl_set(struct inode *ip, vsecattr_t *vsap, int flag, cred_t *cr)
{
	...
	/*
	 * only the owner of the file or superuser can change the ACLs
	 */
...
ufs_acl_setattr(struct inode *ip, struct vattr *vap, cred_t *cr)
{
	 ...
	 * exists.  This is so chmod will actually change the group
	...
	if (mask & AT_MODE) {
		...
		sp->agroup->acl_ic_perm = (o_mode_t)(ip->i_mode & 070) >> 3;
		if (sp->aclass.acl_ismask)
