...
#ident	"@(#)hat_refmod.c	1.33	98/01/23 SMI"
...
{
	u_int nbits;		/* number of bits */
	u_int bn;		/* bit number */
	u_int id;		/* new vbit, identifier */
	u_int vbits;		/* used vbits of address space */
 ...
 */
static void
hrm_setbits(struct hrmstat *hrm, caddr_t addr, u_int bits)
{
	u_int po, bo, spb;
	u_int nbits;
 ...
 */
void
hat_getstat(struct as *as, caddr_t addr, size_t len, u_int id,
	    caddr_t datap, int clearflag)
{
	...
	dp = datap;
	a = (caddr_t)((uintptr_t)addr & PAGEMASK);
	while (a < addr + len) {
		struct hrmstat	*hrm;
