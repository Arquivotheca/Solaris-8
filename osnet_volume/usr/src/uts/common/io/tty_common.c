...
#pragma ident	"@(#)tty_common.c	2.53	97/10/22 SMI"
...
ttycommon_ioctl(tty_common_t *tc, queue_t *q, mblk_t *mp, int *errorp)
{
	...
	*errorp = 0;	/* no error detected yet */
	...
	case TCSETSW:
	case TCSETS: {
		struct termios *cb = (struct termios *)mp->b_cont->b_rptr;
	...
	case TCSETAW:
	case TCSETA: {
		struct termio *cb = (struct termio *)mp->b_cont->b_rptr;
		/*
		 * The only information we look at are the iflag word
		 * and the cflag word.
	...
	case TIOCSWINSZ: {
		struct winsize *ws = (struct winsize *)mp->b_cont->b_rptr;
	...
	case TCGETA: {
		...
		if ((datap = allocb(sizeof (struct termio),
		    BPRI_HI)) == NULL) {
...
ttyinit()
{
	...
	if (ddi_prop_lookup_string_array(DDI_DEV_T_ANY, ddi_root_node(), 0,
		...
		if (ddi_prop_update_byte_array(DDI_DEV_T_NONE, dip,
		    property, (u_char *) &default_termios,
	...
	if (e_ddi_prop_update_byte_array(DDI_DEV_T_NONE, dip, property,
	    (u_char *) tp, sizeof (struct termios)) != DDI_PROP_SUCCESS) {
...
static int
termioval(char **sp, u_int *valp, char *ep)
{
	char *s = *sp;
	u_int digit;
