...
#pragma ident	"@(#)pcn.h	1.17	99/08/18 SMI"
...
#define	REG32(reg, off)		(uint32_t *)((reg) + off)
#define	REG16(reg, off)		(uint16_t *)((reg) + off)
#define	REG8(reg, off)		(uint8_t *)((reg) + off)
...
union PCN_RecvMsgDesc {
	struct {
		ulong_t rbadr;			/* buffer address */
		volatile ulong_t bcnt : 12;	/* buffer byte count */
		volatile ulong_t ones : 4;	/* must be ones */
		volatile ulong_t : 8;
		volatile ulong_t enp : 1;	/* end of packet */
		volatile ulong_t stp : 1;	/* start of packet */
		volatile ulong_t buff : 1;	/* buffer error */
		volatile ulong_t crc : 1;	/* CRC error */
		volatile ulong_t oflo : 1;	/* overflow error */
		volatile ulong_t fram : 1;	/* framing error */
		volatile ulong_t err : 1;	/* or'ing of error bits */
		volatile ulong_t own : 1;	/* owner */
		volatile ulong_t mcnt : 12;	/* message byte count */
		volatile ulong_t zeros : 4;	/* will be zeros */
		volatile ulong_t rpc : 8;	/* runt packet count */
		volatile ulong_t rcc : 8;	/* receive collision count */
		ulong_t reserved;
	} rmd_bits;
	ulong_t rmd[4];
};
...
union PCN_XmitMsgDesc {
	struct {
		ulong_t tbadr;			/* buffer address */
		volatile ulong_t bcnt: 12;	/* buffer byte count */
		volatile ulong_t ones : 4;	/* must be ones */
		volatile ulong_t : 8;
		volatile ulong_t enp : 1;	/* end of packet */
		volatile ulong_t stp : 1;	/* start of packet */
		volatile ulong_t def : 1;	/* deferred */
		volatile ulong_t one : 1;	/* one retry required */
		volatile ulong_t more : 1;	/* more than 1 retry required */
		volatile ulong_t add_fcs : 1;	/* add FCS */
		volatile ulong_t err : 1;	/* or'ing of error bits */
		volatile ulong_t own : 1;	/* owner */
		volatile ulong_t trc : 4;	/* transmit retry count */
		volatile ulong_t : 12;
		volatile ulong_t tdr : 10;	/* time domain reflectometry */
		volatile ulong_t rtry : 1;	/* retry error */
		volatile ulong_t lcar : 1; 	/* loss of carrier error */
		volatile ulong_t lcol : 1;	/* late collision error */
		volatile ulong_t exdef : 1;	/* excessive deferral */
		volatile ulong_t uflo : 1;	/* underflow */
		volatile ulong_t buff : 1;	/* buffer error */
		ulong_t reserved;
	} tmd_bits;
	ulong_t tmd[4];
...
struct PCN_IOmem {
	...
	ulong_t	pbase;		/* physical base address */
	ulong_t	pptr;		/* physical current pointer */
...
struct pcninstance {
	...
	int soft_phy;		/* The driver handles programming the PHY */
	enum mii_phy_state inphy_state;
	enum mii_phy_state exphy_state;
	int pcn_exphy_addr;	/* PHY address of external PHY */
	ulong_t curport; 	/* Last port selected. Required to set */
				/* PORTSEL after soft reset */
