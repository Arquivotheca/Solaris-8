...
#pragma ident	"@(#)pcide.c	1.34	99/10/28 SMI"
...
static int pcata_identify(dev_info_t *devi);
...
static int pcata_id(ata_soft_t *softp, ushort_t *buf);
...
static int
pcata_identify(dev_info_t *devi)
{
	char	*name = ddi_get_name(devi);

	if (strcmp(name, "pccard,disk") == 0)  {
#ifdef	ATA_DEBUG
		if (pcata_debug & DENT)
			cmn_err(CE_CONT, "_identify: pass '%s'\n", name);
#endif
		return (DDI_IDENTIFIED);
	}
#ifdef	ATA_DEBUG
	cmn_err(CE_CONT, "_identify: fail '%s'\n", name);
#endif
...
pcata_start(ata_unit_t *unitp, buf_t *bp, int blkno)
{
	...
	if (bp->b_private == (void *)0xBEE)
		pktp->cp_passthru = (uint32_t)bp->b_back;

#ifdef ATA_DEBUG
	if (pcata_debug & DIO) {
		cmn_err(CE_CONT, "passthru command seen: 0x%x\n",
			pktp->cp_passthru);
	}
#endif
...
pcata_go(ata_unit_t *unitp)
{
...
#ifdef ATA_DEBUGx
	if (pcata_debug & DENT) {
		cmn_err(CE_CONT, "_go (%x) status %x error %x\n", unitp,
			(void) csx_Get8(softp->handle, AT_ALTSTATUS),
	...
	csx_Put8(softp->handle, AT_DRVHD, ac_hd);
	csx_Put8(softp->handle, AT_DEVCTL, ac_devctl);
	csx_Put8(softp->handle, AT_SECT, ac_sec);
	csx_Put8(softp->handle, AT_COUNT, ac_count);
	csx_Put8(softp->handle, AT_LCYL, ac_lwcyl);
	csx_Put8(softp->handle, AT_HCYL, ac_hicyl);
	...
	if (pktp->ac_direction == AT_OUT) {
		if (pcata_send_data(unitp,
			min(pktp->cp_resid, pktp->ac_bytes_per_block))
			== DDI_FAILURE) {
...
pcata_iocmpl(ata_soft_t *softp)
{
	...
	if (pktp ==  NULL) {
		/* must have canceled an outstanding I/O request */
		cmn_err(CE_CONT, "_iocmpl No outstanding I/O\n");
		return;
	}
...
pcata_intr_hi(ata_soft_t *softp)
{
...
#ifdef ATA_DEBUG
	if (pcata_debug & DENT)
		cmn_err(CE_CONT, "_intr_hi softp = %p\n", (void *)softp);
#endif
	mutex_enter(&softp->hi_mutex);
	...
	/*
	 * Read the status and error registers,
	 * this clears an interrupt from the ata device
	 * even if we were not expecting one.
	 */
	 ...
	 */
	if ((status & ATS_BSY) == 0 && (softp->intr_pending == IPEND_HARD)) {
		rval = DDI_INTR_CLAIMED;

		softp->intr_pending = IPEND_SOFT;
...
#ifdef ATA_DEBUG
	if (pcata_debug & DENT)
		cmn_err(CE_CONT, "_intr_hi status=%x claimed=%d pending=%d\n",
			status,
			(rval == DDI_INTR_CLAIMED),
			softp->intr_pending);
#endif
...
pcata_intr(char *parm)
{
	...
	if (softp->intr_pending != IPEND_SOFT) {
		return (DDI_INTR_UNCLAIMED);
	}

#ifdef ATA_DEBUG
	if (pcata_debug & DENT)
		cmn_err(CE_CONT, "_intr entry (%p)\n", (void *)softp);
#endif
	...
	/*
	 * until now intr_hi is blocked because intr_pending is nonzero
	 * now we'll clear intr_pending and start new I/O
	 */
	mutex_enter(&softp->hi_mutex);
	softp->intr_pending = IPEND_NONE;
	mutex_exit(&softp->hi_mutex);
...
pcata_clear_queues(ata_unit_t *unitp)
{
	...
	for (pktp = softp->ab_head; pktp; pktp = pktp->pkt_forw) {
		/* XXX/lcl why isn't bioerror/biodone performed here */
...
#ifdef ATA_DEBUG
	cmn_err(CE_CONT, "_wait timeout: ival: 0x%x  eval: 0x%x\n",
		ival, csx_Get8(softp->handle, port));
#endif
	return (1);
}
...
pcata_get_data(ata_unit_t *unitp, int count)
{
	...
	csx_RepGet16(softp->handle, (ushort_t *)pktp->ac_v_addr,
		AT_DATA, count >> 1, DDI_DEV_NO_AUTOINCR);
#ifdef ATA_DEBUG
	if (pcata_debug & DIO)
...
pcata_getedt(ata_soft_t *softp, int dmax)
{
	...
	if (!(CARD_PRESENT_VALID(softp)))
		return (DDI_FAILURE);
	csx_Put8(softp->handle, AT_DEVCTL, ENABLE_INTERRUPT|AT_SRST|AT_NIEN);

	drv_usecwait(1000);
	if (!(CARD_PRESENT_VALID(softp)))
		return (DDI_FAILURE);

	csx_Put8(softp->handle, AT_DEVCTL, ENABLE_INTERRUPT|AT_NIEN);
	...
	for (dcount = drive = 0; drive < dmax; drive++) {
		 ...
		 */
		if (drive == 0) {
			csx_Put8(softp->handle, AT_DRVHD, ATDH_DRIVE0);
		} else {
			csx_Put8(softp->handle, AT_DRVHD, ATDH_DRIVE1);
		}
		csx_Put8(softp->handle, AT_FEATURE, 0);
...
static int
pcata_id(ata_soft_t *softp, ushort_t *buf)
{
	struct atarpbuf	*rpbp = (struct atarpbuf *)buf;

#ifdef ATA_DEBUG
	if (pcata_debug)
		cmn_err(CE_CONT, "**** id controller: 0x%x ",
			softp->ab_cmd-AT_CMD);
#endif

	/* command also known as IDENTIFY DEVICE */
	csx_Put8(softp->handle, AT_CMD, ATC_READPARMS);
	if (pcata_wait1(AT_STATUS, ATS_DRQ, ATS_BSY, 1000000, softp)) {
	 ...
	 */
	if (pcata_wait1(AT_STATUS, ATS_DRDY, ATS_BSY | ATS_DRQ, 500, softp))
		return (1);

	if (!CARD_PRESENT_VALID(softp))
		return (1);
	return (csx_Get8(softp->handle, AT_STATUS) & ATS_ERR);
}
 ...
 */
static uchar_t
pcata_drive_type(ata_soft_t *softp, ushort_t *secbuf)
{

	if (pcata_wait1(AT_STATUS,
	    (ATS_DRDY | ATS_DSC), (ATS_BSY | ATS_ERR), 100000, softp))
		return (ATA_DEV_NONE);

	if (pcata_id(softp, secbuf) != 0)
		return (ATA_DEV_NONE);
	return (ATA_DEV_DISK);
}
...
pcata_setpar(int drive, int heads, int sectors, ata_soft_t *softp)
{
	...
	if (!CARD_PRESENT_VALID(softp))
		return (DDI_FAILURE);
	csx_Put8(softp->handle, AT_DRVHD, (heads - 1) |
		(drive == 0 ? ATDH_DRIVE0 : ATDH_DRIVE1));
	csx_Put8(softp->handle, AT_COUNT, sectors);
...
pcata_set_rw_multiple(ata_soft_t *softp, int drive)
{
	 ...
	 * set drive number
	 */
	csx_Put8(softp->handle, AT_DRVHD, drive == 0 ? ATDH_DRIVE0 :
		ATDH_DRIVE1);
	 ...
	 * disable interrupt
	 */
	csx_Put8(softp->handle, AT_NIEN|ENABLE_INTERRUPT);
#endif
	...
	for (size = 32; size > 0 && accepted_size == -1 &&
		 ...
		 * send the command
		 */
		csx_Put8(softp->handle, AT_COUNT, size);
		csx_Put8(softp->handle, AT_CMD, ATC_SETMULT);
		if (pcata_wait(softp->ab_status, 0, ATS_BSY, softp))
			/*
			 * there should have been a drive here but it
			 * didn't respond properly. It stayed BUSY.
			 * complete failure!
		...
		for (i = 0; i < ATA_LOOP_CNT && CARD_PRESENT_VALID(softp);
			i++) {
			if (((laststat = csx_Get8(softp->handle, AT_STATUS))
				& (ATS_DRDY | ATS_ERR)) != 0)
				break;
			drv_usecwait(10);
		}
	...
	csx_Put8(softp->handle, AT_COUNT, accepted_size);
	csx_Put8(softp->handle, AT_CMD, ATC_SETMULT);
	if (pcata_wait(softp->ab_status, 0, ATS_BSY, softp))
		/*
		 * there should have been a drive here but it
	 ...
	 * disable interrupt
	 */
	outb(softp->ab_devctl, AT_NIEN|ENABLE_INTERRUPT);
#endif
...
pcata_iosetup(ata_unit_t *unitp, struct ata_cmpkt *pktp)
{
	...
	if (pktp->cp_passthru) {
		...
		case DADKIO_RWCMD_READ:
			pktp->ac_cmd = ATC_RDSEC;
			pktp->ac_direction = AT_IN;
			break;
		case DADKIO_RWCMD_WRITE:
			pktp->ac_cmd = ATC_WRSEC;
			pktp->ac_direction = AT_OUT;
			break;
		}
	} else {
...
pcata_spinup(ata_soft_t *softp, int slot)
{
	...
	csx_Put8(softp->handle, AT_COUNT, softp->ab_block_factor[0]);
	csx_Put8(softp->handle, AT_CMD, ATC_SETMULT);
