...
#pragma ident	"@(#)hubd.c	1.4	99/09/24 SMI"
...
static struct modldrv modldrv = {
	...
	"USB Hub Driver 1.4", /* Name of the module. */
	&hubd_ops,	/* driver ops */
};
...
_init(void)
{
	...
	if ((rval = ddi_soft_state_init(&hubd_state,
	    sizeof (hubd_t), HUBD_INITIAL_SOFT_SPACE)) != 0) {
		return (rval);
	}

	if ((rval = mod_install(&modlinkage)) != 0) {
		ddi_soft_state_fini(&hubd_state);
...
hubd_get_dip(dev_t dev)
{
	...
	hubd_t *hubd = ddi_get_soft_state(hubd_state, instance);
