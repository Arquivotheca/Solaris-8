 ...
 * All rights reserved.
 */
#pragma ident	"@(#)fdc.c	1.45	99/05/20 SMI"
...
static ddi_dma_lim_t fdc_dma_lim;
...
int fdc_exec(struct fdcntlr *, int, int);
    ...
    void *arg, void *result)
{
	...
	if (!(fcp = (struct fdcntlr *)ddi_get_driver_private(dip)))
		return (DDI_FAILURE);
	...
	case DDI_CTLOPS_INITCHILD:
	{
		...
		ddi_set_driver_private(udip, (caddr_t)fjp);
	...
	case DDI_CTLOPS_UNINITCHILD:
	{
		...
		fjp = (struct fcu_obj *)ddi_get_driver_private(udip);
...
{
	register struct fdcntlr *fcp;
	register int rval;

	switch (cmd) {
	case DDI_INFO_DEVT2DEVINFO:
		if (fcp = (struct fdcntlr *)ddi_get_soft_state(fdc_state_head,
		    (dev_t)arg)) {
			*result = fcp->c_dip;
			rval = DDI_SUCCESS;
			break;
...
static int
fdc_probe(register dev_info_t *dip)
{
	int	debug[2];
	int ioaddr;
	int	len;
...
fdc_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		...
		if (ddi_soft_state_zalloc(fdc_state_head, ctlr) != 0)
			return (DDI_FAILURE);
		fcp = (struct fdcntlr *)ddi_get_soft_state(fdc_state_head,
		    ctlr);
		...
		(void) ddi_dmae_getlim(dip, &fdc_dma_lim);
		...
		ddi_set_driver_private(dip, (caddr_t)fcp);
...
fdc_propinit2(struct fdcntlr *fcp, int cntlr)
{
	...
	if (ddi_prop_op(DDI_DEV_T_ANY, dip, PROP_LEN_AND_VAL_BUF,
		...
		(void) ddi_prop_create(DDI_DEV_T_NONE, dip, DDI_PROP_CANSLEEP,
		    "chip", (caddr_t)&(fcp->c_chip), (int)sizeof (int));
		/*
...
fdc_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	fcp = (struct fdcntlr *)ddi_get_driver_private(dip);
...
fdc_dkinfo(struct fcu_obj *fjp, struct dk_cinfo *dcp)
{
	...
	(void) strncpy((char *)&dcp->dki_dname, ddi_get_name(fjp->fj_dip),
		DK_DEVLEN);
    ...
    int sector, struct buf *bp)
{
	...
	csb->csb_dmawindow = NULL;
	csb->csb_dmasegment = NULL;
	dmalimits = fdc_dma_lim;
	dmalimits.dlim_granular = fjp->fj_chars->fdc_sec_size;

	if (ddi_dma_buf_setup(fcp->c_dip, bp, dmar_flags,
	    DDI_DMA_SLEEP, 0, &dmalimits, &csb->csb_dmahandle) != DDI_SUCCESS) {
		rval = EINVAL;
		goto out;
	}
...
out:
	if (csb->csb_dmahandle) {
		if (ddi_dma_free(csb->csb_dmahandle) != DDI_SUCCESS)
			cmn_err(CE_WARN, "fdrwbuf: dma free failed\n");
		csb->csb_dmahandle = NULL;
	}
	return (rval);
}
    ...
    int sector, caddr_t bufp, uint_t len)
{
	...
	if (ddi_dma_addr_setup(fcp->c_dip, NULL, bufp, len, dmar_flags,
	    DDI_DMA_SLEEP, 0, &dmalimits, &csb->csb_dmahandle) != DDI_SUCCESS) {
		rval = EINVAL;
		goto out;
	}
...
out:
	if (csb->csb_dmahandle) {
		if (ddi_dma_free(csb->csb_dmahandle) != DDI_SUCCESS)
			cmn_err(CE_WARN, "fdrw: dma free failed\n");
		csb->csb_dmahandle = NULL;
	}
	return (rval);
}
...
fdtrkformat(struct fcu_obj *fjp, int funit, int cyl, int head, int filldata)
{
	...
	*csb->csb_cmd = FO_FRMT | FO_MFM;
	 ...
	 */
	fmdatlen = 4 * numsctr;
	dp = fmdatp = (uchar_t *)kmem_alloc(fmdatlen, KM_SLEEP);
	...
	dmalimits = fdc_dma_lim;
	for (; dmalimits.dlim_granular < fmdatlen;
	    dmalimits.dlim_granular <<= 1);

	if (ddi_dma_addr_setup(fcp->c_dip, NULL, (caddr_t)fmdatp, fmdatlen,
	    DDI_DMA_WRITE, DDI_DMA_SLEEP, 0, &dmalimits, &csb->csb_dmahandle)
	    != DDI_SUCCESS) {
		rval = EINVAL;
		goto out;
	}
...
out:
	if (csb->csb_dmahandle) {
		if (ddi_dma_free(csb->csb_dmahandle) != DDI_SUCCESS)
			cmn_err(CE_WARN, "fdtrkformat: dma free failed\n");
		csb->csb_dmahandle = NULL;
	}
	kmem_free((caddr_t)fmdatp, fmdatlen);
	return (rval);
}
...
fdrawioctl(struct fcu_obj *fjp, int funit, caddr_t arg)
{
	...
	if (csb->csb_opflags & (CSB_OFDMARD | CSB_OFDMAWT)) {
		...
		if (ddi_dma_addr_setup(fcp->c_dip, NULL, fdrp->fdr_addr,
		    (uint_t)fdrp->fdr_nbytes, dmar_flags, DDI_DMA_SLEEP, 0,
		    &dmalimits, &csb->csb_dmahandle) != DDI_SUCCESS) {
			rval = EINVAL;
			goto out;
		}
	}
	...
	FCERRPRINT(FDEP_L1, FDEM_RAWI,
	    (CE_CONT, "nbytes: %x, opflags: %x, addr: %x, len: %x\n",
	    csb->csb_ncmds, csb->csb_opflags, (int)fdrp->fdr_addr,
	    fdrp->fdr_nbytes));
...
out:
	if (csb->csb_dmahandle) {
		if (ddi_dma_free(csb->csb_dmahandle) != DDI_SUCCESS)
			cmn_err(CE_WARN, "fdrawioctl: dma free failed\n");
		csb->csb_dmahandle = NULL;
	}
...
void
fdcquiesce(register struct fdcntlr *fcp)
{
	int unit;
...
fdc_dmae_isr(struct fdcntlr *fcp)
{
	...
	if (csb->csb_dmahandle && !csb->csb_cmdstat) {
		if (ddi_dma_nextseg(csb->csb_dmawindow, csb->csb_dmasegment,
		    &csb->csb_dmasegment) == DDI_SUCCESS &&
		    ddi_dma_segtocookie(csb->csb_dmasegment, &off, &len,
		    &csb->csb_dmacookie) == DDI_SUCCESS)
			/*
			 * read/write operation requires another DMA cookie:
			 *  process next one
			 */
...
fdc_exec(struct fdcntlr *fcp, int sleep, int change)
{
	...
	if (csb->csb_dmahandle) {
		if ((csb->csb_dmawindow || ddi_dma_nextwin(csb->csb_dmahandle,
		    csb->csb_dmawindow, &csb->csb_dmawindow) != DDI_SUCCESS) ||
		    ddi_dma_nextseg(csb->csb_dmawindow, csb->csb_dmasegment,
		    &csb->csb_dmasegment) != DDI_SUCCESS ||
		    ddi_dma_segtocookie(csb->csb_dmasegment, &off, &len,
		    &csb->csb_dmacookie) != DDI_SUCCESS ||
...
retry:
	...
	if (csb->csb_dmahandle) {
		bzero((caddr_t)&dmaereq, sizeof (struct ddi_dmae_req));
	...
	if (csb->csb_cmdstat && *csb->csb_cmd != FO_SDRV) {
		...
		if (csb->csb_xstate == FXS_END)
			csb->csb_xstate = FXS_START;
		if (fdc_dma_lim.dlim_sgllen > 1 &&
		    csb->csb_dmahandle && csb->csb_dmawindow &&
			/*
			 * restarted read/write operation requires
			 * first DMA cookie of current window
			 */
		    (ddi_dma_nextseg(csb->csb_dmawindow, NULL,
		    &csb->csb_dmasegment) != DDI_SUCCESS ||
		    ddi_dma_segtocookie(csb->csb_dmasegment, &off, &len,
		    &csb->csb_dmacookie) != DDI_SUCCESS)) {
			mutex_exit(&fcp->c_lock);
...
{
	register struct fdcntlr *fcp = (struct fdcntlr *)arg;
	register struct fdcsb *csb;
	off_t blklen, off;
	int drive;
	int newstate;
	...
	/*
	 * the following is a crude attempt to determine that an
	 * interrupt is really pending on this controller
	 */
	if ((pendstate == (MS_RQM | MS_DIO | MS_CB) || pendstate == MS_RQM) &&
		...
		if (newstate == FXS_END) {
			...
			if (csb->csb_dmahandle && !csb->csb_cmdstat &&
				 ...
				 */
			    ((rval = ddi_dma_nextseg(csb->csb_dmawindow,
			    csb->csb_dmasegment, &csb->csb_dmasegment)) ==
			    DDI_SUCCESS ||
			    ((rval = ddi_dma_nextwin(csb->csb_dmahandle,
			    csb->csb_dmawindow, &csb->csb_dmawindow)) ==
			    DDI_SUCCESS &&
			    (rval = ddi_dma_nextseg(csb->csb_dmawindow, NULL,
			    &csb->csb_dmasegment)) == DDI_SUCCESS))) {
				/*
				 * read/write operation requires another
				 * DMA cookie: process next one
				 */
				if (ddi_dma_segtocookie(csb->csb_dmasegment,
				    &off, &blklen, &csb->csb_dmacookie) !=
				    DDI_SUCCESS)
					cmn_err(CE_WARN,
					"fdc_intr: dma segtocookie failed\n");
