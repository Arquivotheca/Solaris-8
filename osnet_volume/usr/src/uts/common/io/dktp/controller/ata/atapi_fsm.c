...
#pragma ident "@(#)atapi_fsm.c	1.3	99/02/17 SMI"
...
atapi_fsm_error(
	...
	uchar_t	   event)
{
	ADBG_ERROR(("atapi protocol error: 0x%x 0x%x 0x%x\n",
	...
	ata_pkt_t	*ata_pktp)
{
	...
	if (ata_pktp->ap_pciide_dma) {
		 ...
		 * DMA but no Overlap
		 */
		ddi_putb(io_hdl1, ata_ctlp->ac_feature, ATF_ATAPI_DMA);
	...
	ADBG_WARN(("atapi_start_cmd: 0x%x status 0x%x error 0x%x\n",
		ata_pktp->ap_cmd,
		ddi_getb(io_hdl2,  ata_ctlp->ac_altstatus),
	...
	ata_pkt_t	*ata_pktp)
{
	 ...
	 */
	ddi_rep_putw(io_hdl1, (ushort_t *)ata_pktp->ap_cdbp, ata_ctlp->ac_data,
	...
	while (padding) {
		ddi_putw(io_hdl1, ata_ctlp->ac_data, 0);
		padding--;
	}
	...
	ata_pkt_t	*ata_pktp)
{
	 ...
	 */
	drive_bytes = ((int)ddi_getb(io_hdl1, ata_ctlp->ac_hcyl) << 8)
			+ ddi_getb(io_hdl1, ata_ctlp->ac_lcyl);
	...
	if (xfer_words) {
		...
		ddi_rep_getw(io_hdl1, (ushort *)ata_pktp->ap_v_addr,
	...
	if (xfer_bytes & 1) {
		...
		tmp_word = ddi_getw(io_hdl1, ata_ctlp->ac_data);
	...
	if (drive_bytes > 0) {
		...
		for (; drive_bytes > 0; drive_bytes -= 2)
			(void) ddi_getw(io_hdl1, ata_ctlp->ac_data);
	}
	...
	ata_pkt_t	*ata_pktp)
{
	...
	if (xfer_words) {
		...
		ddi_rep_putw(io_hdl1, (ushort *)ata_pktp->ap_v_addr,
			ata_ctlp->ac_data, xfer_words, DDI_DEV_NO_AUTOINCR);
		ata_pktp->ap_v_addr += byte_count;
	}
	...
	if (xfer_bytes & 1) {
		...
		ddi_putw(io_hdl1, ata_ctlp->ac_data, tmp_word);
	}
	...
	ata_drv_t	*ata_drvp)
{
	...
	ddi_putb(io_hdl1, ata_ctlp->ac_cmd, ATC_DEVICE_RESET);
	...
	ata_pkt_t	*ata_pktp)
{
	...
	ADBG_TRANSPORT(("atapi_start: pkt = 0x%x\n", (uint_t)ata_pktp));
	...
	ata_pkt_t	*ata_pktp)
{
	 ...
	 */
	intr_reason = ddi_getb(io_hdl1, ata_ctlp->ac_count);
