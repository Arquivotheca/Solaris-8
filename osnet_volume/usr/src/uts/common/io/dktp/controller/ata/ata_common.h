...
#pragma ident	"@(#)ata_common.h	1.39	99/07/14 SMI"
...
typedef struct ata_ctl {
	...
	ulong_t		 ac_sg_paddr;	  /* phys addr of S/G list */
...
/* 					OFFSET COMMENT			*/
	...
	ushort_t  ai_remcyls;	  /*   2  # of removable cylinders	*/
	...
	ushort_t ai_padding1[35];	  /* pad to 125 */
	...
	ushort_t ai_padding2[96]; /* pad to 255 */
};
...
typedef struct ata_pkt {
	...
	ulong_t		ap_boffset;	/* current offset into I/O buffer */
	ulong_t		ap_bcount;	/* # bytes in this request */

	caddr_t		ap_v_addr;	/* I/O buffer address */
	ulong_t		ap_resid;	/* # bytes left to read/write */
	...
	ulong_t		ap_resid_sav;	/* Original # of bytes */
					/* left to read/write. */
...
#define	ATA_DELAY_400NSEC(H, A)					\
	((void) ddi_io_getb((H), (uint8_t *)(A) + AT_ALTSTATUS), 	\
	(void) ddi_io_getb((H), (uint8_t *)(A) + AT_ALTSTATUS),	\
	(void) ddi_io_getb((H), (uint8_t *)(A) + AT_ALTSTATUS),	\
	(void) ddi_io_getb((H), (uint8_t *)(A) + AT_ALTSTATUS))
#else
