...
#pragma ident "@(#)atapi.c	1.45	99/06/21 SMI"
...
static	void	atapi_tran_tgt_free(dev_info_t *hba_dip, dev_info_t *tgt_dip,
			scsi_hba_tran_t *hba_tran, struct scsi_device *sd);
static	int	atapi_tran_abort(struct scsi_address *ap,
			struct scsi_pkt *spktp);
static	int	atapi_tran_reset(struct scsi_address *ap, int level);
static	int	atapi_tran_getcap(struct scsi_address *ap, char *capstr,
			int whom);
...
static	struct scsi_pkt	*atapi_tran_init_pkt(struct scsi_address *ap,
			struct scsi_pkt *spktp, struct buf *bp,
			int cmdlen, int statuslen,
			int tgtlen, int flags,
			int (*callback)(caddr_t), caddr_t arg);
static	void	atapi_tran_destroy_pkt(struct scsi_address *ap,
			struct scsi_pkt *spktp);
static	void	atapi_tran_dmafree(struct scsi_address *ap,
			struct scsi_pkt *spktp);
static	void	atapi_tran_sync_pkt(struct scsi_address *ap,
			struct scsi_pkt *spktp);
static	int	atapi_tran_start(struct scsi_address *ap,
			struct scsi_pkt *spktp);
 ...
 * packet callbacks
 */
static	void	atapi_complete(ata_pkt_t *ata_pktp, int do_callback);
	...
	caddr_t ioaddr)
{
	...
	if (ddi_getb(io_hdl, (uchar_t *)ioaddr + AT_HCYL) == ATAPI_SIG_HI &&
		ddi_getb(io_hdl, (uchar_t *)ioaddr + AT_LCYL) != ATAPI_SIG_LO)
		rc = TRUE;
	 ...
	 * data for.
	 */
	ddi_putb(io_hdl, (uchar_t *)ioaddr + AT_HCYL, 0);
	ddi_putb(io_hdl, (uchar_t *)ioaddr + AT_LCYL, 0);
	...
	caddr_t		 arg)
{
	...
	if (GTGTP2ATADRVP(ADDR2GTGTP(ap))->ad_pciide_dma == FALSE) {
		/* the drive doesn't support DMA */
		goto skip_dma_setup;
	}
	...
	int do_callback)
{
	...
	if (ata_pktp->ap_flags & AP_ARQ_ERROR) {
		ADBG_ARQ(("atapi_complete ARQ error 0x%x\n", ata_pktp));
		spktp->pkt_reason = CMD_TRAN_ERR;
		...
		ADBG_ARQ(("atapi_complete ARQ okay 0x%x\n", ata_pktp));
	...
	ata_pkt_t	*ata_pktp)
{
	 ...
	 */
	ddi_putb(io_hdl1, (uchar_t *)ioaddr1 + AT_DRVHD,
