...
#pragma ident "@(#)ata_dma.c	1.4	99/06/21 SMI"
...
#define	PCIIDE_STATUS_GET(hdl, addr)	\
	ddi_getb((hdl), ((uchar_t *)(addr) + PCIIDE_BMISX_REG))
	...
	ata_ctl_t *ata_ctlp)
{
	...
	if (rc != DDI_SUCCESS) {
		ADBG_ERROR(("ata_pciide_alloc 0x%x handle %d\n",
			    ata_ctlp, rc));
		goto err3;
	}
	...
	if (rc != DDI_SUCCESS) {
		ADBG_ERROR(("ata_pciide_alloc 0x%x mem %d\n",
			    ata_ctlp, rc));
		goto err2;
	}
	...
	if (rc != DDI_DMA_MAPPED) {
		ADBG_ERROR(("ata_pciide_alloc 0x%x bind %d\n", ata_ctlp, rc));
		goto err1;
	}
	...
	int	   sg_cnt)
{
	...
	uint		*dstp = (uint *)ata_ctlp->ac_sg_list;
	int		 idx;

	ASSERT(dstp != 0);
	ASSERT(sg_cnt != 0);

	ADBG_DMA(("ata dma_setup 0x%x 0x%x %d\n", ata_ctlp, srcp, sg_cnt));
	/*
	...
	for (idx = 0; idx < sg_cnt - 1; idx++, srcp++) {
		ddi_putl(sg_acc_handle, dstp++, srcp->p_address);
		ddi_putl(sg_acc_handle, dstp++, srcp->p_count);
	}
	 ...
	 */
	ddi_putl(bmhandle, (uint *)(bmaddr + PCIIDE_BMIDTPX_REG),
		ata_ctlp->ac_sg_paddr);

	ADBG_DMA(("ata dma_setup 0x%x 0x%x %d\n",
		bmaddr, ata_ctlp->ac_sg_paddr));
}
	...
	uchar_t direction)
{
	 ...
	 */
	tmp = ddi_getb(bmhandle, (uchar_t *)bmaddr + PCIIDE_BMICX_REG);
	tmp &= PCIIDE_BMICX_MASK;
	ddi_putb(bmhandle, (uchar_t *)bmaddr + PCIIDE_BMICX_REG,
		(tmp |  direction));
	...
	ata_ctl_t *ata_ctlp)
{
	...
	ADBG_DMA(("ata_pciide_dma_stop 0x%x 0x%x\n", bmaddr, tmp));

	ddi_putb(bmhandle, (uchar_t *)bmaddr + PCIIDE_BMICX_REG, tmp);
}
	...
	int	 seg_index)
{
	...
	ASSERT(((ushort)dmackp->dmac_size & PCIIDE_PRDE_CNT_MASK) == 0);
	ASSERT((ushort)dmackp->dmac_size <= PCIIDE_PRDE_CNT_MAX);

	ADBG_TRACE(("adp_dma_sg_func: gcmdp 0x%x dmackp 0x%x s %d idx %d\n",
	...
	ata_ctl_t *ata_ctlp)
{
	...
	ADBG_DMA(("ata_pciide_status_clear 0x%x 0x%x\n",
		bmaddr, status));

	/*
	 * Clear the latches (and preserve the other bits)
	 */
	ddi_putb(bmhandle, (uchar_t *)bmaddr + PCIIDE_BMISX_REG, tmp);

#ifdef NAT_SEMI_PC87415_BUG
	/* ??? chip errata ??? */
	if (ata_ctlp->ac_nat_semi_bug) {
		tmp = ddi_getb(bmhandle, bmaddr + PCIIDE_BMICX_REG);
		tmp &= PCIIDE_BMICX_MASK;
		ddi_putb(bmhandle, bmaddr + PCIIDE_BMICX_REG,
			(tmp | PCIIDE_BMISX_IDERR | PCIIDE_BMISX_IDEINTS));
	}
#endif
	return (status);
	...
	ata_ctl_t *ata_ctlp)
{
	...
	ADBG_DMA(("ata_pciide_status_dmacheck_clear 0x%x 0x%x\n",
	...
	ata_ctl_t *ata_ctlp)
{
	...
	status = PCIIDE_STATUS_GET(ata_ctlp->ac_bmhandle, ata_ctlp->ac_bmaddr);
	ADBG_DMA(("ata_pciide_status_pending 0x%x 0x%x\n",
		ata_ctlp->ac_bmaddr, status));
