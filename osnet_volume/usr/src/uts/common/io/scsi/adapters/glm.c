...
#pragma ident	"@(#)glm.c	1.153	99/11/12 SMI"

/*
 * glm - Symbios 53c810a and 53c875 SCSI Processor HBA driver.
 */
...
static int glm_quiesce_bus(dev_info_t *dip);
static int glm_unquiesce_bus(dev_info_t *dip);
...
static struct modldrv modldrv = {
	...
	"GLM SCSI HBA Driver 1.153.",   /* Name of the module. */
	&glm_ops,	/* driver ops */
};
...
glm_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	hba_tran->tran_quiesce		= glm_quiesce_bus;
	hba_tran->tran_unquiesce	= glm_unquiesce_bus;
...
glm_config_space_init(struct glm *glm)
{
	 ...
	 *	f - 53c875
	 */
	glm->g_devid = pci_config_get16(config_handle, PCI_CONF_DEVID);
	...
	glm->g_revid >>= 4;
	glm->g_revid |= (pci_config_get8(config_handle, PCI_CONF_REVID) & 0xf0);
...
glm_restart_cmd(void *arg)
{
	...
	for (i = 0; i < N_GLM_UNITS; i += NLUNS_GLM) {
...
glm_check_intcode(glm_t *glm, glm_unit_t *unit, uint_t action)
{
	...
	case NINT_IWR:
	{
		...
		if (index < 0 || index > cmd->cmd_cookiec) {
			glm_log(glm, CE_WARN, "IWR index out of range");
			return (NACTION_DO_BUS_RESET | NACTION_ERR | action);
		}
	...
	if (unit != NULL) {
		...
		glm_log(glm, CE_WARN, "Resetting scsi bus, %s.",
		    errmsg);
	}
	return (NACTION_DO_BUS_RESET | NACTION_ERR);
}
...
glm53c87x_save_byte_count(glm_t *glm, glm_unit_t *unit)
{
	...
	if (index < 1 || index > unit->nt_ncmdp->cmd_cookiec) {
...
/* ARGSUSED */
static int
glm_quiesce_bus(dev_info_t *dip)
{
...
/* ARGSUSED */
static int
glm_unquiesce_bus(dev_info_t *dip)
{
