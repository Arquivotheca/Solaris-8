...
#pragma	ident "@(#)vuidm4p.c	1.4	94/10/25 SMI"
...
unsigned char vuidm4p_buf[VBUF_SIZE+1];
int vuidm4p_ptr = 0;
#endif
...
#define	MOUSE_BUTTON_M		(unchar)(0x40)	/* Middle button */
#define	MOUSE_BUTTON_L		(unchar)(0x20)	/* Left   button */
#define	MOUSE_BUTTON_R		(unchar)(0x10)	/* Right  button */
...
vuidm4p(queue_t *const qp, mblk_t *mp)
{
	...
	r = mp->b_wptr - mp->b_rptr;
		...
		/* We've seen delta Y.  If we do NOT have the sync bit set, this
		 * indicates the middle button's status.
		 */
		case MOUSE_DELTA_Y:
			STATEP->state = MOUSE_START;
			...
			if (STATEP->oldbuttons != STATEP->buttons) {
				VUID_PUTNEXT(qp, MS_MIDDLE, FE_PAIR_NONE, 0,
				  ((STATEP->buttons & MOUSE_BUTTON_M) ? 1 : 0));
