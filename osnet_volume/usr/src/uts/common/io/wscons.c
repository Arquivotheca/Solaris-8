...
#pragma ident	"@(#)wscons.c	1.72	99/05/04 SMI"
...
#ifndef DPRINTF
#ifdef DEBUG
void	wc_dprintf(const char *fmt, ...);
...
#if	defined(_CONSOLE_OUTPUT_VIA_SOFTWARE)
/*
 * The kernel really should supply basic functions like this...
 */
static void *
...
wcioctl(queue_t *q, mblk_t *mp)
{
	...
	case WC_OPEN_FB:
		 ...
		 * for ioc_cr == kcred.)
		 */
		if ((iocp->ioc_error = drv_priv(iocp->ioc_cr)) != 0)
			goto open_fail;
...
close_fail:
	...
	default:
		 ...
		 */
		datasize =
			ttycommon_ioctl(&wscons.wc_ttycommon, q, mp, &error);
		if (datasize != 0) {
			if (wscons.wc_bufcallid != 0)
				qunbufcall(q, wscons.wc_bufcallid);
