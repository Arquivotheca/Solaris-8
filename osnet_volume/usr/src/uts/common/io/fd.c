...
#pragma ident	"@(#)fd.c	1.49	99/09/10 SMI"
 ...
 */
int fd_unit_is_open(struct fdisk *);
static int fdgetlabel(struct fcu_obj *, int);
static void fdstart(register struct fcu_obj *);
...
{
	if (fdpp) {
		*fdpp = (struct fdisk *)ddi_get_soft_state(fd_state_head,
		    DRIVE(dev));
		if (*fdpp && fjpp) {
			*fjpp = (*fdpp)->d_obj;
			if (*fjpp)
...
static int
fd_probe(register dev_info_t *dip)
{
#ifdef CMOS_CONF_MEM
	int cmos;
	int drive_type;
...
fd_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		...
		if (ddi_soft_state_zalloc(fd_state_head, drive_num) != 0)
			return (DDI_FAILURE);
		fdp = (struct fdisk *)ddi_get_soft_state(fd_state_head,
		    drive_num);
		fjp = fdp->d_obj = (struct fcu_obj *)
		...
		*fjp->fj_attr = fdtypes[fdp->d_deffdtype];
		bcopy((caddr_t)fdparts[fdp->d_deffdtype], (caddr_t)fdp->d_part,
	...
	case DDI_RESUME:
		drive_num = ddi_get_instance(dip);
		if (!(fdp = (struct fdisk *)ddi_get_soft_state(fd_state_head,
		    drive_num)))
			return (DDI_FAILURE);
...
fdgetlabel(struct fcu_obj *fjp, int unit)
{
	...
	register struct fdisk *fdp;
	char *newlabel;
	short *sp;
	short count;
	short xsum;
	int nexttype, tries, try_this;
	int rval;
	short oldlvl;
	...
	FDERRPRINT(FDEP_L0, FDEM_GETL, (CE_CONT,
	    "fdgetlabel fd unit %d kmem_zalloc: ptr = %x, size = %x\n",
	    unit, (int)label, sizeof (struct dk_label)));
	...
	for (tries = nfdtypes; tries; tries--) {
		...
		if ((1 << fdp->d_curfdtype) & fdp->d_media) {
			...
			*fjp->fj_attr = fdtypes[fdp->d_curfdtype];
			bcopy((caddr_t)fdparts[fdp->d_curfdtype],
			    (caddr_t)fdp->d_part,
			    sizeof (struct partition) * NDKMAP);
			/*
	...
	bcopy((caddr_t)label->dkl_vtoc.timestamp,
	    (caddr_t)fdp->d_vtoc_timestamp, sizeof (fdp->d_vtoc_timestamp));
	bcopy((caddr_t)label->dkl_vtoc.v_volume, (caddr_t)fdp->d_vtoc_volume,
	    LEN_DKL_VVOL);
	bcopy((caddr_t)label->dkl_vtoc.v_asciilabel,
	    (caddr_t)fdp->d_vtoc_asciilabel, LEN_DKL_ASCII);
	/*
	 * logical partitions
	 */
	bcopy((caddr_t)label->dkl_vtoc.v_part, (caddr_t)fdp->d_part,
	    sizeof (struct partition) * NDKMAP);
...
nolabel:
	...
	bzero((caddr_t)fdp->d_vtoc_volume, LEN_DKL_VVOL);
...
fd_strategy(register struct buf *bp)
{
	...
	FDERRPRINT(FDEP_L1, FDEM_STRA,
		(CE_CONT, "fd_strategy: bp = 0x%p, dev = 0x%x\n",
		(void*)bp, (int)bp->b_edev));
	...
	if (fjp->fj_chars->fdc_sec_size > NBPSCTR && (bp->b_blkno & 1))  {
		FDERRPRINT(FDEP_L3, FDEM_STRA,
		    (CE_WARN, "fd%d: block %d is not start of sector!",
		    DRIVE(bp->b_edev), bp->b_blkno));
		bp->b_error = EINVAL;
		goto bad;
	}
	...
	if ((bp->b_blkno > pp->p_size)) {
		FDERRPRINT(FDEP_L3, FDEM_STRA,
		    (CE_WARN, "fd%d: block %d is past the end! (nblk=%ld)",
		    DRIVE(bp->b_edev), bp->b_blkno, pp->p_size));
		bp->b_error = ENOSPC;
		goto bad;
	}
 ...
 */
static void
fdstart(register struct fcu_obj *fjp)
{
	struct buf *bp;
	...
	while ((bp = fdp->d_actf) != NULL) {
		...
		FDERRPRINT(FDEP_L0, FDEM_STRT,
		    (CE_CONT, "fdstart: bp=0x%x blkno=0x%x bcount=0x%lx\n",
...
"  blk 0x%x addr 0x%x len 0x%x cyl %d head %d sec %d\n  resid 0x%lx, tlen %d\n",
			    blk, (int)addr, len, cyl, head, sect,
			    bp->b_resid, tlen));
...
get_geom:
	...
	case DKIOCSGEOM:
		if (ddi_copyin((caddr_t)arg, (caddr_t)&cpy.dkg,
	...
	case DKIOCGAPART:
		...
		for (part = 0; part < NDKMAP; part++) {
			if ((fdp->d_part[part].p_start % nblks) != 0)
				return (EINVAL);
			dmp->dkl_cylno = fdp->d_part[part].p_start / nblks;
			dmp->dkl_nblk = fdp->d_part[part].p_size;
			dmp++;
		}

		if (ddi_copyout((caddr_t)&cpy.dka, (caddr_t)arg,
	...
	case DKIOCGVTOC:
		...
		mutex_exit(&fjp->fj_lock);
		if (ddi_copyout((caddr_t)&vtoc, (caddr_t)arg, sizeof (vtoc),
		    flag))
			rval = EFAULT;
		break;

	case DKIOCSVTOC:
		if (ddi_copyin((caddr_t)arg, (caddr_t)&vtoc, sizeof (vtoc),
		    flag)) {
			rval = EFAULT;
			break;
		}
	...
	case DKIOCSTATE:
		...
		if (ddi_copyout((caddr_t)&fdp->d_media_state, (caddr_t)arg,
		    sizeof (int), flag))
			rval = EFAULT;
		break;
	...
	case F_DOR:
		 ...
		 *
		 */
		if (!((*(char *)&arg) & FD_RSETZ))
			fjp->fj_ops->fco_abort(fjp);
		break;

	case F_FCR:
/*
 *		FD	Drive/Media	RPM	transfer	    transfer
 *					     rate(kbytes/sec)	      code
 *		5.25	1.2/1.2		360	 500			0
 *		5.25	1.2/360		360	 300			1
 *		5.25	360/360		300	 250			2
 *		3.5	x.xx/2.88	300	1000			3
 *		3.5	x.xx/1.44	300	 500			0
 *		3.5	x.xx/720	300	 250			2
 */
		if (arg == 0) t_rate = 500;
		else if (arg == 1) t_rate = 300;
		else if (arg == 2) t_rate = 250;
		else if (arg == 3) t_rate = 1000;
		else {
			rval = EFAULT;
			break;
		}
		mutex_enter(&fjp->fj_lock);
		fjp->fj_chars->fdc_transfer_rate = t_rate;
		fdp->d_curfdtype = -1;
		fjp->fj_flags &= ~FUNIT_CHAROK;
		mutex_exit(&fjp->fj_lock);
		break;
	...
	case F_DTYP:
		part = fdp->d_deffdtype;

		if (part == FMT_5H) cpy.temp = DRV_QUAD;
		else if (part == FMT_5D9) cpy.temp = DRV_DBL;
		else if (part == FMT_3E) cpy.temp = DRV_144; /* XXX DRV_288 */
		else if (part == FMT_3H) cpy.temp = DRV_144;
		else if (part == FMT_3D) cpy.temp = DRV_720;
		else {
			cpy.temp = DRV_NONE;
			rval = EFAULT;
		}
		if (ddi_copyout((caddr_t)&cpy.temp, (caddr_t)arg,
		    sizeof (cpy.temp), flag))
			rval = EFAULT;
		break;
#endif

	case FDIOGCHAR:
		if (ddi_copyout((caddr_t)fjp->fj_chars, (caddr_t)arg,
...
(CE_WARN, "fd_ioctl fd unit %d: FDIOSCHAR odd xfer rate %dkbs",
	...
	case FDDEFGEOCHAR:
		...
		*fjp->fj_attr = fdtypes[fdp->d_curfdtype];
		bcopy((caddr_t)fdparts[fdp->d_curfdtype],
		    (caddr_t)fdp->d_part, sizeof (struct partition) * NDKMAP);
		fjp->fj_flags &= ~FUNIT_CHAROK;
		mutex_exit(&fjp->fj_lock);
		break;
	...
	case FDGETDRIVECHAR:
		if (ddi_copyout((caddr_t)fjp->fj_drive, (caddr_t)arg,
	...
	{
		struct fd_cmd fc;
		register int cyl, head, spc, spt;

		if (ddi_copyin((caddr_t)arg, (caddr_t)&fc, sizeof (fc), flag)) {
			rval = EFAULT;
			break;
		}
...
fd_build_user_vtoc(struct fcu_obj *fjp, struct fdisk *fdp, struct vtoc *vtocp)
{
	 ...
	 *
	 */
	bzero((caddr_t)vtocp, sizeof (struct vtoc));

	bcopy((caddr_t)fdp->d_vtoc_bootinfo,
	    (caddr_t)vtocp->v_bootinfo, sizeof (vtocp->v_bootinfo));

	vtocp->v_sanity = VTOC_SANE;
	vtocp->v_version = fdp->d_vtoc_version;
	bcopy((caddr_t)fdp->d_vtoc_volume, (caddr_t)vtocp->v_volume,
	    LEN_DKL_VVOL);
	if (fjp->fj_flags & FUNIT_LABELOK) {
	 ...
	 */
	bcopy((caddr_t)fdp->d_part, (caddr_t)vtocp->v_part,
	    sizeof (struct partition) * NDKMAP);
	...
	bcopy((caddr_t)fdp->d_vtoc_timestamp,
	    (caddr_t)vtocp->timestamp, sizeof (fdp->d_vtoc_timestamp));
	bcopy((caddr_t)fdp->d_vtoc_asciilabel, (caddr_t)vtocp->v_asciilabel,
	    LEN_DKL_ASCII);
}
    ...
    struct dk_label *labelp)
{
	...
	bcopy((caddr_t)vtocp->v_bootinfo, (caddr_t)fdp->d_vtoc_bootinfo,
	    sizeof (vtocp->v_bootinfo));
	fdp->d_vtoc_version = vtocp->v_version;
	bcopy((caddr_t)vtocp->v_volume, (caddr_t)fdp->d_vtoc_volume,
	    LEN_DKL_VVOL);

	/*
	 * Copy partitioning information.
	 */
	bcopy((caddr_t)vtocp->v_part, (caddr_t)fdp->d_part,
	    sizeof (struct partition) * NDKMAP);

	bcopy((caddr_t)vtocp->timestamp, (caddr_t)fdp->d_vtoc_timestamp,
	    sizeof (fdp->d_vtoc_timestamp));

	bcopy((caddr_t)vtocp->v_asciilabel, (caddr_t)fdp->d_vtoc_asciilabel,
	    LEN_DKL_ASCII);
	...
	bcopy((caddr_t)vtocp, (caddr_t)&labelp->dkl_vtoc,
	    sizeof (struct vtoc));
...
fd_rawioctl(struct fcu_obj *fjp, int unit, caddr_t arg, int mode)
{
	...
	case FDRAW_FORMAT:
		...
		if (ddi_copyin(uaddr, fdr.fdr_addr, (uint_t)fdr.fdr_nbytes,
		    mode)) {
	...
	if (ddi_copyout(fdr.fdr_result, ((struct fd_raw *)arg)->fdr_result, 10,
	    mode))
		rval = EFAULT;
    ...
    char *name, caddr_t valuep, int *lengthp)
{
	if (strcmp(name, "nblocks") == 0) {
		...
		FDERRPRINT(FDEP_L1, FDEM_PROP,
		    (CE_CONT, "fd_prop_op: dip %lx nblocks\n", (ulong_t)dip));
		...
		lp = &fdp->d_part[PARTITION(dev)];
		nblocks = (int)lp->p_size;
