...
#ident	"@(#)kbtrans.c	1.12	99/08/19 SMI"
 ...
 *
 * It is believed that it is safe to call
 * these functions within debugger mode
 * except kbtrans_dprintf.  Debugger mode is a single
 * threaded mode where most kernel services are
 * not available, including memory allocation.  Debugger mode is for
 * kadb and OBP debugging, where the debugger calls back into the kernel
 * to obtain console input.
 *
