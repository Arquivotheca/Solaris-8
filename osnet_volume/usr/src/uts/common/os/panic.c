...
#pragma ident	"@(#)panic.c	1.5	99/11/24 SMI"
...
panicsys(const char *format, va_list alist, struct regs *rp, int on_panic_stack)
{
	 ...
	 */
	if (cp->cpu_on_intr && cp->cpu_intr_thread != NULL) {
		kthread_t *it = cp->cpu_intr_thread;

		intr_stack = cp->cpu_intr_stack;
		on_intr = cp->cpu_on_intr;
...
/*
 * Panic_hook is no longer used.  A simple delay function is left here so
 * that DiskSuite drivers can continue to load until we get them fixed.
 */
void
panic_hook(void)
{
