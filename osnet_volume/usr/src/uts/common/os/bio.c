...
#pragma ident	"@(#)bio.c	1.99	99/09/22 SMI"	/* SVr4.0 1.29  */
...
loop:
	 ...
	 * B_KERNBUF | B_NOCACHE | B_AGE | B_ERROR | B_DONE to assure the
	 * following:
...
biowait(struct buf *bp)
{
	...
	cpup = CPU;
	atomic_add_32((uint32_t *)&cpup->cpu_stat.cpu_syswait.iowait, 1);
	...
	if (flags & B_READ) {
		...
		if (pp != NULL && pp->p_vnode != NULL) {
			if (IS_SWAPVP(pp->p_vnode)) {
				CPU_STAT_ADDQ(cpup, cpu_vminfo.anonpgin,
						btopr(len));
			} else {
				if (pp->p_vnode->v_flag & VVMEXEC) {
					CPU_STAT_ADDQ(cpup, cpu_vminfo.execpgin,
							btopr(len));
				} else {
					CPU_STAT_ADDQ(cpup, cpu_vminfo.fspgin,
							btopr(len));
				}
			}
		}
