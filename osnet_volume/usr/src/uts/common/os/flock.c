...
#pragma ident	"@(#)flock.c	1.28	99/07/26 SMI"
...
static void unlock_lockmgr_granted();
static void wakeup_sleeping_lockmgr_locks();
...
flk_wait_execute_request(lock_descriptor_t *request)
{
	...
	if (nlm_status_size == 0) {	/* not booted as a cluster */
		...
		/*
		 * If the request is an NLM server lock request, and the NLM
		 * state of the lock request is not NLM_UP (because the
		 * NLM server is shutting down), then cancel the sleeping lock
		 * and return error ENOLCK that will encourage the client to
		 * retransmit.
		 */
		if (IS_LOCKMGR(request) && !IS_NLM_UP(request)) {
			flk_cancel_sleeping_lock(request, 1);
			return (ENOLCK);
		}
	}
	...
	if (IS_PXFS(request)) {
		 ...
		 */
		ASSERT(l_cbp(&request->l_flock) != NULL);
	...
	if (IS_LOCKMGR(request) && (l_callback(&request->l_flock) != NULL)) {
		...
		cprp = (*cb_proc)(cb_args);		/* execute callback */
 ...
 * associated with a particular set of identifiers.  If pid == NOPID, the
 * pid is ignored.  If use_sysid is FALSE, then the sysid is ignored.  If
 * vp is NULL, then the vnode pointer is ignored.
 *
...
locklist_t *
get_lock_list(int list_type, int sysid, boolean_t use_sysid, pid_t pid,
	    const vnode_t *vp)
{
	lock_descriptor_t	*lock;
...
{
	return (get_lock_list(FLK_SLEEPING_STATE, sysid, B_TRUE, pid, NULL));
}

locklist_t *
flk_get_active_locks(int sysid, pid_t pid)
{
	return (get_lock_list(FLK_ACTIVE_STATE, sysid, B_TRUE, pid, NULL));
}
...
{
	return (get_lock_list(FLK_ACTIVE_STATE, 0, B_FALSE, NOPID, vp));
}
