...
#pragma ident	"@(#)fio.c	1.73	99/08/31 SMI"
 ...
 * we ascend the tree until we find a non-empty right subtree.  We then
 * descend that subtree in a binary search for the smallest fd.  Finally,
 * we ascend the tree again to increment the allocation count of every
 * subtree containing the newly-allocated fd.  Freeing an fd requires
 * only the last step: we ascend the tree to decrement allocation counts.
 * Each of these three steps (ascent to find non-empty subtree, descent
 * to find lowest fd, ascent to update allocation counts) is O(log n),
 * thus the algorithm as a whole is O(log n).
 *
 ...
 *	descendants in a child of node N, including itself -- is given by
 *	clearing all but the least significant bit of N.  This follows
 *      immediately from (T1).  Applying this rule to our example, we see
 *      that CSIZE(100) = 100, CSIZE(x10) = 10, and CSIZE(xx1) = 1.
 *
 ...
 *     We start at the subtree rooted at minfd.  If this subtree is not
 *     full -- that is, if fip->fi_list[minfd].uf_alloc != CSIZE(minfd) --
 *     then step 1 is done.  Otherwise, we know that all fds in this
 *     subtree are taken, so we ascend to RPARENT(minfd) using (R1a).
 *     We repeat this process until we either find a candidate subtree
 *     or exceed fip->fi_nfiles.  We use (C1a) to compute CSIZE().
 *
...
closeandsetf(int fd, file_t *newfp)
{
	...
	if (newfp != NULL) {
		...
		while (ufp->uf_busy && ufp->uf_file == NULL) {
			mutex_exit(&fip->fi_lock);
			cv_wait_stop(&ufp->uf_wanted_cv, &ufp->uf_lock);
			UF_EXIT(ufp);
			mutex_enter(&fip->fi_lock);
			UF_ENTER(ufp, fip, fd);
		}
	 ...
	 */
	ASSERT(ufp->uf_refcnt == 0 || p->p_lwptotal > 1);
	if (ufp->uf_refcnt > 0) {
		UF_EXIT(ufp);
		COUNT(afd_wait);
		 ...
		 * holdlwps(HOLDFORK1) fails only if another lwp wants to
		 * perform a fork() or the process is exiting.  In either
		 * case, all other lwps are either returning from their
		 * system calls (because of HOLDFORK) or calling lwp_exit()
		 * (because of EXITLWPS) so we don't need to scan them.
		 */
		if (holdlwps(HOLDFORK1)) {
			kthread_t *t;
...
ufalloc(int start)
{
	proc_t *p = curproc;
	uf_info_t *fip = P_FINFO(p);
	int filelimit = (int)P_CURLIMIT(p, RLIMIT_NOFILE);
	uf_entry_t *ufp;
	int nfiles;
	int fd;
...
f_getfd(int fd)
{
	...
	UF_ENTER(ufp, fip, fd);
	flag = ufp->uf_flag;
	if ((fp = ufp->uf_file) != NULL && (fp->f_flag & FWRITE) &&
...
fassign(vnode_t **vpp, int mode, int *fdp)
{
	...
	if (error = falloc((vnode_t *)NULL, mode & FMASK, &fp, &fd))
		return (error);
