...
#pragma ident	"@(#)dacf.c	1.4	99/12/01 SMI"
...
/*
 * Enumeration of the device specificiers exported by the dacf framework.
 *
...
dacf_op_invoke(dacf_rule_t *rule, dacf_infohdl_t info_hdl, int flags)
{
	...
	/*
	 * if infohdl is a handle to an alias node, resolve it.
	 */
	dmdp = (struct ddi_minor_data *)info_hdl;
	while (dmdp->type == DDM_ALIAS) {
		dprintf("resolving alias\n");
		dmdp = dmdp->ddm_admp;
		ASSERT(dmdp);
	}
	info_hdl = (dacf_infohdl_t)dmdp;
