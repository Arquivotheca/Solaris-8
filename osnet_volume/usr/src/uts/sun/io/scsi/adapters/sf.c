...
#pragma ident	"@(#)sf.c	1.44	99/10/27 SMI"

/*
 * sf - Solaris Fibre Channel driver
 *
 * This module implements some of the Fibure Channel FC-4 layer, converting
...
/* because naming is done differently on these two platforms */
...
static int sf_bus_remove_eventcall(dev_info_t *, dev_info_t *,
    ddi_eventcookie_t);
...
#if defined(sun4m) || defined(sun4d)
static void sf_remove_from_hash(struct sf *, struct sf_target *);
#endif
...
static	char	sf_version[] = "1.44 99/10/27";	/* version of the module */
...
{ SF_EVENT_TAG_INSERT, FCAL_INSERT_EVENT, EPL_KERNEL },
{ SF_EVENT_TAG_REMOVE, FCAL_REMOVE_EVENT, EPL_INTERRUPT }
...
/* used when creating devinfo nodes for our children */
static struct {
	char *name;
	int dev_type;
	int busy;
	...
	{"ssd", DTYPE_DIRECT, 1},
	{"ses", DTYPE_ESI, 1},
	{"st",	DTYPE_SEQUENTIAL, 1},
};

static int sf_num_children = sizeof (sf_children) / sizeof (sf_children[0]);
...
_init(void)
{
	...
	cv_init(&sf_cv, NULL, CV_DRIVER, NULL);
	sf_watch_running = 0;
...
sf_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		...
		if ((handle = (struct fcal_transport *)ddi_get_driver_private(
		    dip)) == NULL) {
			cmn_err(CE_WARN,
		...
		sf->sf_events.ndi_events_version = NDI_EVENTS_REV0;
		sf->sf_events.ndi_n_events = SF_N_NDI_EVENTS;
		sf->sf_events.ndi_event_set = sf->sf_event_defs;
		...
		sf_insert_eid = ndi_event_getcookie(FCAL_INSERT_EVENT);
		sf_remove_eid = ndi_event_getcookie(FCAL_REMOVE_EVENT);
		...
		if (thread_create(NULL, DEFAULTSTKSZ,
		    (void (*)())sf_hp_daemon, (caddr_t)sf, 0, &p0, TS_RUN,
		    minclsyspri) == NULL) {
			mutex_exit(&sf_global_mutex);
			sf_log(sf, CE_WARN, "unable to create hotplug thread");
			(void) scsi_hba_detach(dip);
			goto fail;
		}

		/* set "hot plug thread initialized" flag */
    ...
    scsi_hba_tran_t *hba_tran, struct scsi_device *sd)
{
	...
	if (ddi_prop_op(DDI_DEV_T_ANY, tgt_dip, PROP_LEN_AND_VAL_BUF,
		...
		mutex_enter(&sf_global_mutex);
		if (!sf_first_time && (sf_count != 0)) {
			clock_t lb;
			sf_first_time = 1;	/* only do this once */
			(void) drv_getparm(LBOLT, &lb);
			/* wait a pseudo-random amt of time then give up */
			(void) cv_timedwait(&sf_cv, &sf_global_mutex, lb +
			    drv_usectohz(SF_INIT_WAIT_TIMEOUT));
		}
		mutex_exit(&sf_global_mutex);
		return (DDI_NOT_WELL_FORMED);
	}
...
{
	int kf;
	register int failure = FALSE;
	register struct sf_pkt *cmd;
	register struct sf *sf = ADDR2SF(ap);
	register struct sf_target *target = ADDR2TARGET(ap);
...
{
	register struct sf_pkt *cmd = (struct sf_pkt *)pkt->pkt_ha_private;
	register struct sf *sf = ADDR2SF(ap);
	register struct sf_target *target = ADDR2TARGET(ap);
...
#if !defined(sun4m) && !defined(sun4d)
/*
 * called by transport to get port WWN property (except sun4m and sun4u)
 */
/* ARGSUSED */
static int
...
/*
 * called by transport to get target soft AL-PA (except sun4m and sun4u)
 */
/* ARGSUSED */
static int
...
sf_add_cr_pool(struct sf *sf)
{
	...
	while (i < ptr->ntot) {
		...
		if (ddi_dma_htoc(ptr->cmd_dma_handle,
		    (off_t)((caddr_t)cptr - ptr->cmd_base), &cookie) !=
		    DDI_SUCCESS) {
			goto fail;
		}
		...
		if (ddi_dma_htoc(ptr->rsp_dma_handle,
		    (off_t)(eptr - ptr->rsp_base), &cookie) !=
		    DDI_SUCCESS) {
			goto fail;
		}
...
{
	register struct sf *sf = (struct sf *)arg;
	register struct sf_target	*target;
	int i;
	struct sf_pkt *cmd;
	struct scsi_pkt *pkt;
...
sf_els_callback(struct fcal_packet *fpkt)
{
	...
	register struct sf *sf = privp->sf;
	struct sf *tsf;
	int tgt_id;
...
{
	register struct sf_target *target, *ntarget, *otarget, *ptarget;
	int hash;
#if defined(sun4m) || defined(sun4d)
	scsi_hba_tran_t	*hba_tran;
	int wwn_changed = FALSE;
#endif
#ifdef RAID_LUNS
...
#if defined(sun4m) || defined(sun4d)
			if (otarget->sft_lun.l == ntarget->sft_lun.l) {
				/*
				 * If we already have the same LUN in
				 * the list that means that the
				 * WWN has changed and we need to
				 * copy the important info from the old
				 * LUN and delete it.
				 */
				ntarget->sft_dip = otarget->sft_dip;
				ntarget->sft_state |= (otarget->sft_state &
					SF_TARGET_INIT_DONE);
				ntarget->sft_tran = otarget->sft_tran;
				ntarget->sft_tran->tran_tgt_private
					= ntarget;
				sf_remove_from_hash(sf, otarget);
				target->sft_next_lun = otarget->sft_next_lun;
				kmem_free(otarget, sizeof (struct sf_target));
				break;
			}
#endif
		}
...
#if defined(sun4m) || defined(sun4d)
		if (otarget != NULL) {
			target->sft_dip = otarget->sft_dip;
			target->sft_state |= (otarget->sft_state &
						SF_TARGET_INIT_DONE);
			if (wwn_changed) {
				target->sft_next_lun = otarget->sft_next_lun;
				if (target->sft_state & SF_TARGET_INIT_DONE) {
					hba_tran = otarget->sft_tran;
					target->sft_tran = hba_tran;
					hba_tran->tran_tgt_private = target;
				}
				sf_remove_from_hash(sf, otarget);
    ...
    struct sf_target *target)
{
	...
	((union scsi_cdb *)reportlun->fcp_cdb)->scc_cmd = SCMD_REPORTLUN;
...
fail:
	sf_log(sf, CE_WARN,
		"%s failure for REPORTLUN to al_pa %x\n",
		msg, privp->dest_nport_id);
	sf_els_free(fpkt);
...
fail:
		...
		mutex_enter(&sf->sf_mutex);
		if (sf->sf_lip_cnt == privp->lip_cnt) {
			sf_log(sf, CE_WARN, "!REPORTLUN to al_pa %x failed\n",
				privp->dest_nport_id);
			sf_offline_target(sf, target);
			sf->sf_device_count--;
			ASSERT(sf->sf_device_count >= 0);
    ...
    struct sf_target *target)
{
	...
	SF_DEBUG(5, (sf, CE_WARN,
		"!Sending INQUIRY to al_pa %x lun %llx\n",
		privp->dest_nport_id,
		SCSA_LUN(target)));
	return (sf_els_transport(sf, privp));
...
fail:
	sf_log(sf, CE_WARN,
		"%s failure for INQUIRY to al_pa %x\n",
		msg, privp->dest_nport_id);
	sf_els_free(fpkt);
...
sf_inq_callback(struct fcal_packet *fpkt)
{
	struct sf_els_hdr *privp = (struct sf_els_hdr *)fpkt->
	    fcal_pkt_private;
	struct scsi_inquiry *ptr = (struct scsi_inquiry *)privp->data_buf;
	struct sf *sf = privp->sf;
	struct sf *tsf;
	struct sf_target *target = privp->target;
	struct fcp_rsp *rsp;
	...
	if (fpkt->fcal_pkt_status == FCAL_STATUS_OK) {
		...
		if ((rsp->fcp_u.fcp_status.scsi_status == STATUS_GOOD) &&
			...
			if (!rsp->fcp_u.fcp_status.rsp_len_set ||
				...
				SF_DEBUG(2, (sf, CE_CONT,
				    "!INQUIRY to al_pa %x lun %llx succeeded\n",
	...
	if (++(privp->retries) < sf_els_retries ||
		...
		if (delayed_retry) {
			...
			privp->retries--;
			(void) drv_getparm(LBOLT, &i);
			privp->timeout = sf_watchdog_time + (i & 1);
...
fail:
		...
		if (sf->sf_lip_cnt == privp->lip_cnt) {
			sf_offline_target(sf, target);
			sf_log(sf, CE_NOTE,
			    "INQUIRY to al_pa %x lun %llx failed. "
			    "Retry Count: %d\n",
			    privp->dest_nport_id, SCSA_LUN(target),
			    privp->retries);
			sf->sf_device_count--;
...
next_entry:
	...
	/* decrement count of threads in sf */
	mutex_enter(&sf_global_mutex);
	if (--sf_count == 0) {
...
{
	int i, tgt_id = sf_alpa_to_switch[target->sft_al_pa];
	dev_info_t *cdip = NULL;


	for (i = 0; i < sf_num_children; i++) {
		if (target->sft_device_type == sf_children[i].dev_type) {
			break;
		}
	}

	if (i == sf_num_children) {
		sf_log(sf, CE_WARN, "target 0x%x, unknown device type %x\n",
				tgt_id, target->sft_device_type);
		return;
	}

	if (ndi_devi_alloc(sf->sf_dip, sf_children[i].name,
...
sf_offline_target(struct sf *sf, struct sf_target *target)
{
	...
	while (target != NULL) {
		sf_log(sf, CE_NOTE,
			"!target 0x%x al_pa 0x%x lun %llx offlined\n",
...
sf_abort(struct scsi_address *ap, struct scsi_pkt *pkt)
{
	...
	if (pkt != NULL) {
		...
		fpkt = (struct fcal_packet *)((char *)cmd +
		    sizeof (struct sf_pkt));
		SF_DEBUG(2, (sf, CE_NOTE, "sf_abort packet %x\n",
		    (int)fpkt));
		pcmd = NULL;
		mutex_enter(&sf->sf_cmd_mutex);
		ncmd = sf->sf_pkt_head;
...
sf_reset(struct scsi_address *ap, int level)
{
	...
	if (level == RESET_TARGET) {
		...
		if ((rval == TRUE) && (lip_cnt == sf->sf_lip_cnt)) {
			p->target = target;
			p->lip_cnt = lip_cnt;
			(void) drv_getparm(LBOLT, &p->timeout);
			p->timeout += drv_usectohz(SF_TARGET_RESET_DELAY);
...
sf_start(struct scsi_address *ap, struct scsi_pkt *pkt)
{
	...
	if (cmd->cmd_state == SF_STATE_ISSUED) {
		cmn_err(CE_PANIC, "sf: issuing packet twice %p\n",
			(void *)cmd);
	}
...
sf_cmd_callback(struct fcal_packet *fpkt)
{
	...
	if (cmd->cmd_state == SF_STATE_IDLE) {
		cmn_err(CE_PANIC, "sf: completing idle packet %p\n",
			(void *)cmd);
	}
	...
	if (fpkt->fcal_pkt_status == FCAL_STATUS_OK) {
		...
		if ((pkt->pkt_scbp != NULL) &&
		    ((*(pkt->pkt_scbp) = rsp->fcp_u.fcp_status.scsi_status) !=
			STATUS_GOOD)) {
		...
		if (rsp->fcp_u.fcp_status.rsp_len_set) {
			...
			bep = (struct fcp_rsp_info *)(&rsp->fcp_response_len +
			    1);
		...
		if (rsp->fcp_u.fcp_status.sense_len_set) {
			...
			caddr_t sense = (caddr_t)rsp + sizeof (struct fcp_rsp)
			    + rsp->fcp_response_len;
		...
		/*
		 * The firmware returns the number of bytes actually xfered
		 * into/out of host. Compare this with what we asked and if it
		 * is different, we lost frames ?
		 */
...
target->sft_al_pa]);
		...
		case FCAL_STATUS_INCOMPLETE_DMA_ERR:
			if (sf_core && (sf_core & SF_CORE_INCOMPLETE_DMA)) {
				sf_token = (int *)fpkt->fcal_socal_request.\
	...
	if (msg1 != NULL && sfdebug >= 4) {
		sf_log(sf, CE_WARN,
		    "!Transport error on cmd=%x target=0x%x:  %s\n", fpkt,
	...
	if (!msg1 && !msg2)
		SF_DEBUG(6, (sf, CE_NOTE, "Completing pkt %p\n", pkt));
	if (pkt->pkt_comp != NULL) {
		(*pkt->pkt_comp)(pkt);
	}
}
...
{
	register struct sf_pkt *cmd, *prev_cmd = NULL;
...
sf_check_targets(struct sf *sf)
{
	 ...
	 */
					...
					if ((cmd->cmd_block->fcp_cntl.cntl_reset
						== 1) || (cmd->cmd_block->
						fcp_cntl.cntl_abort_tsk == 1)) {
					    cmd->cmd_block->fcp_cntl.cntl_reset
						= 0;
...
sf_target_timeout(struct sf *sf, struct sf_pkt *cmd)
{
	...
	SF_DEBUG(1, (sf, CE_NOTE, "Command %x to target %x timed out\n",
	    cmd->cmd_fp_pkt, cmd->cmd_pkt->pkt_address.a_target));

	fpkt = (struct fcal_packet *)((char *)cmd + sizeof (struct sf_pkt));

	if (sf_core && (sf_core & SF_CORE_CMD_TIMEOUT)) {
		sf_token = (int *)fpkt->fcal_socal_request.sr_soc_hdr.\
		    sh_request_token;
		(void) soc_take_core(sf->sf_sochandle, sf->sf_socp);
		sf_core = 0;
	}
...
try_again:
		...
		sf_log(sf, CE_NOTE, "%s abort to al_pa %x failed. "
			"status=0x%x, forcing LIP\n", what,
			privp->dest_nport_id, rval);
		privp = NULL;
		if (sf->sf_lip_cnt == lip_cnt) {
			sf_force_lip(sf);
		}
 ...
 */
static void
sf_force_lip(register struct sf *sf)
{
	int i;
	register struct sf_target *target;
...
sf_open(dev_t *dev_p, int flag, int otyp, cred_t *cred_p)
{
	...
	sf = ddi_get_soft_state(sf_state, getminor(dev));
	if (sf == NULL) {
		return (ENXIO);
	}
    ...
    int cmd, intptr_t arg, int mode, cred_t *cred_p, int *rval_p)
{
	...
	/*
	 * now that dcp is allocated we must do ndi_dc_freehdl() before
	 * exiting
	 */
	...
	case SFIOCGMAP:
		...
		if (sf->sf_lilp_map->lilp_magic == FCAL_BADLILP_MAGIC) {
			...
			/* Need to genetate a fake lilp map */
			for (i = 0; i < sf_max_targets; i++) {
				if (sf->sf_targets[i])
	...
	case DEVCTL_BUS_GETSTATE:
		if (ndi_dc_return_bus_state(sf->sf_dip, dcp) != NDI_SUCCESS) {
			retval = EFAULT;
			goto dun;
		}
		break;
 ...
 */
static int
sf_bus_remove_eventcall(dev_info_t *dip, dev_info_t *rdip,
    ddi_eventcookie_t eventid)
{
	...
	return (ndi_event_remove_callback(sf->sf_event_hdl, rdip,
    ...
    ddi_eventcookie_t eventid, void *impldata)
{
	...
	ddi_eventcookie_t remove_event = ndi_event_getcookie(
						DDI_DEVI_REMOVE_EVENT);

	/* is this a remove event ?? */
	if (bcmp(&eventid, &remove_event,
				sizeof (ddi_eventcookie_t)) == 0) {
		struct sf_target *target;
		struct sf *sf = ddi_get_soft_state(sf_state,
		...
		if ((target = sf_get_target_from_dip(sf, rdip)) != NULL) {
			...
			target->sft_state &= ~SF_TARGET_INIT_DONE;
			mutex_exit(&target->sft_mutex);
			return (DDI_EVENT_CLAIMED); /* event handled */
		}
...
loop:
	...
	while (sf->sf_hp_elem_head != NULL) {
		...
		elem = sf->sf_hp_elem_head;	/* save ptr to head of list */

		/* take element off of list */
		if (sf->sf_hp_elem_head == sf->sf_hp_elem_tail) {
			/* element was only one in list -- list now empty */
			sf->sf_hp_elem_head = sf->sf_hp_elem_tail = NULL;
		} else {
	...
	/* see if hotplug thread has not been initialized */
	if (sf->sf_hp_initted == 0) {
		cv_signal(&sf->sf_hp_daemon_cv);
		mutex_exit(&sf->sf_hp_daemon_mutex);
...
static void
sf_remove_from_hash(struct sf *sf, struct sf_target *target)
{
	int hash;
	struct sf_target *targetp, *prevp;
	uchar_t *wwn;


	ASSERT(mutex_owned(&sf->sf_mutex));
	wwn = (uchar_t *)&target->sft_port_wwn;
#ifndef RAID_LUNS
	hash = SF_HASH(wwn, target->sft_lun.l);
#else
	hash = SF_HASH(wwn, target->sft_raid_lun);
#endif
	targetp = sf->sf_wwn_lists[hash];
	prevp = NULL;
	while (targetp != NULL) {
		if (targetp == target) {
			if (prevp == NULL) {
				sf->sf_wwn_lists[hash] = targetp->sft_next;
			} else {
				prevp->sft_next = targetp->sft_next;
			}
			break;
		}
		prevp = targetp;
		targetp = targetp->sft_next;
	}
}
