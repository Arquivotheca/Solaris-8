...
#pragma ident	"@(#)ghd_timer.c	1.17	99/05/20 SMI"
	...
	for (;;) {
		...
		default:
			...
			GDBG_ERROR(("?ghd_timer_newstate: HBA reset failed "
				    "hba 0x%x gcmdp 0x%x gtgtp 0x%x\n",
				    hba_handle, gcmdp, gtgtp));
			/*
		...
		if ((calltype != GHD_NEWSTATE_TGTREQ) &&
			...
			scsi_log(cccp->ccc_hba_dip, cccp->ccc_label, CE_WARN,
				"%s: %s, target=%d lun=%d", ct_msgp, msgp,
				gtgtp->gt_target, gtgtp->gt_lun);
		}

		GDBG_WARN(("?ghd_timer_newstate: %s, h=0x%x g=0x%x gtgtp=0x%x:"
				" act %d nextact %d nstate %d\n",
				msgp, hba_handle, gcmdp, gtgtp,
		...
		if ((*cccp->ccc_timeout_func)(hba_handle, gcmdp,
			...
			/* reset bus failed something drastic wrong */
