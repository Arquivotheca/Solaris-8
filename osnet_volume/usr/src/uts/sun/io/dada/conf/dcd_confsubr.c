...
#pragma	ident	"@(#)dcd_confsubr.c 1.11     98/02/25 SMI"
...
void makecommand(struct dcd_pkt *, int, u_char, uint32_t,
				u_char, uint32_t, u_char, u_char);
 ...
 */
void
dcd_unprobe(register struct dcd_device *devp)
{
	if (devp->dcd_ident) {
...
int
dcd_hba_probe(register struct dcd_device *devp, int (*callback)())
{

	register struct dcd_pkt *ident_pkt = NULL;
	register int rval = DCDPROBE_NOMEM;
	...
	ident_bp = dcd_alloc_consistent_buf(ROUTE, (struct buf *)NULL,
			(u_int) SUN_IDENTSIZE, B_READ, cb_flag, NULL);
	if (ident_bp == NULL) {
		goto out;
	}
