...
#pragma ident	"@(#)fd.c	1.116	99/10/22 SMI"
...
static struct modldrv modldrv = {
	...
	"Floppy Driver v1.108", 	/* Name of the module. */
    ...
    char *name, caddr_t valuep, int *lengthp)
{
	if (strcmp(name, "nblocks") == 0) {
		...
		if (fd_unit_is_open(fdc->c_un)) {
			lp = &un->un_label.dkl_map[FDPARTITION(dev)];
...
fdstart(struct fdctlr *fdc)
{
	...
	uint_t	last_part;	/* last block of the parition */
 ...
 *	is reset, initialised and the state is set to FD_STATE_NORMAL.
 */
