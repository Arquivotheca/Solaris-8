...
#pragma ident	"@(#)hme.h	1.55	99/10/22 SMI"
...
typedef struct qfe_id {
	int qfe_slot;		/* slot where qfe resides */
	dev_info_t *qfe_pdip;	/* ptr to qfe parent devinfo */
} qfe_id_t;


struct	qfe {
	qfe_id_t		qfe_id;		/* Sbus Slot ID */
	dev_info_t		*qfe_dip;	/* devinfo of this card */
	struct	hme	*qfe_dev[4];	/* hmep ptrs to member qfe devs */
	uint32_t	*hme_intmask[4];	/* per dev intmask reg ptr */
	kmutex_t		qfe_intrlock;	/* protect intr-side fields */
	ddi_iblock_cookie_t	qfe_cookie;	/* interrupt cookie */
	struct	qfe		*qfe_nextp;	/* next in a linked list  */
};
...
struct	hmekstat {
	...
	struct	kstat_named	hk_rmtus;	/* read MTUs */
