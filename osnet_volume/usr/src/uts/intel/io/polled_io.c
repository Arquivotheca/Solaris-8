 ...
 * All rights reserved.
 */
#pragma ident   "@(#)polled_io.c	1.22	99/08/19 SMI"
...
/*
 * consconfig is aware of which devices are the stdin and stout.
 * The post-attach/pre-detach functions are an extension of
 * consconfig because they know about the dynamic changes to the
 * stdin device.  Neither an individual driver nor the DDI
 * framework knows what device is really the stdin.
 */
/*
 * Issues:
...
_NOTE(ARGUNUSED(flags))
	extern cons_polledio_t	*system_polledio;

	system_polledio = polled_io;
#endif
	 ...
	 */
	if (dvec != NULL &&
	    dvec->dv_version >= DEBUGVEC_VERSION_2) {
...
/*
 * This is the routine that kadb calls to save any state information
 * before using the input device.  This routine, and all of the
 * routines that it calls, are responsible for saving any state
 * information so that it can be restored when kadb mode is over.
 */
static void
...
/*
 * This is the routine that kadb calls when it is giving up control of the
