...
#pragma ident	"@(#)store.s	1.2	99/05/04 SMI"
...
/	extrn	SR_MASKS,SR_FLAGS,SR_ERRORS,SR_CONTROLS
/a_msr	ends
/
/	assume	%ds:a_msr
/
...
store:	/proc
	...
	testb	invalid_mask,SR_MASKS	/ if unmasked, just exit
...
store_to_memory:
	...
	jne	go_to_storer
	orb	invalid_mask,SR_ERRORS	/ set i_error
	testb	invalid_mask,SR_MASKS	/ if unmasked, just exit
...
/
bcd_store:	/proc
	push	SR_MASKS				/ save current precision
	orb	prec64,SR_CONTROLS			/ and set to 64-bit
...
invalid_bcd:
	orb	invalid_mask,SR_ERRORS	/ set i_error
	testb	invalid_mask,SR_MASKS	/ is i_error unmasked?
	jnz	store_bcd_indef
...
store_signed_bcd:
	...
	movb	%ah,msb_frac1
store_bcd_result:
	les	SR_MEM_OFFSET,%edi	/move frac1 to memory
	push	%ds				/ save a?msr
	push	%ss
	pop	%ds
	lea	word_frac1,%esi
...
/..
	...
	call	pop_free
bcd_store_done:
	pop	SR_MASKS				/ restore old setting
	ret
/bcd_store	endp
/
/"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
...
single_real_store:	/proc
	...
	orb	invalid_mask,SR_ERRORS	/ set i_error for snan
	testb	invalid_mask,SR_MASKS	/ if unmasked, just exit
...
/		pop		edi
	...
	orb	overflow_mask,SR_ERRORS	/ set o_error
	testb	overflow_mask,SR_MASKS	/ if unmasked, violate ieee standard
...
masked_overflow:
	orb	$inexact_mask,SR_ERRORS	/ set inexact_error
	...
	orb	$a_mask,SR_FLAGS			/ set a_bit
pop_store_valid_done:
	popf						/ reload flags
store_valid_done:
	ret									/ exit
store_underflow_:
	testb	underflow_mask,SR_MASKS	/ if underflow masked,
	jnz	check_masked_underflow	/ then branch to check for inexact
	orb	underflow_mask,SR_ERRORS/ else, set u_error
/
...
check_masked_underflow:
	...
	orb	underflow_mask+inexact_mask,SR_ERRORS
decrement_exponent:
...
special_16:
	...
	orb	invalid_mask,SR_ERRORS/ no, set invalid error
	movw	$0x8000,%ax			/ load inv constant into ax
	testb	invalid_mask,SR_MASKS/ is invalid error masked?
