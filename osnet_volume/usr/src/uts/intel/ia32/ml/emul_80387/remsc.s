...
#pragma ident	"@(#)remsc.s	1.2	99/05/04 SMI"
...
remr:	/proc
	...
	andb	$-1 ! [a_mask+c_mask],SR_FLAGS/ clear '87 a-flag, and also
...
valid_case:
	...
	orb	$c_mask,SR_FLAGS	/ reduction and replace expon_diff
...
calc_exponent:
	...
	call	test_4w
	jnz	do_normalize
	testb	$c_mask,SR_FLAGS		/ was reduction complete?
...
check_rem_underflow:
	...
	push	SR_MASKS
	orb	prec64,SR_CONTROLS		/ "or" works only because prec64
...
remr_zero:
	...
	jne	put_op1
	movb	positive,sign1(%ebp)
	testb	rnd_down,SR_CONTROLS
	jz	put_op1
	testb	rnd_up,SR_CONTROLS
	jnz	put_op1
...
do_put_result:
	...
	call	put_result
	andb	$-1![zero_mask+a_mask+sign_mask],SR_FLAGS
	movb	q,%al
...
signal_for_snan:
	orb	invalid_mask,SR_ERRORS/ set i-error for signaling nan operand
	testb	invalid_mask,SR_MASKS/ if unmasked, return forthwith
...
catch_spcl_scale_cases:
	andb	$-1 ! [a_mask],SR_FLAGS/ clear '87 a-bit
	movb	tag1(%ebp),%al		/ both operands valid?
	orb	tag2(%ebp),%al
	jnz	special_cases_handler	/ no, branch to handler of special cases
valid_scale_case:
	push	SR_MASKS			/ save current rounding controls
	orb	rnd_to_zero,SR_CONTROLS	/ institute round by chopping
					/ this "or" works only because
					/ rnd_to_zero sets entire rc field
	call	fix32			/ convert scale factor to int32
	pop	SR_MASKS			/ restore rnd control
