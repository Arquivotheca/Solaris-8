...
#pragma ident	"@(#)dcode.s	1.4	99/05/04 SMI"
...
/ *********************************************************************
/
/	label to mark start of user-readable portion of emulator
/
/ *********************************************************************
	.globl	e80387_start
...
e80387_start(void)
{
	extern void arith();
	extern void compar();
	extern void fetch_an_op();
	extern void load_con();
	extern void move_10_bytes();
	extern void remr();
	extern void stcw();
	extern void store();
...
/	Solaris kernel assumes that %fs is correctly set when %gs is.
/	Emulator uses %fs but not %gs, so need to clear %gs here so
/	that assumption is not violated when emulating kernel FP.
/	User thread support expects %gs to remain unchanged (even during
/	signal processing) so do not clear %gs except when emulating
/	kernel FP.
	testb	$3, %al
	jnz	from_user_mode
	xorl	%eax, %eax
...
from_user_mode:
	...
	/ order halfwords.
	/
	/ The following code assumes that any stack segment selector
	/ other than USER_DS or KDSSEL will be for a 16-bit segment.
	/ This assumption is not strictly true, but probably is for
	/ practical purposes.  Could do an larb instruction after the
	/ second je instruction and determine more about the segment.
	/
	movw	%ss, %ax
	cmpw	$USER_DS, %ax
...
store_on:
	...
	movw    %dx,SR_MEM_BASE   / yes, store base in a?msr
	mov     %eax,SR_MEM_OFFSET  / store offset in a?msr
store_return:
...
/	Signal floating point exception by doing INT $16.  Put back context
/	first so that kernel handler sees same one.  Get it back again
/	afterwards because kernel handler will change it.  Preserve
/	registers because get_fp_ctx can destroy them.
	pushal
	call	set_fp_ctx
	int	$16
	call	get_fp_ctx
...
no_pending_unmasked_xcptns:
	...
	mov     %eax,SR_INSTR_OFFSET        / store offset in status reg
	movw    %es,SR_INSTR_BASE   / store base in status reg
	pop	%eax
do_operation:
	movswl  %bx,%ebx
...
/				This commented out at Phil's suggestion
	...
	ja	get_done
mark_errors:
	orw     $0x8080, SR_ERRORS  /taking care of es and B
	movb	$0x02,%al			/ load nonzero ie mask
	jmp	get_done
release_stack:
	andw    $0x7f7f, SR_ERRORS
get_done:
/        -------------------------------------JKD
...
/	Load %fs first because Solaris kernel assumes %fs contains its
/	normal value if %gs contains its own.  Could reverse order of
/	%fs and %gs on the stack instead if no-one else depends on it.
	movw	4(%esp), %fs
...
load_lf_1bb:
	...
	andw	%ax,%di
	movswl  %di,%edi
	pushl	group_1bb_lf(%edi)	/ stack next format/location
...
return_1bb:
	popl	%edi			/ load operand 1 information
	popl	%esi			/ load operand 2 information
	popl	%ecx			/ load result 1 information
	popl	%edx			/ load result 2 information
	ret
group_1ba:
...
/       Issue fast system call T_GETFP to copy FP context from LWP
/       structure onto local stack.  Fast system call fails if page
/       fault occurs, so touch both ends of range first and retry
/       if it still fails.
/
/	This routine does not save and restore registers.  Caller
/	must preserve any it needs.
get_fp_ctx:
	pushl	%ebp			/ Save %ebp for regs case
get_fp_pgflt:
	leal    fpdata(%ebp), %ebx
	movl    $0,fpdata(%ebp)
	movl    $0,fpdata_last(%ebp)
	movl    $T_FGETFP, %eax
	int     $T_FASTTRAP
	jz	get_fp_pgflt		/ T_FGETFP sets Z-flag on page fault
	jc	get_via_regs
	addl	$4, %esp		/ Discard saved %ebp
	ret

get_via_regs:
/	If we get here, the T_FGETFP was unable to determine that the
/	address we gave it in %ebx was safe.  Either we are running on
/	a 386 which allows the kernel to write non-writable pages or
/	our %ss is non-standard.  It has returned the first 7 words of
/	the context in the general registers other than %esp.  We get
/	the remainder of the context by doing more T_FGETFP calls with
/	%ebx set to the offset into the context where we want it to start.
	pushl	%ebp			/ value returned by fast trap
	movl	4(%esp), %ebp		/ %ebp saved at get_fp_ctx
	movl	%eax, fpdata+4(%ebp)
	popl	%eax			/ value returned in %ebp by fast trap
	movl	%eax, fpdata(%ebp)
	movl	%ebx, fpdata+8(%ebp)
	movl	%ecx, fpdata+12(%ebp)
	movl	%edx, fpdata+16(%ebp)
	movl	%edi, fpdata+20(%ebp)
	movl	%esi, fpdata+24(%ebp)
	movl	$28, %ebx
	movl    $T_FGETFP, %eax
	int     $T_FASTTRAP
	pushl	%ebp			/ value returned by fast trap
	movl	4(%esp), %ebp		/ %ebp saved at get_fp_ctx
	movl	%eax, fpdata+32(%ebp)
	popl	%eax			/ value returned in %ebp by fast trap
	movl	%eax, fpdata+28(%ebp)
	movl	%ebx, fpdata+36(%ebp)
	movl	%ecx, fpdata+40(%ebp)
	movl	%edx, fpdata+44(%ebp)
	movl	%edi, fpdata+48(%ebp)
	movl	%esi, fpdata+52(%ebp)
	movl	$56, %ebx
	movl    $T_FGETFP, %eax
	int     $T_FASTTRAP
	pushl	%ebp			/ value returned by fast trap
	movl	4(%esp), %ebp		/ %ebp saved at get_fp_ctx
	movl	%eax, fpdata+60(%ebp)
	popl	%eax			/ value returned in %ebp by fast trap
	movl	%eax, fpdata+56(%ebp)
	movl	%ebx, fpdata+64(%ebp)
	movl	%ecx, fpdata+68(%ebp)
	movl	%edx, fpdata+72(%ebp)
	movl	%edi, fpdata+76(%ebp)
	movl	%esi, fpdata+80(%ebp)
	movl	$80, %ebx
	movl    $T_FGETFP, %eax
	int     $T_FASTTRAP
	popl	%ebp			/ %ebp saved at get_fp_ctx
	movl	%eax, fpdata+84(%ebp)
	movl	%ebx, fpdata+88(%ebp)
	movl	%ecx, fpdata+92(%ebp)
	movl	%edx, fpdata+96(%ebp)
	movl	%edi, fpdata+100(%ebp)
	movl	%esi, fpdata+104(%ebp)
	ret

/       Issue fast system call T_FSETFP to copy FP context to LWP
/       structure from local stack.  Fast system call fails if page
/       fault occurs, so touch both ends of range first and retry
/       if it still fails.
set_fp_ctx:
	leal    fpdata(%ebp), %ebx
	testl   $0,fpdata(%ebp)
	testl   $0,fpdata_last(%ebp)
	movl    $T_FSETFP, %eax
	int     $T_FASTTRAP
	jz	set_fp_ctx	/ T_FSETFP sets Z-flag on page fault
	ret
