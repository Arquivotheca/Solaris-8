...
#pragma ident	"@(#)reg.s	1.2	99/05/04 SMI"
...
single_denormalized:
	...
	testb   denorm_mask,SR_MASKS   / short cut for unmasked d-error
...
/
	...
	push    SR_SEG                  / set ds to address register stack
	pop	%ds
...
load:	/proc
	...
	testb   invalid_mask,SR_MASKS     / else, stack underflow error
...
is_op1_denorm_:
	...
	jne	op1_zero_or_inf
	movb	true,signal_d_error_
	testb   denorm_mask,SR_MASKS    / short cut if unmasked d-error
	jz	interim_exit
...
op1_zero:
	...
	testb   zero_divide_mask,SR_MASKS   / if z_error is unmasked
...
save_regs:	/proc
	...
	leal    SR_REGSTACK,%esi                /  40 - top*5
	push	%esi				/ starting address =
	cwtl
	add	%eax,%esi			/  SR_REGSTACK + top*5
...
/
/	outputs:
/		loads the SR_REGSTACK area of a?msr from memory
/
/	data accessed:
/
/	data changed:
...
/	push	%ds
	...
	subw	%ax,%cx				/ from top of stack =
	leal    SR_REGSTACK,%edi                /  40 - top*5
	push	%edi				/ starting address =
	cwtl
	add	%eax,%edi				/  SR_REGSTACK + top*5
...
exam:	/proc
	...
	orb     $[zero_mask+sign_mask],SR_FLAGS     / set s and z bits
	andb    $ -1 ! [a_mask+c_mask],SR_FLAGS     / clear a and c bits
	jmp	check_a_bit
something_on_stack:
	andb    $ -1 ![zero_mask+c_mask+a_mask+sign_mask],SR_FLAGS
	...
	cmpb	denormd,%al
	jne	valid_or_zero
	orb     $[zero_mask+c_mask],SR_FLAGS   / set s, z, c to 0, 1, 1
					/ for denormalized op
	jmp	check_a_bit
valid_or_zero:
...
other_i_error_:
	andb    $-1 ! a_mask,SR_FLAGS       / clear a-bit
	mov	$0,before_error_signals(%ebp)	/ initialize all to zero
	movb	tag1(%ebp),%al				/ load operand tag
	cmpb	unsupp,%al
	jne	is_op1_nan_
	testb   invalid_mask,SR_MASKS   / short cut if unmasked i-error
