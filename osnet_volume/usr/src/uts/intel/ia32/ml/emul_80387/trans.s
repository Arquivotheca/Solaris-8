...
#pragma ident	"@(#)trans.s	1.3	99/05/04 SMI"
...
/""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
atan_core:	/proc
	push	SR_MASKS			/ save precision control
/	and	dword ptr SR_MASKS, 0fffff3ffh; set rounding control to nearest
	or	$0x0300,SR_MASKS 	/ set precision control to prec64
	orb	$inexact_mask,SR_ERRORS	/ always inexact
...
atn_put_atn:
	pop	SR_MASKS			/ restore precision control
	ret
/atan_core	endp
/
/
...
two_arg_trans:	/proc
	...
	testb	invalid_mask,SR_MASKS/ if unmasked, just return
...
two_arg_special_case_:
	andb	$ -1 ! a_mask,SR_FLAGS		/ clear a-bit initially
...
/
both_ops_denormd:
	orb	denorm_mask,SR_ERRORS
	testb	denorm_mask,SR_MASKS
...
two_arg_special_case:
	...
	jz	op1_nan_
op_unsupp:
	orb	invalid_mask,SR_ERRORS/ here, at least one op is unsupported.
	testb	invalid_mask,SR_MASKS
	jz	two_arg_unmasked_exit
	jmp	return_indef
...
invalid_operand:
	orb	invalid_mask,SR_ERRORS/ invalid operand sets
	testb	invalid_mask,SR_MASKS
...
zerror_flip_sign_y_inf:
	orb	zero_divide_mask,SR_ERRORS
	testb	zero_divide_mask,SR_MASKS
	jz	two_arg_unmasked_exit
flip_sign_y_inf:
...
/		or		SR_ERRORS,	zero_divide_mask
/		test	SR_MASKS,	zero_divide_mask
/		jz		two_arg_unmasked_exit
/		or		SR_ERRORS,	denorm_mask
/		test	SR_MASKS,	denorm_mask
/		jz		two_arg_unmasked_exit
...
more_than_half_:
	orb	$inexact_mask,SR_ERRORS	/ set inexact error flag
	push	SR_MASKS	/ save precision control
	or	$0x0300,SR_MASKS 	/ set precision control to prec64
	and	$0x0fffff3ff,SR_MASKS	/ set rc = to_nearest
...
exp_exit:
	...
	orb	$a_mask,SR_FLAGS
exp_restore:
	pop	SR_MASKS	/ restore precision and rounding controls
	mov	$offset_result,%edi
	jmp	put_si_result
...
/
log_rat_apx:
	orb	$inexact_mask,SR_ERRORS		/ set p flag
	call	move_frac2_frac1
	call	mulx
	mov	dword_exp_tmp,%ecx
	cmp	$72,%ecx
...
two_result_op1_chk:	/proc
	...
	andb	$ -1 ! c_mask,SR_FLAGS		/ clear '87 c-flag
	testb	invalid_mask,SR_MASKS
...
/ '87 c-flag has been cleared by this point
	...
	cmp	$63,%eax
	jle	do_reduction
	orb	$c_mask,SR_FLAGS		/ indicate incomplete reduction
...
reduction_done:
	orb	$inexact_mask,SR_ERRORS	/ precision error unless zero mod
/check_for_tan_op:
/		cmpb		op_type,	tan_op
/		je		reduce_hapi
reduce_pi:
...
mod_set_inexact:			/ a zero mod will be changed here
	orb	$inexact_mask,SR_ERRORS	/ to qrtr_pi, so precision error
complementary_angle:
...
op2_gets_1:
	...
	movb	%ah,sign2(%ebp)
	push	SR_MASKS
/	and		dword ptr SR_MASKS, 0fffff3ffh	; set rc = to_nearest
	or	$0x0300,SR_MASKS 	/ set precision control to prec64
	push	trans_info
	jmp	reverse_div
tan_calc:
...
sin_specific:	/proc
	...
	or	$0x0f00,SR_MASKS 	/ set rc to chop!
	movb	$add_op,operation_type(%ebp)
	call	subadd
	call	post_op_round
	lea	result_dword_frac(%ebp),%esi
...
/ *********************************************************************
/
/	label to mark finish of user-readable portion of emulator
/
/ *********************************************************************
	.globl	e80387_finish
e80387_finish:
