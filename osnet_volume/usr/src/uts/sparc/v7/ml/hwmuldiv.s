...
#pragma ident	"@(#)hwmuldiv.s	1.25	99/07/26 SMI"
 ...
 *	get lint prototypes. Likewise, the _ip_* routines require a setup
 *	not possible from C.
 */
 ...
 */
	...
	RTENTRY(.udiv_sw)	! UNSIGNED DIVIDE
	b	.divide
	mov	0,%g1		! result always positive

	RTENTRY(.div_sw)	! SIGNED DIVIDE
...
!	compute size of quotient, scale comparand
	...
	mov	%o0,%o3
	cmp	%o3,%o5
	blu	.got_result	! if %o3<%o5 already, there's no point in
				! continuing
	mov	0,%o2
	...
	! and are only done if %o3 >= 0. Because both %o3 and %o5 may have the
	! highorder bit set in the first step, just falling into the regular
...
1:
	...
	RTENTRY(.urem_sw)	! UNSIGNED REMAINDER
	b	.remainder
	mov	0,%g1		! result always positive

	RTENTRY(.rem_sw)	! SIGNED REMAINDER
	orcc	%o1,%o0,%g0 ! are either %o0 or %o1 negative
	bge	.remainder	! if not, skip this junk
...
!	compute size of quotient, scale comparand
	...
	mov	%o0,%o3
	cmp	%o3,%o5
	blu	.rgot_result	! if %o3<%o5 already, there's no point in
				! continuing
	mov	0,%o2
	sethi	%hi(1<<(32-4 -1)),%g2
	cmp	%o3,%g2
...
1:	mov	%i0,%o0
	...
	mov	%i1,%o2
	st	%o0,[%i2]
	tst	%o1		! If result is > 2^23-1, overflow 
	bz	5f
	mov	-1, %l3		! overflow set return 0xffffffff
	st	%l3, [%i2]
5:
	mov	SIMU_SUCCESS, %i0
	ret
...
1:	mov	%i0,%o0
	...
	and	%o0,%l3,%o3
	orcc	%o1,%o3,%g0
	bz	2f		! no positive overflow!
	cmp	%o1, -1
	be	3f		! check for negative result no overflow
	nop
	bl	2f
	mov	%l3, %o0	! negative overflow, result = 0x80000000
	b	2f
	not	%l3, %o0	! positive overflow, result = 0x7fffffff
3:	
	andcc	%l3, %o0, %g0
	bz,a	2f
	mov	%l3, %o0	
...
1:	mov	%i0,%o0
	...
	mov	0, %l0		! clear NZVC
	tst	%o1 		! check for overflow
	bz	.no_ov
	mov	-1, %l3
	b	.set_ov		! use common code in _ip_divcc
	mov	%l3, %o0
	SET_SIZE(_ip_udivcc)
...
1:	mov	%i0,%o0
	...
	and	%o0,%l3,%o3
	orcc	%o1,%o3,%g0
	bz	.no_ov		! no positive overflow!
	cmp	%o1, -1
	be	3f		! check for negative result but no overflow
	nop
	bl	.set_ov
	mov	%l3, %o0	! negative overflow, result = 0x80000000
	b	.set_ov
...
3:	
	andcc	%l3, %o0, %g0
	bnz	.no_ov		! negative result but not overflow
	nop
	mov	%l3, %o0	! negative overflow, result = 0x80000000
.set_ov:
	or	%l0, 2, %l0
.no_ov:
	st	%o0, [%i2]	! store result (with correction for overflow)
	tst	%o0
	bz,a	3f
	or	%l0, 0x4, %l0	! set Z if result[31:0] = 0
	srl	%o0, 31, %l1	! %l1 = result<31>
	sll	%l1, 3, %l1	! N = result<31>
	or	%l1, %l0, %l0
3:
	sll	%l0, 20, %l0	! shift to the icc position in psr
	ld	[%i4],%l1	! get psr value
	set	0x00f00000,%l2	! mask for cc field of psr
	andn	%l1,%l2,%l1	! clear icc field
	or	%l1,%l0,%l1	! put in icc
	st	%l1,[%i4]	! store new psr value
	mov	SIMU_SUCCESS, %i0
	ret
	restore
.div_by_zero:
	retl
	mov	SIMU_DZERO, %o0	! divide by zero detected
	SET_SIZE(_ip_divcc)
...
2:
	...
	subcc	%g0,%i0,%i0
	subxcc	%g0,%i1,%i1
	bvs	4f		! overflow on the complement - overflow result
	.empty			! instruction after .ldivide is OK in delay slot
...
!	st	%i2,[%i5]
	...
	restore
    !
    ! dividend complement overflowed.  This only happens if it was 0x8000..0000.
    ! Result is overflow ... maximum absolute value with the correct sign. 
    ! sign = %l3 < 0  : 0x80000000 000000000 
    ! sign = %l3 >= 0 : 0x7fffffff fffffffff
    !
4:
	set	0x80000000, %o1	! %o1 = mask 
	and	%l3, %o1, %o0	! %o0 = 80000000 or 00000000 move sign to %o0
	not	%o0		! %o0 = 7fffffff or ffffffff opposite of sign
	sra	%o0, 31, %i0	! %i0 = 00000000 or ffffffff low word of result
	andn	%i0, %o1, %o2	! %o2 = 00000000 or 7fffffff form mask
	orn	%o2, %o0, %i1	! %i1 = 80000000 or 7fffffff high word of result
	ret
	restore %g0, 0, %o2	! clear remainder
...
/*
 * Homemade PLT for "dynamic" linking of multiply and divide routines
 *
 * WARNING: self-modifying code.
 *
 * These entries must all look like:
 *
 * r:
 *	sethi	%hi(r_sw), %g1
 *	jmpl	%g1 + %lo(r_sw), %g0
 *	sethi	%hi(r_hw), %g1	! effectively a nop
 *	jmpl	%g1 + %lo(r_hw), %g0
 *
 * After patching, this looks like:
 *
 * r:
 *	sethi	%hi(r_hw), %g1
 *	jmpl	%g1 + %lo(r_hw), %g0
...
/*ARGSUSED*/
void
kern_use_hwinstr(int hwmul, int hwdiv)
{}

#else

/*
 * XXX	Watch out for supervisor unimp on Ross processors.
 */
#define	PATCH_VEC(nm)			\
	set	nm, %g2;		\
	ld	[%g2+8], %g1;		\
