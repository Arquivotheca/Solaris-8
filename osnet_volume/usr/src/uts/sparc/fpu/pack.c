...
#pragma ident	"@(#)pack.c	1.25	97/07/09 SMI"
...
packdouble(
	...
	u_int		*py)	/* and the lower 32 bits of the significand */
{
	px->sign = pu->sign;
	switch (pu->fpclass) {
	case fp_zero:
...
packextended(
	...
	u_int		*py,	/* 2nd word of extended significand */
	u_int		*pz,	/* 3rd word of extended significand */
	u_int		*pw)	/* 4th word of extended significand */
{
	px->sign = pu->sign;
	switch (pu->fpclass) {
	case fp_zero:
...
_fp_pack(
	...
	u_int		n,	/* register where datum starts */
