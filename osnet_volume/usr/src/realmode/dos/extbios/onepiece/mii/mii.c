...
mii_init_phy(mii_handle_t mac, int phy)
{
	...
	if (!phydata)
		return (MII_NOMEM);
#endif
	phydata->id = (ulong)mac->mii_read(dip, phy, MII_PHYIDH) << 16;
	phydata->id |= (ulong)mac->mii_read(dip, phy, MII_PHYIDL);
...
mii_sync(mii_handle_t mac, int phy)
{
	...
	if (ddi_getlongprop(DDI_DEV_T_ANY, mac->mii_dip,
		...
		if (phyd->fix_speed || phyd->fix_duplex)
			mii_fixspeed(mac, phy, phyd->fix_speed,
			    phyd->fix_duplex);
	}
	return (MII_SUCCESS);
}
...
mii_portmon(mii_handle_t mac)
{
	...
	mac->portmon_timer = timeout(mii_portmon, (void *)mac,
...
{
	ushort quickpoll = mac->mii_read(mac->mii_dip, phy, ICS_QUICKPOLL);
