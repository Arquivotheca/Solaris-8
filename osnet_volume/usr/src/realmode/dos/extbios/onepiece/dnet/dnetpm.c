...
#ident  "@(#)dnetpm.c 1.11     98/02/19 SMI"
...
static u_int dnetintr(gld_mac_info_t *);
#endif	/* REALMODE */

/* Internal functions used by the above entry points */
static void write_gpr(struct dnetinstance *dnetp, ulong val);
...
static void update_opr(gld_mac_info_t *macinfo);
...
/* SROM read functions */
static int dnet_read_srom(dev_info_t, int, ddi_acc_handle_t, int, u_char *,
    int);
static void dnet_read21040addr(dev_info_t *, ddi_acc_handle_t, int, u_char *,
    int *);
static void dnet_read21140srom(ddi_acc_handle_t, int, u_char *, int);
static int get_alternative_srom_image(dev_info_t, u_char *, int);
...
#ifdef BUG_4010796
static void set_alternative_srom_image(dev_info_t, u_char *, int);
...
extern u_int gld_intr(gld_mac_info_t *macinfo);
...
static void parse_controller_leaf(struct dnetinstance *dnetp, LEAF_FORMAT *leaf,
    u_char *vi);
static u_char * parse_media_block(struct dnetinstance *dnetp,
    media_block_t *block, u_char *vi);
static int check_srom_valid(u_char *);
static void dnet_dumpbin(char *msg, u_char *, int size, int len);
...
/* PHY MII Routines */
static ushort dnet_mii_read(dev_info_t *dip, int phy_addr, int reg_num);
static void dnet_mii_write(dev_info_t *dip, int phy_addr, int reg_num,
			int reg_dat);
static void write_mii(struct dnetinstance *, ulong, int);
...
ulong dnet_usecelapsed(struct dnetinstance *dnetp);
static void timestamp(struct dnetinstance *, char *);
void gptimer_cb(struct dnetinstance *dnetp);
void dnet_usectimeout(struct dnetinstance *, ulong, int, timercb_t);
...
#ifdef REALMODE
static ushort *realmode_gprseq = NULL;
...
static ddi_dma_attr_t dma_attr = {
	...
	0,			/* dma_attr_addr_lo */
	(u_int)0xFFFFFFFF,	/* dma_attr_addr_hi */
	0x7FFFFFFF,		/* dma_attr_count_max */
	4,			/* dma_attr_align */
	0x3F,			/* dma_attr_burstsizes */
	1,			/* dma_attr_minxfer */
	(u_int)0xFFFFFFFF,	/* dma_attr_maxxfer */
	(u_int)0xFFFFFFFF,	/* dma_attr_seg */
	1,			/* dma_attr_sgllen */
	1,			/* dma_attr_granular */
	0,			/* dma_attr_flags */
};
...
static char hackintr_lockname[] = "dnet_hack_intr";
...
static u_int dnet_hack_intr(struct hackintr_inf *);
...
_init(void)
{
	...
	mutex_init(&dnet_rbuf_lock, "dnet_rbuf_lock", MUTEX_DRIVER, NULL);
	setup_legacy_blocks();
...
dnetprobe(dev_info_t *devinfo)
{
	...
	if (dnetdebug & DNETDDI)
		cmn_err(CE_NOTE, "dnetprobe(0x0x%x)", devinfo);
#endif
...
#define	DNET_PCI_RNUMBER	1
...
#ifdef DNETDEBUG
	if (dnetdebug & DNETDDI)
		cmn_err(CE_NOTE, "dnethack(0x%x)", devinfo);
#endif
	 ...
	 * Reset the chip
	 */
	ddi_io_putl(io_handle, io_reg + BUS_MODE_REG, SW_RESET);
	drv_usecwait(3);
	ddi_io_putl(io_handle, io_reg + BUS_MODE_REG, 0);
	drv_usecwait(8);
...
#define	DNET_PCI_RNUMBER	1
...
#ifdef DNETDEBUG
	if (dnetdebug & DNETDDI)
		cmn_err(CE_NOTE, "dnetattach(0x%x)", devinfo);
#endif
	...
	if (gld_register(devinfo, "dnet", macinfo) == DDI_SUCCESS) {
		...
		dnet_stop_board(macinfo);
		mutex_exit(&macinfo->gldm_maclock);
		return (DDI_SUCCESS);
	}
fail1:
	dnet_detach_hacked_interrupt(devinfo);
...
dnetdetach(dev_info_t *devinfo, ddi_detach_cmd_t cmd)
{
...
#ifdef DNETDEBUG
	if (dnetdebug & DNETDDI)
		cmn_err(CE_NOTE, "dnetdetach(0x0x%x)", devinfo);
#endif
	 ...
	 */
	ddi_prop_remove(DDI_DEV_T_NONE, devinfo, "DNET_HACK");
	ddi_prop_remove(DDI_DEV_T_NONE, ddi_get_parent(devinfo), "DNET_SROM");
	ddi_prop_remove(DDI_DEV_T_NONE, ddi_get_parent(devinfo), "DNET_DEVNUM");
#endif
...
dnet_reset(gld_mac_info_t *macinfo)
{
	...
	if (dnetdebug & DNETTRACE)
		cmn_err(CE_NOTE, "dnet_reset(0x0x%x)", macinfo);
#endif
...
#ifdef DNETDEBUG
	if (dnetdebug & DNETTRACE)
		cmn_err(CE_NOTE, "dnet_init_board(0x0x%x)", macinfo);
#endif
	set_opr(macinfo);
	set_gpr(macinfo);
	set_sia(macinfo);
...
#ifdef DNETDEBUG
	if (dnetdebug & DNETTRACE)
		cmn_err(CE_NOTE, "dnet_start_board(0x0x%x)", macinfo);
#endif
...
#ifdef DNETDEBUG
	if (dnetdebug & DNETTRACE)
		cmn_err(CE_NOTE, "dnet_stop_board(0x0x%x)", macinfo);
#endif
	/*
...
dnet_saddr(gld_mac_info_t *macinfo)
{
...
#ifdef DNETDEBUG
	if (dnetdebug & DNETTRACE)
		cmn_err(CE_NOTE, "dnet_saddr(0x0x%x)", macinfo);
#endif
	val = ddi_io_getl(dnetp->io_handle, dnetp->io_reg + OPN_MODE_REG);
	 ...
	 */
	hashp = (u_long *)dnetp->setup_buf_vaddr;
	index = hashindex((u_char *)macinfo->gldm_broadcast);
...
#ifdef DNETDEBUG
	if (dnetdebug & DNETTRACE)
		cmn_err(CE_NOTE, "dnet_dlsdmult(0x0x%x, %s)", macinfo,
				op ? "ON" : "OFF");
#endif
...
#ifdef DNETDEBUG
	if (dnetdebug & DNETTRACE)
		cmn_err(CE_NOTE, "dnet_prom(0x0x%x, %s)", macinfo,
				on ? "ON" : "OFF");
#endif
...
#ifdef DNETDEBUG
	if (dnetdebug & DNETTRACE)
		cmn_err(CE_NOTE, "dnet_gstat(0x0x%x)", macinfo);
#endif
	return (DDI_SUCCESS);
}
...
dnet_send(gld_mac_info_t *macinfo, mblk_t *mp)
{
	...
	int	retval;
	mblk_t	*bp;
	u_long	tx_interrupt_mask;

#ifdef DNETDEBUG
	if (dnetdebug & DNETSEND)
		cmn_err(CE_NOTE, "dnet_send(0x%x, 0x%x)", macinfo, mp);
#endif
	if (dnetp->need_saddr)
		dnet_saddr(macinfo);
	...
	while (bp != NULL) {
		...
		while (ncookies) {
			...
			if (bufn % 2) {
			    ...
			    *(u_long *)&ring[index].desc0 = 0;
			    *(u_long *)&ring[index].desc1 &= DNET_END_OF_RING;
			    ring[index].buffer1 =
				(paddr_t)dma_cookie.dmac_address;
	 ...
	 */
	ddi_io_putl(dnetp->io_handle, dnetp->io_reg+TX_POLL_REG,
	    TX_POLL_DEMAND);
...
dnetintr(gld_mac_info_t *macinfo)
{
	...
	if (dnetdebug & DNETINT)
		cmn_err(CE_NOTE, "dnetintr(0x0x%x)", macinfo);
#endif
...
#if 0
	/*
	 * This counter is now updated in the getp routine. The DEC overrun
	 * erratum workaround will not work correctly if this is done here,
	 * because the value is self-clearing.
	 */
	...
	if (int_status & GPTIMER_INTR) {
		ddi_io_putl(dnetp->io_handle,
			    dnetp->io_reg + STATUS_REG, GPTIMER_INTR);
		if (dnetp->timer.cb)
	...
	if (int_status & RX_INTR) {
		ddi_io_putl(dnetp->io_handle,
			    dnetp->io_reg + STATUS_REG, RX_INTR);
		dnet_getp(macinfo);
	}
...
dnet_getp(gld_mac_info_t *macinfo)
{
	...
	if (dnetdebug & DNETRECV)
		cmn_err(CE_NOTE, "dnet_getp(0x0x%x)", macinfo);
#endif
	...
	while (!(desc[dnetp->rx_current_desc].desc0.own)) {
		...
		if (!desc[index].desc0.err_summary) {
			 ...
			 */
			newbuf = (u_char *)dnet_rbuf_alloc(0);
			if (newbuf != NULL) {
				frp = (struct free_ptr *)
		...
		if (desc[index].desc0.err_summary || (mp == NULL)) {
			...
			ddi_io_putl(dnetp->io_handle,
			    dnetp->io_reg + RX_POLL_REG, RX_POLL_DEMAND);
...
static void
write_gpr(struct dnetinstance *dnetp, ulong val)
{
#ifdef DEBUG
	...
	default:
		...
		ddi_io_putl(dnetp->io_handle, dnetp->io_reg + GP_REG, val);
		break;
	}
}
...
read_gpr(struct dnetinstance *dnetp)
{
	...
	case DEVICE_ID_21143:
		...
		return (ddi_io_getl(dnetp->io_handle,
			dnetp->io_reg + SIA_GENERAL_REG) >> 16);
	default:
		return (ddi_io_getl(dnetp->io_handle, dnetp->io_reg + GP_REG));
	}
}
...
set_gpr(gld_mac_info_t *macinfo)
{
	...
	if (dnetp->phyaddr != -1)
		mii_sync(dnetp->mii, dnetp->phyaddr);
	drv_usecwait(5);
}
...
set_opr(gld_mac_info_t *macinfo)
{
	...
	if (opnmode_len) {
		ddi_io_putl(dnetp->io_handle, dnetp->io_reg+OPN_MODE_REG, val);
		dnet_reset_board(macinfo);
		ddi_io_putl(dnetp->io_handle, dnetp->io_reg+OPN_MODE_REG, val);
		return;
	}
...
set_sia(gld_mac_info_t *macinfo)
{
	...
	if (block->type == 2) {
		...
		ddi_io_putl(dnetp->io_handle,
		    dnetp->io_reg + SIA_CONNECT_REG, 0);
		ddi_io_putl(dnetp->io_handle,
		    dnetp->io_reg + SIA_TXRX_REG, 0);
	}
}
...
dnet_alloc_bufs(gld_mac_info_t *macinfo)
{
	...
	if (dnetp->dma_handle == NULL)
		ddi_dma_alloc_handle(dnetp->devinfo, &dma_attr,
		    DDI_DMA_SLEEP, 0, &dnetp->dma_handle);

	if (dnetp->dma_handle_tx == NULL)
		ddi_dma_alloc_handle(dnetp->devinfo, &dma_attr_tx,
	...
	if (dnetp->rx_desc == NULL) {
		...
		dnetp->rx_buf_vaddr =
			(u_char **)kmem_zalloc(dnetp->max_rx_desc *
					sizeof (u_char **), KM_SLEEP);
		/*
...
#ifdef DNETDEBUG
	if (dnetdebug & DNETTRACE)
		cmn_err(CE_NOTE, "dnet_reclaim_Tx_desc(0x0x%x)", macinfo);
#endif
...
dnet_rbuf_init(dev_info_t *dip, int nbufs)
{
	...
	if (dma_handle == NULL) {
		ddi_dma_alloc_handle(dip, &dma_attr, DDI_DMA_SLEEP, 0,
					&dma_handle);
	}
...
dnet_rbuf_free(caddr_t vaddr)
{
	...
	if (rp == NULL) {
		cmn_err(CE_WARN, "DNET: rbuf_free: bad addr 0x%x", vaddr);
		mutex_exit(&dnet_rbuf_lock);
		return;
	}
...
dnet_read_srom(dev_info_t devinfo, int board_type, ddi_acc_handle_t io_handle,
    int io_reg, u_char *vi, int maxlen)
{
    ...
    u_char *addr, int *len)
{
	...
	*len = ddi_getprop(DDI_DEV_T_ANY, dip,
	...
	/* Reset ROM pointer */
	ddi_io_putl(io_handle, io_reg + ETHER_ROM_REG, 0);
	for (i = 0; i < *len; i++) {
		do {
			val = ddi_io_getl(io_handle, io_reg + ETHER_ROM_REG);
 ...
 */
static void
dnet_read21140srom(ddi_acc_handle_t io_handle, int io_reg, u_char *addr,
    int maxlen)
{
	u_int 		i, j;
	...
	for (i = 0; i <	maxlen; i += 2) {
		ddi_io_putl(io_handle, io_reg + ETHER_ROM_REG,
			    READ_OP | SEL_ROM);
		drv_nsecwait(30);
		ddi_io_putl(io_handle, io_reg + ETHER_ROM_REG,
...
#ifdef REALMODE
static int
get_alternative_srom_image(dev_info_t devinfo, u_char *vi, int len)
{
}

static void
set_alternative_srom_image(dev_info_t devinfo, u_char *vi, int len)
{
}
#else
...
set_alternative_srom_image(dev_info_t devinfo, u_char *vi, int len)
{
	...
	ddi_prop_create(DDI_DEV_T_NONE, ddi_get_parent(devinfo),
	    DDI_PROP_CANSLEEP, "DNET_SROM", (caddr_t)vi, len);
	ddi_prop_create(DDI_DEV_T_NONE, devinfo,
 ...
 */
static void
linkset_add(ulong *set, int media)
{
...
static int
linkset_isset(ulong linkset, int media)
{
...
find_active_media(gld_mac_info_t *macinfo)
{
	...
	if (dnetp->phyaddr != -1) {
		...
		if (ddi_getprop(DDI_DEV_T_ANY, dnetp->devinfo,
			...
			mii_start_portmon(dnetp->mii, dnet_mii_link_cb,
		...
		case 1: case 3:
			...
			for (i = 0; ; i++) {
				if (mii_linkup(dnetp->mii, dnetp->phyaddr)) {
					mii_getspeed(dnetp->mii, dnetp->phyaddr,
						&dnetp->mii_speed,
						&dnetp->mii_duplex);
					dnetp->mii_up = 1;
...
dnet_link_sense(gld_mac_info_t *macinfo)
{
	...
	struct dnetinstance *dnetp = (struct dnetinstance *)
					(macinfo->gldm_private);
	media_block_t *block = dnetp->selected_media_block;
	ulong link, status, mask, polarity;
...
#ifdef DNETDEBUG
	if (dnetdebug & DNETSENSE)
		cmn_err(CE_NOTE, "desc0 bits = %d, %d, %d, %d, %d, %d",
...
dnet_hack_interrupts(gld_mac_info_t *macinfo, int secondary)
{
	...
	if (!secondary) {
		...
		if (ddi_add_intr(devinfo, 0, &macinfo->gldm_cookie, NULL,
		    (uint (*)(char *))dnet_hack_intr,
		    (caddr_t)hackintr_inf) != DDI_SUCCESS) {
			ddi_prop_remove(DDI_DEV_T_NONE, ddi_get_parent(devinfo),
				hackintr_propname);
			goto fail;
		}
		 ...
		 */
		mutex_init(&hackintr_inf->lock, hackintr_lockname, MUTEX_DRIVER,
		    macinfo->gldm_cookie);
	...
	for (i = 0; i < MAX_INST; i++) {
		if ((mac = hackintr_inf->macinfos[i]) != NULL) {
			...
			ddi_io_putl(altdnetp->io_handle,
				altdnetp->io_reg + INT_MASK_REG, 0);
		}
	}
...
do_phy(gld_mac_info_t *macinfo)
{
	...
	setup_block(macinfo);
	mii_create(dip, dnet_mii_write, dnet_mii_read, &dnetp->mii);
...
dnet_mii_read(dev_info_t *dip, int phy_addr, int reg_num)
{
	...
	ulong data = 0;
	int i;
	int bits_in_ushort = ((sizeof (ushort))*8);
	int turned_around = 0;
	...
	command_word = (ulong)phy_addr << MII_PHY_ADDR_ALIGN;
	command_word |= (ulong)reg_num << MII_REG_ADDR_ALIGN;
	...
	tmp = ddi_io_getl(dnetp->io_handle, dnetp->io_reg + ETHER_ROM_REG);
...
dnet_mii_write(dev_info_t *dip, int phy_addr, int reg_num, int reg_dat)
{
	...
	command_word = ((ulong)phy_addr << MII_PHY_ADDR_ALIGN);
	command_word |= ((ulong)reg_num << MII_REG_ADDR_ALIGN);
	command_word |= (MII_WRITE_FRAME | (ulong)reg_dat);
 ...
 */
static void
write_mii(struct dnetinstance *dnetp, ulong mii_data, int data_size)
{
	int i;
	ulong dbit;
...
dnet_mii_link_cb(dev_info_t *dip, int phy, enum mii_phy_state state)
{
	...
	if (state == phy_state_linkup) {
		dnetp->mii_up = 1;
		mii_getspeed(dnetp->mii, dnetp->phyaddr, &dnetp->mii_speed,
			&dnetp->mii_duplex);
		dnetp->selected_media_block = leaf->mii_block;
		setup_block(macinfo);
	} else {
 ...
 */
static void
dnet_parse_srom(struct dnetinstance *dnetp, SROM_FORMAT *sr, u_char *vi)
{
	...
	if (dnetp->sr.init_from_srom && dnetp->board_type != DEVICE_ID_21040) {
		...
		if (ether_mfg == ADAPTEC_ETHER) {
			static ushort cogent_gprseq[] = {0x821, 0};
...
#else
				ddi_prop_create(DDI_DEV_T_NONE, dnetp->devinfo,
		...
		u_char *mac = vi + ddi_getprop(DDI_DEV_T_ANY, dnetp->devinfo,
...
static void
parse_controller_leaf(struct dnetinstance *dnetp, LEAF_FORMAT *leaf, u_char *vi)
{
	int i;
...
static u_char *
parse_media_block(struct dnetinstance *dnetp, media_block_t *block, u_char *vi)
{
	int i;
...
{
	int i;
	cmn_err(CE_NOTE, "Block(%x): type %x, media %s, command: %x ",
	    block, block->type, media_str[block->media_code], block->command);
	dnet_dumpbin("\tGPR Seq", (u_char *)block->gprseq, 2,
	    block->gprseqlen *2);
	dnet_dumpbin("\tGPR Reset", (u_char *)block->rstseq, 2,
	    block->rstseqlen *2);
	switch (block->type) {
	case 1: case 3:
...
#ifdef DNETDEBUG
void
dnet_usectimeout(struct dnetinstance *dnetp, ulong usecs, int contin,
    timercb_t cback)
{
...
{
	ulong ticks = dnetp->timer.start_ticks -
	    (inl(dnetp->io_reg + GP_TIMER_REG) & 0xffff);
	return ((ticks * 8192) / 100);
}

void
gptimer_cb(struct dnetinstance *dnetp)
{
	cmn_err(CE_NOTE, "Timeout!");
}

static void
timestamp(struct dnetinstance *dnetp,  char *buf)
{
	...
	for (divisor = 1000000000; divisor /= 10; divisor) {
