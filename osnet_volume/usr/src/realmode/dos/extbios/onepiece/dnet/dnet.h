...
#ident	"@(#)dnet.h	1.16	97/08/25 SMI\n"
...
struct dnetinstance {
	...
	u_char 			*setup_buf_vaddr; /* setup buffer (virt) */
	...
	mblk_t			**tx_msgbufp;	/* streams message buffers */
						/* used for xmit */
	u_char			**rx_buf_vaddr;	/* recv buf addresses (virt) */
#endif
	...
	ulong			disallowed_media;
	int			disable_scrambler;
	int			overrun_workaround;
	int			interrupts_disabled;
	int			mii_up;
	ulong			gprsia; /* Write-through for 143's gprsia reg */
