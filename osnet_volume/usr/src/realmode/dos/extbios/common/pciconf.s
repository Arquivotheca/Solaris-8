...
;
; ident	"@(#)pciconf.s	1.5	96/07/16 SMI\n"
;
...
;
;   Description:	returns 1 if device is found and 0 if not.
...
;						 unchar value);
;
;   Description:	returns 1 if byte is write and 0 if not.
;
...
;						 ushort value);
;
;   Description:	returns 1 if word is write and 0 if not.
;
...
;						 ulong value);
;
;   Description:	returns 1 if dword is write and 0 if not.
;
