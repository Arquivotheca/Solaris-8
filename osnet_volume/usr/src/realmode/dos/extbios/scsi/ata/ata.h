...
#define	ATA_START_MOTOR_WAIT	100	/* milliseconds */
#define	ATA_SENSE_RETRIES	3	/* Number of sense retries */

typedef struct atapi_ccb {
	unchar ccb_drive;		/* Send the cdb to this drive id */
	unchar ccb_lun;
	unchar ccb_scsi_cmd[16];	/* The actual cdb */

	ushort ccb_data_length;		/* original data transfer length */
	unchar far *ccb_data_addr;	/* original data address */

	ushort ccb_xfer_resid;		/* number bytes not yet transferred */
	unchar far *ccb_xferp;		/* next xfer address */

	int	ccb_rc;			/* result code */
	unchar	ccb_skey;		/* sense key from atapi error reg */
} ccb_t;
...
typedef struct ata_info {
	ushort	ata_base_port;
	ushort	ata_targid;
	ushort	ata_lun;

	unchar	ata_atapi : 1;		/* ata disk or atapi cdrom */
	unchar	ata_cdb16 : 1;		/* 12 or 16 byte CDBs */
	unchar	ata_lba_mode : 1 ;	/* LBA mode disk */

	unchar	ata_single_sector : 1 ;	/* do all reads 1 sector at a time */
	unchar	ata_bogus_bsy : 1 ;	/* doesn't manage ATS_BSY correctly */
	unchar	ata_nec_bad_status : 1 ;/* bogus intr reason for status phase */
	unchar	ata_bogus_drq : 1;	/* drq toggles instead of busy bit */
	unchar	ata_fellback : 1 ;	/* fallback mode enabled */

	unchar	ata_state;	/* state of the ATAPI FSM */
	ushort	ata_drq_delay;	/* CMD DRQ delay for atapi devs */

	ushort  ata_cyls;	/* cylinders,  1 based */
	ushort  ata_heads;	/* heads,  1 based */
	ushort  ata_secs;	/* sectors per track, 1 based */
	ulong	ata_disksize;	/* total sectors, 1 based */
} ata_info_t;



/*
 * Global function prototypes for main.c
 */
...
void	bzero(unchar *p, ulong n);
int	ata_poll1(ushort, unchar, unchar, long interval);
int	ata_poll3(ushort, unchar, unchar, unchar, unchar, unchar, unchar, long);
void	repoutsw(ushort port, ushort *buffer, ulong count);
void	ata_pause(void);

/*
 * global functions in ata.c
 */
int	ata_pio_data_in(uint ioaddr1, uint ioaddr2, unchar cmd,
			unchar far * bufaddr, ushort sec_count, int long_wait,
			int expect_drdy);
int	ata_id(uint ioaddr1, uint ioaddr2);
int	ata_inquire(ata_info_t *atap, ushort base_port, ushort targid,
		    ushort lun);
int	ata_lock(ata_info_t *atap, int lock);
int	ata_motor(ata_info_t *atap, int on);
int	ata_read(ata_info_t *atap, ulong start_block, ushort sec_count,
		 ushort bsize, unchar far *bufaddr);
int	ata_readcap(ata_info_t *atap);
int	ata_sense(ata_info_t *atap, ushort base_port, ushort targid,
		  ushort lun);

/*
 * global functions in atapi.c
 */
int	atapi_check_for_atapi_12(uint ioaddr1, uint ioaddr2);
void	atapi_cmd_error(ata_info_t *atap, ushort ioaddr1);
int	atapi_id(uint ioaddr1, uint ioaddr2, int long_wait);
int	atapi_inquire(ata_info_t *atap, ushort base_port, ushort targid,
		      ushort lun);
int	atapi_lock(ata_info_t *atap, int lock);
int	atapi_mode_sense(ata_info_t *atap, unchar page_code, unchar far *mbuf,
			 ushort msize);
int	atapi_motor(ata_info_t *atap, int on);
int	atapi_read(ata_info_t *atap, ulong start_block, ushort sec_count,
		   ushort bsize, unchar far *bufaddr);
int	atapi_readcap(ata_info_t *atap);
int	atapi_sense(ata_info_t *atap, ushort base_port, ushort targid,
		    ushort lun);
void	atapi_chk_for_lame_drive(ata_info_t *atap, struct ata_id *aidp);

void	readphys(unchar far *buf, ushort ioport, ushort count);

/* return codes from atapi functions called by main */
#define	ATA_IO_OKAY		0
#define	ATA_CHECK_CONDITION	1
#define	ATA_FATAL_ERROR		2

/* failure return code for atapi_sense() */
#define ATAPI_REQSENSE_FAILED	-1
 ...
 * Math routines
 */
ulong ULongMult( ulong M1, ulong M2 );
ulong ULongDiv( ulong Dividend, ulong Divisor );
ulong ULongRem( ulong Dividend, ulong Divisor );
