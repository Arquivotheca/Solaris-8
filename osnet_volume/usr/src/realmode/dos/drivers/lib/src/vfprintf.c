...
#ident "@(#)vfprintf.c	1.4	97/03/17 SMI"
 
/*
 *	Real mode printf support:
 *
 ...
 *
 *	    The only stdio routine called from the real mode printf routines
 *	    is fputc.  This routine can be implemented with a low-level
 *	    "putchar", thereby allowing printf to be called from real mode
...
vfprintf(FILE *fp, const char *fmt, va_list ap)
{
	 ...
	 *
	 *  This is the primary real mode printf routine; other printf variants
	 ...
	 *
	 *	1.  We don't support floating point format specs (%e, %f, %g).
