...
#pragma ident	"@(#)readfile_i386.c	1.35	99/08/19 SMI"
...
extern unsigned int prom_vlimit();
	...
	register char **arg;
{
	...
	*arg = "";
	...
	if (BOP_GETPROP(bopp, "boot-args", bargs) != -1)
		...
		printf("missing boot-args property. boot problem?\n");
		howto = 0;
	}
	...
	for (i = 0; i < nphdrs; i++) {
		...
		if (phdr->p_type == PT_LOAD) {
			...
			if (phdr->p_flags & PF_X) {
				...
				sav_kadb_heaptr = (caddr_t)prom_vlimit(0);
				err = prom_alloc((caddr_t)loadaddr, size,
				    align);
				if (err != (caddr_t)base)
					prom_panic("Unable to get memory "
					    "for text_seg.\n");
				prom_vlimit(sav_kadb_heaptr);
...
segbrk(caddr_t *spp, unsigned int bytes, int align)
{
	...
	if (va + bytes > pva) {
		...
		sav_kadb_heaptr = (caddr_t)prom_vlimit(0);
		if (prom_alloc(pva, size, alloc_align) != pva) {
