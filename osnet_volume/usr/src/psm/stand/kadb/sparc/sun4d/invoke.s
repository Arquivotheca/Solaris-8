...
#pragma ident "@(#)invoke.s	1.3	97/11/25 SMI"

/*
 * Kernel function call invocation for SPARC V8 systems
 */
...
kernel_invoke(unsigned long (*func)(unsigned long, ...),
	...
	bgu	    pushargs		! check for stack args
...
pushargs:
	...
	ld	    [%l0 + %i1], %l2	! last arg
1:
	st	    %l2, [%l1 + %i1]
	subcc	    %i1, 4, %i1
	bnz,a	    1b			! stop when %i1 is 0
	ld	    [%l0 + %i1], %l2	! next arg down
